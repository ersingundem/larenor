...
@pytest.mark.parametrize(
    "user",
    [
        {"id": 2, "permissions": 2, "userType": 3, "jellyfinUsername": USERNAME},
        {"id": 1, "permissions": 32, "userType": 3, "jellyfinUsername": USERNAME},
        {"id": 1, "permissions": 2, "userType": 1, "jellyfinUsername": USERNAME},
        {"id": 1, "permissions": 2, "userType": 3, "jellyfinUsername": "foreign"},
    ],
)
...
def test_untrusted_inputs_send_nothing(username, credential):
    ...
    with pytest.raises(SeerrInitialAdminError, match="^invalid_seerr_initial_admin$"):
        SeerrInitialAdmin().create(
            connection,
            username=username,
