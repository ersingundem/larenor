...
def legacy_v2(app):
    with app.state.core.db.transaction() as connection:
        ...
        # Persisted automation rules also require the later Core identity.
        for table in ('automation_rule_records', 'automation_rule_state'):
            connection.execute(f'DROP TABLE {table}')
        connection.execute("DELETE FROM metadata WHERE key='automation_rule_schema'")
        ...
        # Central infrastructure bindings also postdate the context-free v2 schema.
