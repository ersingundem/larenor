...
def records():
    return [
        record(
            ...
            FRONT,
            1_000_000,
            "Ön kapıda paket bırakıldı",
            ["paket", "kişi"],
        ),
        record(
            "9" * 32, "a" * 32, FRONT, 1_100_000, "Kedi kapının önünden geçti", ["kedi"]
        ),
...
def request(
    query="ön kapıda paket", *, cameras=(FRONT,), cursor=None, page_size=10, revision=11
):
...
def test_local_metadata_search_is_scoped_revision_bound_and_explicitly_degraded():
    foreign = record(
        "0" * 32, "1" * 32, FRONT, 1_400_000, "Ön kapıda paket", ["paket"]
    ).model_copy(update={"homeId": "0" * 32})
...
def test_cursor_is_opaque_bounded_and_bound_to_query_scope_authority_and_revision():
    ...
    second = search.search(
        scoped, request(query="kapı", page_size=1, cursor=first.nextCursor)
    )
    ...
    with pytest.raises(ApiError, match="revision_conflict"):
        index(revision=12).search(
            authority(), request(query="kapı", page_size=1, cursor=first.nextCursor)
        )
...
def test_privacy_camera_and_live_authority_fail_closed_before_planner():
    ...
    search = index(
        current=current, planner=lambda query: planned.append(query) or [query]
    )
    ...
    unavailable = CameraSearchIndex(
        ...
        revision=11,
        cursorKey=b"k" * 32,
        authorityResolver=lambda _account: (_ for _ in ()).throw(
...
def test_private_owner_or_explicit_admin_can_read_without_leaking_to_members():
    ...
    admin = authority(
        account="e" * 32, role="admin", cameras=(FRONT, GARDEN), private=True
    )
...
def test_semantic_planner_is_query_only_bounded_and_failure_degrades_locally():
    ...
    degraded = index(
        planner=lambda _query: (_ for _ in ()).throw(RuntimeError("private"))
    )
...
def test_time_bounds_deleted_index_snapshot_and_invalid_contracts_fail_closed():
    ...
    assert (
        search.search(
            authority(), request(query="paket").model_copy(update={"endMs": 999_999})
        ).results
        == []
    )
    without_clip = index(source=records()[1:], revision=12)
    assert (
        without_clip.search(authority(), request(query="paket", revision=12)).results
        == []
    )
...
def test_results_are_bounded_deterministic_and_secret_free():
    ...
    result = index(source=many).search(
        authority(), request(query="paket", page_size=50)
    )
