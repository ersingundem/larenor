"""Exact managed-container endpoint proof before Qbittorrent bootstrap."""
...
from larenor_server.plugins.qbittorrent_endpoint import (
    QbittorrentEndpointError, open_qbittorrent_endpoint, prove_qbittorrent_endpoint,
)
from test_managed_container_binding import build_qbittorrent, snapshot
...
def test_running_exact_container_yields_private_fixed_qbittorrent_endpoint():
    stack, binding, observed = running()
    proof = prove_qbittorrent_endpoint(observed, binding, stack, '5' * 64)
    assert (proof.container_id, proof.network_id, proof.address, proof.port,
            proof.plan_hash) == (
        '5' * 64, binding.network_id, '172.28.0.2', 8080, stack.planHash,
    )
...
def test_drift_or_nonprivate_endpoint_never_becomes_connection_authority(damage, monkeypatch):
    ...
    with pytest.raises(QbittorrentEndpointError, match='^qbittorrent_endpoint_untrusted$'):
        open_qbittorrent_endpoint(observed, binding, stack, '5' * 64)
...
def test_connector_uses_one_numeric_ipv4_target_without_dns_or_retry(monkeypatch):
    ...
    opened = open_qbittorrent_endpoint(observed, binding, stack, '5' * 64, timeout=3.0)
    ...
    assert connection.calls[-1] == ('connect', ('172.28.0.2', 8080))
...
def test_connect_failure_closes_once_and_returns_only_a_static_code(monkeypatch):
    ...
    with pytest.raises(QbittorrentEndpointError, match='^qbittorrent_endpoint_unavailable$') as raised:
        open_qbittorrent_endpoint(observed, binding, stack, '5' * 64)
...
def test_timeout_is_packaged_and_invalid_values_open_no_socket(timeout, monkeypatch):
    ...
    with pytest.raises(QbittorrentEndpointError, match='^invalid_qbittorrent_endpoint_limits$'):
        open_qbittorrent_endpoint(observed, binding, stack, '5' * 64, timeout=timeout)


def test_closed_surface_has_no_client_selected_target_or_transport_hooks():
    parameters = inspect.signature(open_qbittorrent_endpoint).parameters
