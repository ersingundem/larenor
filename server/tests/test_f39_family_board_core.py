...
def board(tmp_path):
    ...
    store = FamilyBoardStore(
        path, b"k" * 32, lambda account_id: current.get(account_id), clock=lambda: 17.0
    )
...
def test_idempotent_mutations_conflict_replay_and_immutable_audit(board):
    ...
    with pytest.raises(ApiError, match="revision_conflict"):
        store.apply(
            authority(), command("b" * 32, 0, "update", element=card(text="late"))
        )
    ...
    updated = store.apply(
        authority(), command("c" * 32, 1, "update", element=card(text="Film gecesi"))
    )
    deleted = store.apply(
        authority(), command("d" * 32, 2, "delete", element_id=card().id)
    )
...
def test_encrypted_restart_role_authority_and_secret_free_exports(board):
    ...
    receipt = store.apply(
        authority(), command("9" * 32, 0, "append", element=card(text=secret_text))
    )
    raw = path.read_bytes()
    assert secret_text.encode() not in raw
    assert os.stat(path).st_mode & 0o077 == 0

    restarted = FamilyBoardStore(
        path, b"k" * 32, lambda account_id: current.get(account_id), clock=lambda: 18.0
    )
...
def test_tamper_and_deleted_journal_fail_closed(board, tamper):
    ...
    with store._connection() as connection:
        if tamper == "ciphertext":
            connection.execute(
                "UPDATE family_board_events SET ciphertext=? WHERE sequence=1",
                (b"x" * 32,),
            )
        elif tamper == "delete":
...
def test_authority_change_during_write_is_rejected_without_effect(tmp_path):
    ...
    with store._connection() as connection:
        assert (
            connection.execute("SELECT COUNT(*) FROM family_boards").fetchone()[0] == 0
        )
        assert (
            connection.execute("SELECT COUNT(*) FROM family_board_events").fetchone()[0]
            == 0
        )
