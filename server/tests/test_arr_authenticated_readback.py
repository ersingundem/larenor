...
def test_auth_failure_is_static_and_not_retried(status):
    ...
    with pytest.raises(
        ArrAuthenticatedReadbackError, match='^arr_authentication_failed$'
    ) as raised:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {'appName': 'Sonarr', 'version': '4.0.20.0'},
    ],
)
def test_cross_service_or_version_drift_is_rejected(value):
    with pytest.raises(ArrAuthenticatedReadbackError, match='^arr_readback_mismatch$'):
        ArrAuthenticatedReadback().read(
            Connection([result('sonarr', value)]), service_id='sonarr', api_key=KEY
        )
...
@pytest.mark.parametrize(
    ...
    [
        b'[]',
        b'{}',
        b'{"appName":"Sonarr","appName":"Sonarr","version":"4.0.19.2979"}',
        b'no',
    ],
)
...
@pytest.mark.parametrize(
    'service,key', [('other', KEY), ('sonarr', 'short'), ('sonarr', True)]
)
def test_closed_inputs_have_no_target_or_header_surface(service, key):
    c = Connection([])
    with pytest.raises(
        ArrAuthenticatedReadbackError, match='^invalid_arr_authenticated_readback$'
    ):
        ArrAuthenticatedReadback().read(c, service_id=service, api_key=key)
    assert not {'url', 'host', 'resolver', 'proxy', 'headers', 'token'} & set(
        inspect.signature(ArrAuthenticatedReadback.read).parameters
    )
...
def test_timeout_closes_once_without_retry():
    ...
    c = Stalled([])
    with pytest.raises(
        ArrAuthenticatedReadbackError, match='^arr_authenticated_readback_timeout$'
    ):
        ArrAuthenticatedReadback().read(
            c,
            service_id='sonarr',
            api_key=KEY,
            limits=ArrAuthenticatedReadbackLimits(total_seconds=0.01),
        )
