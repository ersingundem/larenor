...
import pytest
from larenor_server.plugins.arr_endpoint import (
    ArrEndpointError,
    open_arr_endpoint,
    prove_arr_endpoint,
)
...
def build(service, container_journal_id='4' * 32):
    ...
    def provider(resources, volumes, component):
        image = next(
            ...
            if x.kind == 'ensure_image' and x.serviceId == service
        )
        network = resources.resources[-1]
        selected = tuple(
            x
            for x in volumes.resources
            if x.serviceId == service or x.kind == 'managed_library'
        )
        return VerifiedJellyfinResources(
            ...
            ManagedImageProof(
                ...
                json.dumps(
                    {'Env': ['PATH=/usr/bin'], 'Volumes': {'/config': {}}},
                    sort_keys=True,
                    separators=(',', ':'),
                ).encode(),
            ),
    ...
    binding = JellyfinBindingBuilder(
        catalog, policy, container_journal_id, provider, service_id=service
    )(stack)
...
def test_exact_running_container_yields_fixed_private_endpoint(service, port):
    stack, binding, observed = build(service)
    proof = prove_arr_endpoint(observed, binding, stack, '5' * 64, service)
    assert (proof.service_id, proof.port, proof.address) == (
        service,
        port,
        '172.28.0.2',
    ) and '172.28' not in repr(proof)


@pytest.mark.parametrize(
    'damage', ['id', 'stopped', 'public', 'network', 'extra', 'cross_service']
)
def test_drift_never_opens_socket(damage, monkeypatch):
    stack, binding, observed = build('sonarr')
    service = 'sonarr'
    if damage == 'id':
        observed['Id'] = '6' * 64
    elif damage == 'stopped':
    ...
    with pytest.raises(ArrEndpointError, match='^arr_endpoint_untrusted$'):
        open_arr_endpoint(observed, binding, stack, '5' * 64, service)
...
class Connection:
    ...
    def settimeout(self, v):
        self.calls.append(('timeout', v))

    def connect(self, v):
        self.calls.append(('connect', v))
        (_ for _ in ()).throw(OSError()) if self.fail else None
...
def test_opens_one_numeric_target_without_dns_or_retry(monkeypatch):
    stack, binding, observed = build('radarr')
    c = Connection()
    monkeypatch.setattr(
        socket, 'getaddrinfo', lambda *_: (_ for _ in ()).throw(AssertionError())
    )
    monkeypatch.setattr(socket, 'socket', lambda *_: c)
    opened = open_arr_endpoint(observed, binding, stack, '5' * 64, 'radarr')
    assert opened.connection is c and c.calls[-1] == ('connect', ('172.28.0.2', 7878))
...
def test_failure_closes_once(monkeypatch):
    ...
    c = Connection(True)
    monkeypatch.setattr(socket, 'socket', lambda *_: c)
    with pytest.raises(ArrEndpointError, match='^arr_endpoint_unavailable$'):
        open_arr_endpoint(observed, binding, stack, '5' * 64, 'sonarr')
