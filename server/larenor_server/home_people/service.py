"""Encrypted household metadata in a separate storage/cryptographic domain. No account or upstream effects."""
...
from .models import CreatePersonRequest, SetPersonGrantRequest, StoredPerson, UpdatePersonRequest
from .authorization import evaluate_access
from .models import ActorFacts, PersonGrant, HomeScope, Permissions, PersonRecord, PersonRef, PersonTargetFacts
from . import schema
...
class HomePeopleRegistry:
    ...
    def _state(self, c):
        rows = c.execute('SELECT * FROM home_people_state LIMIT 2').fetchall()
    ...
    def _bump(self, c, *, records=0, grants=0):
        ...
        if not 0 <= nr <= schema.MAX_RECORDS or not 0 <= ng <= schema.MAX_TOTAL_GRANTS:
            raise ApiError('revision_conflict', 409)
        c.execute('UPDATE home_people_state SET revision=?,record_count=?,grant_count=?,authentication_tag=? WHERE singleton=1',
                  (revision, nr, ng, schema.state_tag(self._key, self.scope, revision, nr, ng)))

    def _aad(self, row):
        return (f'larenor-home-person-v1:{self.scope.coreId}:{self.scope.homeId}:'
    ...
    def _decode(self, row):
        ...
        ref = PersonRef(**self.scope.model_dump(), id=row['id'], kind=row['kind'])
        ...
        data = StoredPerson.model_validate_json(self._cipher.decrypt(row['nonce'], row['ciphertext'], self._aad(row)))
    ...
    def _save(self, c, row, data):
        data = StoredPerson.model_validate(data)
        plain = data.model_dump_json().encode('utf-8')
        if len(plain) > 32752:
            raise ApiError('invalid_request')
        nonce = secrets.token_bytes(12)
        c.execute('INSERT INTO home_people_records VALUES(?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET '
    ...
    def _transaction(self, actor, core_id, home_id, *, admin=False, action=None, target_id=None):
        self.auth.rate_limit([('home_people_write' if action else 'home_people_read', actor.id, 120)])
        error = None
        try:
            with self.db.transaction() as c:
                ...
                if action:
                    c.execute('INSERT INTO home_people_audit(action,status,actor_id,target_id,created_at) VALUES(?,?,?,?,?)',
                              (action, 'denied' if error else 'success', actor.id, target_id, self.settings.clock()))
                    c.execute('DELETE FROM home_people_audit WHERE sequence IN (SELECT sequence FROM home_people_audit '
    ...
    def _target(self, c, record_id):
        self._id(record_id)
        row = c.execute('SELECT * FROM home_people_records WHERE id=?', (record_id,)).fetchone()
        if row is None:
            raise ApiError('not_found', 404)
    ...
    def _decision(self, actor, row, ref, data, action, **expected):
        ...
        grant = None if permission is None else PersonGrant(subjectId=actor.userId, target=ref,
            aclRevision=row['acl_revision'], permissions=permission)
        return evaluate_access(self.scope, actor, PersonTargetFacts(ref=ref, revision=row['revision'],
    ...
    def _public(self, actor, row, ref, data):
        ...
        return PersonRecord(ref=ref, revision=row['revision'], aclRevision=row['acl_revision'],
    ...
    def validate_storage(self):
        try:
            with self.db.connection() as c:
                ...
                for row in c.execute('SELECT * FROM home_people_records LIMIT ?', (schema.MAX_RECORDS + 1,)):
                    records += 1
                ...
                if (records, grants) != (state['record_count'], state['grant_count']):
                    raise ValueError()
                if c.execute('SELECT COUNT(*) FROM home_people_audit').fetchone()[0] > schema.MAX_AUDIT:
                    raise ValueError()
    ...
    def list(self, actor, core_id, home_id, *, after=None, expected_snapshot=None, limit=25):
        ...
        with self._transaction(actor, core_id, home_id) as (c, facts):
            state = self._state(c)
            digest = hmac.new(self._key, b'larenor:home-person-visible-snapshot:v1\0', 'sha256')
            ...
            for row in c.execute('SELECT * FROM home_people_records ORDER BY id LIMIT ?',
    ...
    def get(self, actor, core_id, home_id, record_id):
        with self._transaction(actor, core_id, home_id) as (c, facts):
            ...
            return {'person': self._public(facts, row, ref, data)}
    ...
    def create(self, actor, core_id, home_id, body):
        ...
        with self._transaction(actor, core_id, home_id, admin=True, action='create', target_id=identity) as (c, facts):
            if self._state(c)['record_count'] >= schema.MAX_RECORDS:
                raise ApiError('revision_conflict', 409)
            row = {'id': identity, 'kind': 'person', 'revision': 1, 'acl_revision': 1}
            data = StoredPerson(label=body.label, order=body.order, grants={})
            self._save(c, row, data); self._bump(c, records=1)
            ref = PersonRef(**self.scope.model_dump(), id=identity, kind='person')
            result = self._public(facts, row, ref, data)
        return {'person': result}

    def update(self, actor, core_id, home_id, record_id, body):
        body = UpdatePersonRequest.model_validate(body)
        with self._transaction(actor, core_id, home_id, admin=True, action='update', target_id=record_id) as (c, facts):
            ...
            if (body.label, body.order) != (data.label, data.order):
                ...
                data = StoredPerson(label=body.label, order=body.order, grants=data.grants)
    ...
    def delete(self, actor, core_id, home_id, record_id, expected_revision, expected_acl_revision):
        ...
        with self._transaction(actor, core_id, home_id, admin=True, action='delete', target_id=record_id) as (c, facts):
            ...
            c.execute('DELETE FROM home_people_records WHERE id=?', (record_id,))
    ...
    def grants(self, actor, core_id, home_id, record_id):
        with self._transaction(actor, core_id, home_id, admin=True) as (c, _):
            row, ref, data = self._target(c, record_id)
            return {'aclRevision': row['acl_revision'], 'grants': [PersonGrant(subjectId=subject, target=ref,
    ...
    def set_grant(self, actor, core_id, home_id, record_id, subject_id, body):
        body = SetPersonGrantRequest.model_validate(body); self._id(subject_id)
        with self._transaction(actor, core_id, home_id, admin=True, action='grant', target_id=record_id) as (c, facts):
            ...
            if existing != body.permissions:
                ...
                data = StoredPerson(label=data.label, order=data.order, grants=grants)
                self._save(c, row, data); self._bump(c, grants=delta)
            result = PersonGrant(subjectId=subject_id, target=ref, aclRevision=row['acl_revision'],
