...
from .models import (
    IrrigationAuthority, IrrigationInputRevisions, IrrigationPlan,
    IrrigationPlanItem, IrrigationPolicy, IrrigationSafetySnapshot,
    ManualWaterOverride, RainForecast, SoilMoistureReading, WaterBudget,
)
...
class IrrigationPlanner:
    ...
    def _authority(self, raw):
        ...
        try:
            current = IrrigationAuthority.model_validate(self._resolve_authority(value.accountId))
        except Exception:
    ...
    def _policy(self, raw):
        try:
            ...
            raise ApiError("invalid_request") from None
        try:
            current = IrrigationPolicy.model_validate(self._resolve_policy(value.policyId))
        except Exception:
    ...
    def plan(self, rawAuthority, rawPolicy, rawSoil, rawSafety, rawForecast, rawBudget,
             *, nowMs, overrides=()):
        ...
        if any((value.coreId, value.homeId) != (policy.coreId, policy.homeId)
        ...
        for zone_id, reading in readings.items():
            ...
            if (reading.coreId, reading.homeId, reading.zoneRevision) != (
                policy.coreId, policy.homeId, zone.zoneRevision
            ):
                raise ApiError("revision_conflict", 409)
        active_overrides = {}
        for item in overrides:
            zone = zones.get(item.zoneId)
            if zone is None or (item.coreId, item.homeId, item.policyId,
                    item.expectedPolicyRevision, item.expectedZoneRevision,
                    item.actorAccountId) != (
                policy.coreId, policy.homeId, policy.policyId,
                policy.policyRevision, zone.zoneRevision, authority.accountId
            ):
                raise ApiError("revision_conflict", 409)
        ...
        safety_reason = None
        if safety.observedAtMs > nowMs or nowMs - safety.observedAtMs > policy.safetyMaxAgeMs:
        ...
        for zone_id in sorted(zones):
            ...
            elif reading.observedAtMs > nowMs or nowMs - reading.observedAtMs > policy.soilMaxAgeMs:
                reason, status = "soil_sensor_stale", "blocked"
            elif zone_id in active_overrides:
                reason, duration = "manual_override", active_overrides[zone_id].durationSeconds
            elif reading.moisturePermille >= policy.targetMoisturePermille:
                reason, status = "moisture_sufficient", "skipped"
            elif forecast_available and forecast.rainMilliMm >= policy.rainDeferralMilliMm:
                reason, status = "rain_forecast", "deferred"
            else:
                deficit = policy.targetMoisturePermille - reading.moisturePermille
                duration = max(1, deficit * zone.maxDurationSeconds // policy.targetMoisturePermille)
            ...
            if status == "planned" and water > remaining:
                ...
                if affordable_seconds < 1:
                    reason, status, duration, water = "budget_exhausted", "skipped", 0, 0
                else:
            ...
            if status != "planned":
                duration, water = 0, 0
            remaining -= water
            items.append(IrrigationPlanItem(
                schemaVersion=1, zone=zone, status=status, reason=reason,
                durationSeconds=duration, estimatedWaterMl=water,
            ))
        ...
        revisions = IrrigationInputRevisions(
            ...
            soilSensorRevisions={key: readings[key].sensorRevision for key in sorted(readings)},
            soilReadingRevisions={key: readings[key].readingRevision for key in sorted(readings)},
        )
        total = sum(item.estimatedWaterMl for item in items)
        identity = {
            "core": policy.coreId, "home": policy.homeId, "policyHash": policy_hash,
            "actor": authority.accountId, "accountRevision": authority.accountRevision,
            "at": nowMs, "inputs": revisions.model_dump(mode="json"),
            "soil": [readings[key].model_dump(mode="json") for key in sorted(readings)],
            "safety": safety.model_dump(mode="json"),
            "forecast": forecast.model_dump(mode="json"),
            "budget": budget.model_dump(mode="json"),
            "overrides": [item.model_dump(mode="json") for item in sorted(
                active_overrides.values(), key=lambda value: value.zoneId
            )],
            "items": [item.model_dump(mode="json") for item in items],
        }
        return IrrigationPlan(
            schemaVersion=1, planId=_hash(identity)[:32], coreId=policy.coreId,
            ...
            inputRevisions=revisions, items=items, totalWaterMl=total,
