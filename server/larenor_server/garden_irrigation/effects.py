...
from .models import (
    IrrigationAuthority, IrrigationPlan, IrrigationPolicy, IrrigationPreview,
    IrrigationReceipt, ValveCommandResult, ValveReadback, ValveWorkerCommand,
    WorkerValveReadback,
)
...
class IrrigationCoordinator:
    def __init__(self, *, audit, authorityResolver, policyResolver, planResolver,
                 maxReceipts=1024):
    ...
    def _live(self, authority, plan):
        ...
        try:
            current_plan = IrrigationPlan.model_validate(self._resolve_plan(plan.planId))
        except Exception:
            raise ApiError("revision_conflict", 409) from None
        if current_authority != authority:
            raise ApiError("revision_conflict", 409)
        if not authority.active or authority.role != "admin" or not authority.canManageIrrigation:
            raise ApiError("forbidden", 403)
        if (plan.coreId, plan.homeId, plan.homeRevision, plan.actorAccountId,
                plan.accountRevision, plan.sessionFamilyId) != (
        ...
        ) or _hash(current_policy.model_dump(mode="json")) != plan.policyHash or current_plan != plan:
            raise ApiError("revision_conflict", 409)
    ...
    def preview(self, rawAuthority, rawPlan, rawReadbacks, *, requestId, nowMs):
        ...
        try:
            readbacks = tuple(ValveReadback.model_validate(item) for item in rawReadbacks)
            if (not isinstance(requestId, str) or len(requestId) != 32
                    or any(char not in "0123456789abcdef" for char in requestId)
                    or type(nowMs) is not int or nowMs < plan.generatedAtMs):
                raise ValueError
        except ValueError:
            raise ApiError("invalid_request") from None
        planned = {item.zone.zoneId: item for item in plan.items if item.status == "planned"}
        observed = {item.zone.zoneId: item for item in readbacks}
        if not planned or len(observed) != len(readbacks) or set(observed) != set(planned):
            raise ApiError("revision_conflict", 409)
        for zone_id, item in observed.items():
            if (item.coreId, item.homeId, item.zone) != (
                plan.coreId, plan.homeId, planned[zone_id].zone
            ) or item.observedAtMs > nowMs or item.valveOpen:
                raise ApiError("revision_conflict", 409)
        with self._lock:
            old_id = self._request_previews.get(requestId)
            if old_id is not None:
                old = self._previews[old_id]
                if (old.authority, old.plan, old.readbacks) != (authority, plan, readbacks):
                    raise ApiError("idempotency_conflict", 409)
                return IrrigationPreview(
                    schemaVersion=1, previewId=old_id, confirmToken=old.token,
                    requestId=old.request_id, planId=old.plan.planId,
                    policyRevision=old.plan.policyRevision, expiresAtMs=old.expires_at,
                    commandCount=sum(item.status == "planned" for item in old.plan.items),
                )
            ...
            stored = _StoredPreview(authority, plan, readbacks, requestId, token, expires)
            digest = _hash(plan.model_dump(mode="json"))
            self._audit.append(
                kind="preview", policyId=plan.policyId, policyRevision=plan.policyRevision,
                actorAccountId=authority.accountId, requestId=requestId,
                status="accepted", payloadHash=digest, atMs=nowMs,
            )
            ...
            return IrrigationPreview(
                ...
                policyRevision=plan.policyRevision, expiresAtMs=expires,
    ...
    def confirm(self, rawAuthority, previewId, confirmToken, *, nowMs, worker):
        with self._lock:
            ...
            if (stored is None or not isinstance(confirmToken, str)
            ...
            if old is not None:
                return old
            if type(nowMs) is not int or nowMs > stored.expires_at or nowMs < plan.generatedAtMs:
                raise ApiError("invalid_request")
            ...
            for zone_id in sorted(planned):
                ...
                command = ValveWorkerCommand(
                    schemaVersion=1, commandId=command_id, requestId=stored.request_id,
                    planId=plan.planId, policyRevision=plan.policyRevision,
                    actorAccountId=authority.accountId, zone=item.zone,
                ...
                try:
                    ...
                    results.append(ValveCommandResult(
                ...
                try:
                    ...
                    results.append(ValveCommandResult(
                        schemaVersion=1, commandId=command_id, zoneId=zone_id,
                        status="failed", code="readback_mismatch", readback=None,
                    ))
                    continue
                exact = (result.commandId == command_id and result.zone == command.zone
                         and result.stateRevision > before.stateRevision
                         and result.observedAtMs >= before.observedAtMs and not result.valveOpen)
                ...
                code = "applied" if exact and flow and bounded_flow else (
                    "readback_mismatch" if not exact else
                    "flow_not_verified" if not flow else "flow_out_of_bounds"
                )
                results.append(ValveCommandResult(
                    schemaVersion=1, commandId=command_id, zoneId=zone_id,
                    status="applied" if code == "applied" else "failed",
                    code=code, readback=result,
                ))
            states = {item.status for item in results}
            status = "applied" if states == {"applied"} else (
                "partial" if len(states) > 1 else next(iter(states))
            )
            receipt = IrrigationReceipt(
                schemaVersion=1, requestId=stored.request_id, planId=plan.planId,
                status=status, results=results, completedAtMs=nowMs,
            )
            digest = _hash(receipt.model_dump(mode="json"))
            self._audit.append(
                kind="result", policyId=plan.policyId, policyRevision=plan.policyRevision,
                actorAccountId=authority.accountId, requestId=stored.request_id,
                status=status, payloadHash=digest, atMs=nowMs,
            )
