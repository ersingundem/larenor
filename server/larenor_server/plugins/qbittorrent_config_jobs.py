"""Encrypted, durable qBittorrent config coordination with no automatic retry."""
...
from .qbittorrent_config_job_models import (
    ...
    PrivateQbittorrentReceipt, QbittorrentConfiguration,
...
class QbittorrentConfigurationManagement:
    ...
    @staticmethod
    def _aad(row):
        return b'larenor:media:qbittorrent-configurations:schema=1:' + json.dumps(
    ...
    def _public(row, payload=None):
        ...
        return QbittorrentConfiguration.model_validate({
            ...
            'inspectionId': row['inspection_id'], 'serviceId': 'qbittorrent',
    ...
    def _decode(self, row):
        try:
            if (len(row['nonce']) != 12
                ...
                raise ValueError()
            payload = QbittorrentConfigurationPayload.model_validate_json(
            ...
            raise ApiError('media_qbittorrent_configuration_storage_unavailable', 503) from None
    ...
    def _validate_row(self, connection, row):
        ...
        try:
            ...
            raise ApiError(
                'media_qbittorrent_configuration_storage_unavailable', 503) from None
        return payload
    ...
    def _save(self, connection, row, payload):
        ...
        connection.execute(
            'UPDATE media_qbittorrent_configurations SET '
        ...
        try:
            with self.db.connection() as connection:
                ...
                rows = connection.execute(
                    'SELECT * FROM media_qbittorrent_configurations LIMIT ?',
            ...
            raise StartupError('invalid_media_qbittorrent_configurations_storage') from None
    ...
    def capabilities(self, actor):
        with self.db.connection() as connection:
            ...
            return {'executionConfigured': self.backend is not None,
                    'installAvailable': False, 'serviceId': 'qbittorrent'}

    def create(self, actor, body):
        body = _body(body, CreateQbittorrentConfigurationRequest)
        with self.db.transaction() as connection:
            ...
            if connection.execute(
                    'SELECT 1 FROM media_qbittorrent_configurations WHERE preparation_id=?',
                    (body.preparationId,)).fetchone() is not None:
                raise ApiError('media_qbittorrent_configuration_conflict', 409)
            ...
            if connection.execute(
                    'SELECT COUNT(*) FROM media_qbittorrent_configurations').fetchone()[0] >= MAX_CONFIGURATIONS:
                raise ApiError('media_qbittorrent_configuration_limit_reached', 409)
            ...
            row = {
                ...
                'sequence': connection.execute(
                    'SELECT COALESCE(MAX(sequence),0)+1 FROM media_qbittorrent_configurations').fetchone()[0],
                ...
                'inspection_id': body.inspectionId, 'state': 'queued',
            ...
            connection.execute(
                'INSERT INTO media_qbittorrent_configurations('
    ...
    def _find(connection, identifier):
        row = connection.execute(
            'SELECT * FROM media_qbittorrent_configurations WHERE id=?',
            (identifier,)).fetchone()
        if row is None:
    ...
    def cancel(self, actor, identifier, body):
        _identifier(identifier)
        body = _body(body, CancelQbittorrentConfigurationRequest)
        with self.db.transaction() as connection:
            self._assert_admin(connection, actor)
            row = self._find(connection, identifier)
            payload = self._decode(row)
    ...
    def _dispatch_lock(self):
        ...
        try:
            descriptor = os.open(
                self.settings.data_dir / '.qbittorrent-configurations.lock',
        ...
        with self._dispatch_lock() as acquired:
            ...
            with self.db.transaction() as connection:
                ...
                if row['state'] == 'running' and not row['cancel_requested']:
                    return self._transition(
                        ...
                        error='qbittorrent_config_interrupted')
                ...
                if not gate.permitted:
                    ...
                    return self._transition(
                        connection, row, payload, state='needs_attention',
                        error=('qbittorrent_config_cancellation_uncertain'
                               if gate.code == 'cancelled'
                               else 'qbittorrent_config_' + gate.code),
                        cancel=gate.code == 'cancelled')
                if self.backend is None:
                    return self._transition(
                        connection, row, payload, state='failed',
                        error='qbittorrent_config_worker_unavailable')
            ...
            try:
                ...
                with self.db.transaction() as connection:
                    ...
                    state = ('needs_attention' if cancelled
                             ...
                             or failure.code == 'qbittorrent_config_authority_changed'
                             else 'failed')
                    error = ('qbittorrent_config_cancellation_uncertain'
            ...
            with self.db.transaction() as connection:
                ...
                if row['cancel_requested']:
                    return self._transition(
                        connection, row, payload, state='needs_attention',
                        error='qbittorrent_config_cancellation_uncertain', cancel=True)
                gate = self._gate_locked(connection, row, payload)
                if not gate.permitted:
                    return self._transition(
                        connection, row, payload, state='needs_attention',
                        error='qbittorrent_config_' + gate.code)
                return self._transition(
