...
| Private Arr API key | The configuration provider reads or generates its persisted `ApiKey`. Larenor has an exact renderer, closed volume writer, journal-reverified networks-off Docker stdin effect, retained-daemon runtime, and UID-protected private installation IPC for fresh owned Sonarr/Radarr `config.xml`. Persistent Core jobs and native authenticated acceptance remain closed. Do not expose a generic file editor or accept Client-supplied XML. [Renderer](../../../docs/arr-owned-config-implementation-2026-09-10.md), [writer](../../../docs/arr-config-helper-implementation-2026-09-10.md), [binding](../../../docs/arr-config-binding-implementation-2026-09-10.md), [effect](../../../docs/arr-config-effect-implementation-2026-09-10.md), [runtime](../../../docs/arr-config-runtime-implementation-2026-09-10.md), [IPC](../../../docs/arr-configuration-ipc-implementation-2026-09-10.md), [upstream provider](https://github.com/Sonarr/Sonarr/blob/v4.0.19.2979/src/NzbDrone.Core/Configuration/ConfigFileProvider.cs). |
...
`reconcile`, Jellyfin `bootstrap`, qBittorrent `configure_qbittorrent` and the
single ordered `install_configured_qbittorrent` operation.
...
the raw operation. The administrator-facing
`/admin/media/qbittorrent-configurations` resource instead persists a bounded,
AES-GCM authenticated job whose private values are generated by Core. It exposes
only fixed lifecycle/error state, dispatches over this socket automatically and
never retries a record left `running`. Its success path first installs the owned
config and then derives qBittorrent create/start from the same current journal
proof, deadline, retained daemon and native thread. The binding accepts only the
owned `/config` and shared writable `/data` volumes and publishes no ports.
Historical configuration-only receipts stay readable and expose no container
