...
| Private Arr API key | The configuration provider reads or generates its persisted `ApiKey`. Larenor has an exact-byte renderer/verifier, a closed stdin-only no-overwrite volume writer, and a pure current-journal binding for fresh owned Sonarr/Radarr `config.xml` files. Journal-bound Docker dispatch and native authenticated acceptance remain closed. Do not expose a generic file editor or accept Client-supplied XML. [Renderer evidence](../../../docs/arr-owned-config-implementation-2026-09-10.md), [writer evidence](../../../docs/arr-config-helper-implementation-2026-09-10.md), [binding evidence](../../../docs/arr-config-binding-implementation-2026-09-10.md), [Sonarr configuration provider](https://github.com/Sonarr/Sonarr/blob/v4.0.19.2979/src/NzbDrone.Core/Configuration/ConfigFileProvider.cs). |
