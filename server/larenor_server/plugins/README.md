...
| Private Arr API key | The configuration provider reads or generates its persisted `ApiKey`. Larenor now has a pure exact-byte renderer/verifier for fresh owned Sonarr/Radarr `config.xml` files, including fixed private ports and disabled analytics/upstream auto-update; filesystem effect and native authenticated acceptance remain closed. Do not expose a generic file editor or accept Client-supplied XML. [Implementation evidence](../../../docs/arr-owned-config-implementation-2026-09-10.md), [Sonarr configuration provider](https://github.com/Sonarr/Sonarr/blob/v4.0.19.2979/src/NzbDrone.Core/Configuration/ConfigFileProvider.cs). |
