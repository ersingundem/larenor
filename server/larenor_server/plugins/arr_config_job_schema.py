"""Additive authenticated storage for durable Arr config jobs."""

from ..errors import StartupError


TABLE = 'media_arr_configurations'
_COLUMNS = (
    ...
    ('inspection_id', 'TEXT', 1, None, 0), ('service_id', 'TEXT', 1, None, 0),
    ('state', 'TEXT', 1, None, 0),
...
def _verify(connection):
    ...
    if columns != _COLUMNS:
        raise StartupError('media_arr_configurations_schema_unsupported')
    ...
    for index in connection.execute(f'PRAGMA index_list({TABLE})'):
        ...
        if index['unique'] and not index['partial']:
            unique.add(fields)
        if index['name'] == 'media_arr_configurations_dispatch':
            dispatch = fields == ('state', 'sequence') and not index['unique'] and not index['partial']
    if unique != {('id',), ('sequence',), ('actor_id', 'request_id'),
                  ('preparation_id', 'service_id')} or not dispatch:
        raise StartupError('media_arr_configurations_schema_unsupported')
...
def migrate_arr_configurations(connection):
    marker = connection.execute(
        "SELECT value FROM metadata WHERE key='media_arr_configurations_schema'",
    ).fetchone()
    tables = {row[0] for row in connection.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'media_arr_configuration%'")}
    if marker is None:
        if tables:
            raise StartupError('media_arr_configurations_schema_unsupported')
        connection.execute(f'''CREATE TABLE {TABLE} (
            ...
            preparation_id TEXT NOT NULL REFERENCES media_preparations(id),
            inspection_id TEXT NOT NULL REFERENCES media_inspections(id),
            service_id TEXT NOT NULL CHECK(service_id IN ('sonarr','radarr')),
            ...
            UNIQUE(preparation_id,service_id)
        )''')
        connection.execute(
            f'CREATE INDEX media_arr_configurations_dispatch ON {TABLE}(state,sequence)')
        connection.execute(
            "INSERT INTO metadata(key,value) VALUES('media_arr_configurations_schema','1')")
