"""Journal-bound qBittorrent config install through one disposable helper.
...
def _require(value, code='qbittorrent_config_effect_untrusted', *, uncertain=False):
    if not value:
        raise QbittorrentConfigEffectError(code, uncertain_effect=uncertain)
...
class QbittorrentConfigHelperResult:
    state: str
    ...
    def __repr__(self):
        return 'QbittorrentConfigHelperResult(<private>)'


@dataclass(frozen=True, repr=False)
class QbittorrentConfigInstallReceipt:
    resource_id: str
    operation_id: str
    journal_id: str
    revision: int
    ...
    def __post_init__(self):
        _require(all(type(item) is str and _IDENTIFIER.fullmatch(item) is not None
                 ...
                 and type(self.configuration_digest) is str
                 and _DIGEST.fullmatch(self.configuration_digest) is not None
                 and self.state in _STATES,
                 'qbittorrent_config_effect_result_failed')

    def __repr__(self):
        return 'QbittorrentConfigInstallReceipt(<private>)'


def _helper_result(stdout, stderr, digest):
    _require(stderr == b'' and type(stdout) is bytes and len(stdout) <= 4096,
             'qbittorrent_config_effect_result_failed', uncertain=True)
    for state in _STATES:
        ...
        if hmac.compare_digest(stdout, expected):
            return QbittorrentConfigHelperResult(state, digest)
    raise QbittorrentConfigEffectError(
        'qbittorrent_config_effect_result_failed', uncertain_effect=True)


class UnixQbittorrentConfigEngine:
    ...
    def __init__(self, endpoint, *, transport_factory=None, stdin_factory=None,
        ...
        try:
            ...
            self._name_factory = uuid.uuid4 if name_factory is None else name_factory
                ...
                EngineStdinError, QbittorrentConfigEffectError):
            raise QbittorrentConfigEffectError(
                'qbittorrent_config_effect_configuration_invalid') from None
    ...
    def _body(binding, image_id):
        ...
        return _canonical({
            ...
            'Cmd': ['install_qbittorrent_config'],
            'AttachStdin': True,
            'AttachStdout': True,
            'AttachStderr': True,
            'OpenStdin': True,
    ...
    def _gate(before_dispatch):
        try:
            ...
            raise QbittorrentConfigEffectError(
                'qbittorrent_config_effect_dispatch_denied') from None
        _require(permitted, 'qbittorrent_config_effect_dispatch_denied')
    ...
    def install(self, binding, helper_image_id, platform, *, cancelled,
        ...
        _require(type(helper_image_id) is str
                 ...
                 'qbittorrent_config_effect_untrusted')
        _require(not cancelled.is_set(), 'qbittorrent_config_effect_cancelled')
        ...
        try:
            ...
            if identity is not None:
                try:
                    ...
                    failure = 'qbittorrent_config_effect_cleanup_failed'
                    uncertain = True
...
class QbittorrentConfigInstaller:
    ...
    def __init__(self, endpoint, helper_image_id, platform, *,
        ...
        try:
            ...
            self._engine = (UnixQbittorrentConfigEngine(
