...
_CODES = frozenset(
    {
        "seerr_endpoint_untrusted",
        "seerr_endpoint_unavailable",
...
class SeerrEndpointProof:
    ...
    def __post_init__(self):
        if (
            ...
            or _ID.fullmatch(self.container_id) is None
            or type(self.network_id) is not str
            or _ID.fullmatch(self.network_id) is None
            or type(self.plan_hash) is not str
            or _ID.fullmatch(self.plan_hash) is None
...
def prove_seerr_endpoint(observed, binding, stack, expected_container_id):
    ...
    try:
        if (
            ...
            or _ID.fullmatch(expected_container_id) is None
        ...
        component = next(
            item for item in selected.components if item.serviceId == "seerr"
        )
        ...
        if (
            len(listeners) != 1
            or listeners[0].protocol != "tcp"
            or listeners[0].purpose != "web"
            or listeners[0].port != 5055
        ):
            raise ValueError()
        ...
        if type(networks) is not dict or set(networks) != {
...
def open_seerr_endpoint(
    observed, binding, stack, expected_container_id, *, timeout=5.0
):
