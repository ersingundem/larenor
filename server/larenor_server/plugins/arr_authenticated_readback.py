...
from ..services.transport import _Deadline, _remaining, _request_bytes, ProbeTransportError
...
_CODES = frozenset({
    'invalid_arr_authenticated_readback', 'arr_authentication_failed',
    'arr_readback_protocol', 'arr_readback_mismatch',
    'arr_authenticated_readback_unavailable', 'arr_authenticated_readback_timeout',
})
...
class ArrAuthenticatedReadback:
    def read(self, connection, *, service_id, api_key,
             limits=ArrAuthenticatedReadbackLimits()):
        if (service_id not in _EXPECTED or not is_arr_api_key(api_key)
        ...
        try:
            ...
            raise ArrAuthenticatedReadbackError('invalid_arr_authenticated_readback') from None
        deadline = time.monotonic() + limits.total_seconds
        scope = None
        try:
            scope = _Deadline(deadline); scope.attach(connection)
            connection.settimeout(_remaining(deadline))
            connection.sendall(_request_bytes(
                'GET', '/api/v3/system/status', service_id,
                {'Accept':'application/json', 'X-Api-Key':api_key}, None))
            status, raw, _ = _response(
                _StartupReader(connection, deadline), limits.max_response_bytes,
            ...
            if type(value) is not dict or type(value.get('appName')) is not str or type(value.get('version')) is not str:
                raise ArrAuthenticatedReadbackError('arr_readback_protocol')
            expected_name, expected_version = _EXPECTED[service_id]
            if value['appName'] != expected_name or value['version'] != expected_version:
            ...
            raise ArrAuthenticatedReadbackError('arr_authenticated_readback_timeout') from None
        except _ConnectionLost:
            raise ArrAuthenticatedReadbackError('arr_authenticated_readback_unavailable') from None
        except (ProbeTransportError, ValueError, TypeError, AttributeError, UnicodeError):
            raise ArrAuthenticatedReadbackError('arr_readback_protocol') from None
        except (OSError, RuntimeError):
            raise ArrAuthenticatedReadbackError(
                'arr_authenticated_readback_timeout' if time.monotonic() >= deadline
                else 'arr_authenticated_readback_unavailable') from None
        finally:
            if scope is not None: scope.finish()
