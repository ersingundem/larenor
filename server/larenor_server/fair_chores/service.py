...
class FairChoreStore:
    ...
    def _task(row: sqlite3.Row) -> ChoreTask:
        try:
            ...
            if not isinstance(order, list) or not order or not all(_identifier(item) for item in order):
                raise ValueError
            task = ChoreTask(
                id=row["id"], core_id=row["core_id"], home_id=row["home_id"],
                title=row["title"], revision=row["revision"],
                assignee_id=row["assignee_id"], members_revision=row["members_revision"],
                member_order=tuple(order), timezone_name=row["timezone_name"],
                interval_days=row["interval_days"], due_at=row["due_at"],
            )
            ZoneInfo(task.timezone_name)
            if task.assignee_id not in task.member_order:
                raise ValueError
            return task
        except (KeyError, TypeError, ValueError, json.JSONDecodeError, ZoneInfoNotFoundError):
            raise StartupError("fair_chore_storage_invalid") from None

    @staticmethod
    def _receipt_json(receipt: ChoreReceipt) -> str:
        return json.dumps(
            {"event_id": receipt.event_id, "action": receipt.action,
    ...
    def _append(
        ...
        command_id: str, action: str, actor_id: str, occurred_at: float,
    ) -> ChoreReceipt:
        if connection.execute("SELECT COUNT(*) FROM fair_chore_events").fetchone()[0] >= MAX_HISTORY:
        ...
        values = (
            sequence, receipt.event_id, task.id, command_id, action, actor_id,
            task.revision, occurred_at, raw, previous_hash,
        )
    ...
    def _load(
        self, connection: sqlite3.Connection, task_id: str, core_id: str, home_id: str,
    ) -> ChoreTask:
    ...
    def create(
        self, actor: Principal, *, core_id: str, home_id: str, title: str,
        members: HouseholdMembers, timezone_name: str, interval_days: int,
        due_at: float,
    ) -> ChoreTask:
        self._validate_scope(core_id, home_id)
        if actor.role != "admin":
        ...
        if (
            ...
            or type(interval_days) is not int or not 1 <= interval_days <= 365
        ...
        now = time.time()
        task = ChoreTask(
            id=uuid.uuid4().hex, core_id=core_id, home_id=home_id,
            title=title.strip(), revision=1, assignee_id=members.ids[0],
            members_revision=members.revision, member_order=members.ids,
            timezone_name=timezone_name, interval_days=interval_days,
            due_at=float(due_at),
        )
        with self.database.transaction() as connection:
            connection.execute(
                "INSERT INTO fair_chore_tasks VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (task.id, core_id, home_id, task.title, 1, task.assignee_id,
                 members.revision, json.dumps(members.ids, separators=(",", ":")),
                 timezone_name, interval_days, task.due_at, now, now),
            )
            self._append(
                connection, task=task, command_id=f"create:{task.id}",
                action="created", actor_id=actor.id, occurred_at=now,
            )
        return task
    ...
    def _next_assignee(task: ChoreTask, members: HouseholdMembers) -> str:
        ...
        candidates = task.member_order[current_index + 1:] + task.member_order[:current_index + 1]
        candidates += tuple(item for item in members.ids if item not in task.member_order)
        return next(item for item in candidates if item in members.ids)

    def complete(
        self, actor: Principal, task_id: str, *, core_id: str, home_id: str,
        expected_revision: int, command_id: str, completed_at: float,
        ...
        with self.database.transaction() as connection:
            ...
            connection.execute(
                ...
                (updated.revision, updated.assignee_id, updated.members_revision,
                 json.dumps(updated.member_order, separators=(",", ":")), updated.due_at,
                 completed_at, task.id, expected_revision),
            )
            if connection.execute("SELECT changes()").fetchone()[0] != 1:
                raise ApiError("revision_conflict", 409)
            return self._append(
                connection, task=updated, command_id=command_id, action="completed",
                actor_id=actor.id, occurred_at=float(completed_at),
            )

    def defer(
        self, actor: Principal, task_id: str, *, core_id: str, home_id: str,
        expected_revision: int, command_id: str, days: int,
    ) -> ChoreReceipt:
        if (
            type(expected_revision) is not int or expected_revision < 1
            or not _identifier(command_id) or type(days) is not int or not 1 <= days <= 30
        ):
            raise ApiError("invalid_request", 400)
        with self.database.transaction() as connection:
            task = self._load(connection, task_id, core_id, home_id)
            ...
            updated = ChoreTask(**{**asdict(task), "revision": task.revision + 1, "due_at": due_at})
            now = time.time()
            connection.execute(
            ...
            return self._append(
                connection, task=updated, command_id=command_id, action="deferred",
                actor_id=actor.id, occurred_at=now,
            )
    ...
    def _verified_history(
        ...
        rows = connection.execute(
            "SELECT * FROM fair_chore_events WHERE task_id=? ORDER BY sequence", (task_id,)
        ).fetchall()
        previous = ""
        events: list[ChoreEvent] = []
        for row in rows:
            values = (
                row["sequence"], row["event_id"], row["task_id"], row["command_id"],
                row["action"], row["actor_id"], row["revision"], row["occurred_at"],
                row["receipt_json"], row["previous_hash"],
            )
            ...
            if (
                ...
                raise StartupError("fair_chore_history_invalid")
            events.append(ChoreEvent(
                row["event_id"], row["action"], row["actor_id"],
                row["revision"], row["occurred_at"],
            ))
