...
from .models import (
    ComfortAuthority, ComfortCommandResult, ComfortDeviceReadback, ComfortPlan,
    ComfortPolicy, ComfortPreview, ComfortReceipt, ComfortWorkerCommand,
...
class ComfortCoordinator:
    ...
    def _live(self, rawAuthority, rawPlan):
        ...
        if not authority.active or authority.role != "admin" or not authority.canManageComfort:
            raise ApiError("forbidden", 403)
        if (
            plan.coreId, plan.homeId, plan.homeRevision, plan.actorAccountId,
    ...
    def _commands(plan, readbacks):
        ...
        for key, value in observed.items():
            if (value.coreId, value.homeId, value.roomId, value.device) != (
                plan.coreId, plan.homeId, key[0], desired[key][0]
            ):
                raise ApiError("revision_conflict", 409)
    ...
    def preview(self, rawAuthority, rawPlan, rawReadbacks, *, requestId, nowMs):
        ...
        try:
            readbacks = tuple(ComfortDeviceReadback.model_validate(item) for item in rawReadbacks)
        ...
        with self._lock:
            ...
            if old_id is not None:
                ...
                return ComfortPreview(
                    ...
                    expiresAtMs=old.expires_at, commandCount=len(commands),
                )
            ...
            stored = _StoredPreview(
                authority, plan, readbacks, requestId, token, nowMs + policy.previewTtlMs
            )
            self._audit.append(
                ...
                status="accepted", payloadHash=digest(plan.model_dump(mode="json")),
                atMs=nowMs,
            )
            ...
            return ComfortPreview(
                ...
                requestId=requestId, planId=plan.planId,
                expiresAtMs=stored.expires_at, commandCount=len(commands),
            )
    ...
    def confirm(self, rawAuthority, previewId, confirmToken, *, nowMs, worker):
        with self._lock:
            ...
            if old is not None:
                return old
            if type(nowMs) is not int or nowMs < plan.generatedAtMs or nowMs > stored.expires_at:
                raise ApiError("invalid_request")
            ...
            for (room_id, kind), (device, desired_state), before in commands:
                ...
                command = ComfortWorkerCommand(
                    ...
                    actorAccountId=authority.accountId, roomId=room_id,
                    targetKind=kind, device=device,
                    expectedStateRevision=before.stateRevision, desiredState=desired_state,
                )
                try:
                ...
                try:
                    ...
                    results.append(ComfortCommandResult(
                        schemaVersion=1, commandId=command_id, roomId=room_id,
                        targetKind=kind, status="failed", code="readback_mismatch",
                        readback=None,
                    ))
                    continue
                exact = (
                    value.commandId == command_id and value.roomId == room_id
                    and value.device == device and value.stateRevision > before.stateRevision
                    and value.state == desired_state and value.observedAtMs >= before.observedAtMs
                )
                results.append(ComfortCommandResult(
                    schemaVersion=1, commandId=command_id, roomId=room_id,
                    targetKind=kind, status="applied" if exact else "failed",
                    code="applied" if exact else "readback_mismatch", readback=value,
                ))
            ...
            self._audit.append(
                ...
                status=status, payloadHash=digest(receipt.model_dump(mode="json")),
                atMs=nowMs,
            )
