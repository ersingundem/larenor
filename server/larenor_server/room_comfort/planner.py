...
class ComfortPlanner:
    ...
    def _authority(self, raw):
        ...
        try:
            current = ComfortAuthority.model_validate(
    ...
    def plan(
        ...
        *,
        ...
        try:
            ...
            occupancy = [
                OccupancySnapshot.model_validate(item) for item in rawOccupancy
            ]
            overrides = [
                ManualComfortOverride.model_validate(item) for item in overrides
            ]
        ...
        if (
            len(climates) != len(climate)
            or len(occupancies) != len(occupancy)
            or set(climates) != set(rooms)
            or set(occupancies) != set(rooms)
        ):
            raise ApiError("revision_conflict", 409)
        ...
        for room_id in sorted(rooms):
            ...
            indoor_stale = (
                ...
                or inside.smokeObservedAtMs > nowMs
                or nowMs - inside.smokeObservedAtMs > policy.indoorMaxAgeMs
            )
            occupancy_stale = (
            ...
            if indoor_stale or weather_stale:
                ...
                if (
                    inside.temperature.value
                    < policy.targetTemperatureMilliC - policy.temperatureToleranceMilliC
                ):
                    hvac = "heat"
        ...
        revisions = ComfortInputRevisions(
            ...
            roomSnapshotRevisions={
                key: climates[key].snapshotRevision for key in sorted(climates)
            },
            occupancyRevisions={
                key: occupancies[key].occupancyRevision for key in sorted(occupancies)
            },
            occupancySourceIds={
                key: occupancies[key].sourceId for key in sorted(occupancies)
            },
        ...
        identity = dict(
            ...
            occupancy=[
                occupancies[key].model_dump(mode="json") for key in sorted(occupancies)
            ],
            overrides=[
                active_overrides[key].model_dump(mode="json")
                for key in sorted(active_overrides)
            ],
        ...
        return ComfortPlan(
            ...
            occupancyAdvisory=occupancy_advisory,
            items=items,
        )
