...
- Ana PNG: `assets/icon/app_icon.png` (uygulama içi ve eski Android sürümleri).
...
- Üretim: `dart run flutter_launcher_icons`. Android'in yerel vektör katmanları ve
  `mipmap-anydpi-v26/ic_launcher.xml` korunur; Android PNG boyutları güncellenir.
...
Style/medium: premium contemporary smart-home app icon with Apple Home-inspired restraint, precise vector-like geometry, softly rounded joins, very subtle dimensional enamel finish and restrained light, not photorealistic, not a mockup. Strong recognizable silhouette at 32px.
...
Composition: one centered mark, emblem occupies about 62% of the image width and height, optically centered with even safe margin; all content fits inside the central circle-safe area. Background full bleed with NO rounded outer corners: Android launchers will apply the final adaptive or legacy mask.
