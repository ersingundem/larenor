...
| Sonarr/Radarr/Lidarr/Readarr | Direct medya bağlantısı | 0298c5a: kaynak sahipliği, çok alanlı kayıt markerı, eski form/HTTP callback reddi ve açık tam kayıt/clear kurtarması;192 odaklı/547 ilgili test ve inceleme temiz. a2658ec Android98 içinde3422Flutter/11E2E geçti; son storage-confirmation ek denetimi ayrı yerel dalda. |
| Jellyseerr, Bazarr, Prowlarr | Direct medya bağlantısı | 37bd4c8:151 yeni odaklı/241 ilgili test ve bağımsız inceleme geçti; tam PIN boş recovery ve eski callback reddi. Yerel birleşime alındı, yeniCI açık. Belirsiz yazı sonrası mevcut reader ayrıca denetleniyor. |
| qBittorrent | Direct medya bağlantısı | 13d022a:121 odaklı/531 ilgili test, %94,1 ilgili kapsam ve bağımsız inceleme geçti. Cookie/version kontrolü, sahipli iptal, belirsiz yazıda reader retirement, açık recovery ve 2× yazı kabulü. Yerel birleşimde, yeniCI açık. |
| Jellyfin | Direct medya bağlantısı | 3b5c51b:98 yeni/297 ilgili test ve bağımsız inceleme geçti; credential tuple ile cihaz ID ayrı, UDP bind/stop yarışları, gerçek nativefocus ve providerreload, belirsiz token kaydı recovery dahil. Son belge/birleşim/CI açık. |
| Proxmox | Direct altyapı bağlantısı | Ayrı pilotta altı alanlı tuple, açık TLS tercihi, gerçek PIN kurtarma ve geç yanıt sınırları uygulanıyor. Provider reload eski callback açığı gerçek RED ile bulundu; son test/inceleme/CI açık. |
| Keenetic | Direct ağ bağlantısı | Sıradaki pilot; provider kurulumu login başlatabildiği için yalnız ekranı gizlemek yeterli değil. Core provider/store ve geç yanıt sınırları açık. |
...
| DoorStationStore / `DoorStation.storageKey` | Direct ev diafonu | HA adresi ve kamera/çağrı/kapı entity referansları içerir. Başka eve kopyalanmaz; cc3db2 provider/store ve eski kapı eylemi/ayar callback reddini ekledi; 542 ilgili test/inceleme temiz, 808938e birleşik Client 3.115 test/analiz geçti, yeni CI açık. |
| MovieNightStore / `MovieNightPreset.storageKey` | Direct ev rutini | HA adresi ve başlangıç/bitiş sahne/script bağları içerir. Oda adı kopyası bu davranışları taşımaz; cc3db2 provider/store ve geç film eylemi reddini ekledi; statik depolama hatası ve mevcut Direct davranış testleri geçti, 808938e birleşik Client 3.115 test/analiz geçti, yeni CI açık. |
| WellbeingStore ve WellbeingDisclosureStore | Kişisel/cihaz özel veri + Direct HA bağları | Ortak ev kaydı olarak paylaşılmaz. Kaynak erişimi reddedildiğinde gizlilik filtresi boş/izinli hale gelemez; kişisel erişim ve PIN politikası korunur. 4eac0f6 ayrı dalında 253 ilgili test geçti; bağımsız inceleme temiz, yerel birleşimde yeniCI açık. Kişisel ayar okuması kaynak bağımsız, ölçüm okumaları ayrı PIN/özel görünüm iznine bağlıdır. |
| `ambient_photos_v1` dosya arşivi | Kullanıcının seçtiği cihaz özel fotoğrafları | Core ambient ekranı arşivi yüklemiyor; cihazın açıkça seçilmiş kişisel arşivi Core evine atanmaz. 4eac0f6 kaynak bağımsız erişim ve belirsiz ayar yazısı testleri ayrı dalda geçti; bağımsız inceleme temiz, yerel birleşimde yeniCI açık. Arşiv silinmez veya otomatik paylaşılmaz. |
