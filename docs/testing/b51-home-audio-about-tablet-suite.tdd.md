...
| Local audio | Source, playback, artwork, and power actions share the service/settings visual language. Enter starts the real bridge action. Every rendered callback captures the active interaction identity, epoch, and native bridge identity; idle/wake or provider replacement cannot revive play, seek, transport, picker, format, artwork, or navigation authority. A picker result from an expired epoch is discarded before native decoding. | EN/TR, 600/1200 px, 200% text matrix in `local_audio_ui_test.dart`; lifecycle/provider/artwork regressions in `local_audio_artwork_ui_test.dart` |
...
- Final focused command covering all three criteria passed **75/75** tests:
...
- The same focused suite with `--coverage` recorded **583/615 lines
  (94.8%)** across the three owned screens: About 67/67 (100%), local audio
  316/331 (95.5%), and room sync 200/217 (92.2%). Focused `flutter analyze` completed
