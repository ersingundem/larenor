...
| Local audio | Source, playback, artwork, and power actions share the service/settings visual language. Enter starts the real bridge action. Format choice exposes a selected semantic state and a visible checkmark, so it never relies on color alone. Every rendered callback captures the active interaction identity, epoch, and native bridge identity; idle/wake or provider replacement cannot revive play, seek, transport, picker, format, artwork, or navigation authority. Controls also render disabled while interaction authority is inactive. A picker result from an expired epoch is discarded before native decoding. | EN/TR, 600/1200 px, 200% text matrix plus explicit non-color format-selection semantics in `local_audio_ui_test.dart`; lifecycle/provider/artwork regressions in `local_audio_artwork_ui_test.dart` |
...
- Final focused command covering all three criteria passed **77/77** tests:
