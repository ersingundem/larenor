...
2. A route-owned Client session obtains one bounded Zigbee preview and requires
   explicit confirmation. Preview and result state survive Core restarts in an
   authenticated encrypted store; a confirmed result cannot replay the device
   command. Success appears only after the exact authenticated readback of
   device, provider, route, version and digest. Thread OTA, unsafe
