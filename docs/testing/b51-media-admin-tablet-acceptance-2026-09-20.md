...
| Prowlarr providers | Refresh and per-indexer enable/disable writes are bound to the exact list snapshot, client, route, and interaction session. Loading/error transitions hide retained toggles. Writes use per-indexer operation leases, remain single-flight, and failure copy is secret-free, localized, and announced through a TalkBack live region. Protocol and priority copy is localized in English and Turkish. |
...
The focused Flutter run passes **22 tests**. The accepted counters remain
