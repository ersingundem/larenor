...
monitored OctoPrint printers. It records reviewed pause or cancel intents; it
...
| Criterion | Evidence |
| --- | --- |
| Printer, job, material and safety snapshots use independent exact revisions, remain bound to the authenticated OctoPrint service revision and survive restart without exposing its API key | `test_printer_job_material_and_safety_revisions_are_exact_and_secret_free` |
...
Real OctoPrint observation, camera/thermal hardware, printer connectivity and
physical pause/cancel readback remain manual integration acceptance. F59 stays
pending until the isolated provider worker, Android tablet surface, end-to-end
journey and physical device gates are complete.
