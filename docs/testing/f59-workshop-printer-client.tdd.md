...
2. Pause/cancel requires a current exact Core/home/account/session/route and
   printer/service/job/material/safety
   revision, then a separate preview and explicit confirmation. Confirmation
   receipts remain `notDispatched` and must be read back from retained history;
   lost acknowledgement or stale authority is never retried automatically.
...
- Focused workshop, service-settings and settings-shell suite: 27 passed.
...
The authenticated Settings shell route is covered in software. A physical
600/1280-class Android tablet acceptance run and disposable OctoPrint/Moonraker
fixture must still prove delivery and device readback before the Core worker may
report anything beyond `notDispatched`. No physical printer, Huawei tablet,
Samsung DeX or production provider acceptance is claimed by this slice.
