...
class SyntheticCoreResourceAdmin {
  ...
  SyntheticCoreResourceAdmin.beforeUpdate() {
    _records.add(
      _clone(_contract['beforeUpdate']['record'] as Map<String, dynamic>),
    );
    _sequence = 1;
  }
  ...
  Future<(int, Map<String, dynamic>?)> handle(
    HttpRequest request,
    String coreId,
    String homeId,
    String userId,
  ) async {
    if (coreId != _contract['context']['coreId'] ||
    ...
    final write =
        {'POST', 'PATCH', 'DELETE'}.contains(request.method) && writeTarget;
    ...
    if (id != null && !RegExp(r'^[0-9a-f]{32}$').hasMatch(id)) {
      return _error(404, 'not_found');
    }
    if (write && (request.method == 'POST') != (id == null)) {
      return _error(404, 'not_found');
    }
    ...
    if (reply.$1 < 300) {
      ...
      if (gate != null) {
        try {
          ...
          if (identical(replyGate, gate)) replyGate = null;
        }
      }
    }
    return reply;
  ...
  _Reply _read(
    ...
    if (id != null) {
      ...
      reads++;
      return (200, {'record': _clone(entry)});
    }
    if (query.keys.any(
    ...
    final expected = query['expectedSnapshot']?.single;
    if (limit == null ||
        !RegExp(r'^[1-9][0-9]{0,2}$').hasMatch(rawLimit) ||
        limit > 100 ||
        (after != null &&
            (!RegExp(r'^[0-9a-f]{32}$').hasMatch(after) || expected == null)) ||
        (expected != null && !RegExp(r'^[0-9a-f]{64}$').hasMatch(expected))) {
      return _error(400, 'invalid_request');
    }
    ...
    final snapshot = sha256
        .convert(
          utf8.encode(
            jsonEncode(['admin-fixture-v1', coreId, homeId, userId, sorted]),
          ),
        )
        .toString();
    ...
    if (after != null && !sorted.any((r) => r['ref']['id'] == after)) {
      return _error(404, 'not_found');
    }
    final rest = sorted
        .where(
          (r) =>
              after == null || (r['ref']['id'] as String).compareTo(after) > 0,
        )
        .toList();
    final page = rest.take(limit).map(_clone).toList();
    reads++;
    return (
      200,
      {
  ...
  static int? _revision(Object? value) =>
      value is int && value >= 1 && value <= 9223372036854775807 ? value : null;
  static int? _queryRevision(String? value) =>
      value != null && RegExp(r'^[1-9][0-9]{0,18}$').hasMatch(value)
      ? _revision(int.tryParse(value))
      : null;
  _Reply _write(
    String method,
    String? id,
    ...
    if (method != 'DELETE') {
      final keys = method == 'POST'
          ? {'kind', 'label', 'order'}
          : {'expectedRevision', 'expectedAclRevision', 'label', 'order'};
      if (body!.length != keys.length ||
          !keys.every(body.containsKey) ||
          body['label'] is! String ||
          body['order'] is! int) {
        return _error(400, 'invalid_request');
      }
    }
    ...
    if (method == 'POST') {
      if (!{'room', 'resource'}.contains(body!['kind'])) {
        return _error(400, 'invalid_request');
      }
      if (_records.length >= 32) return _error(409, 'resource_limit');
      _sequence++;
      final identity = _sequence <= 2
          ? '$_sequence' * 32
          : _sequence.toRadixString(16).padLeft(32, '0');
      final record = <String, dynamic>{
        'ref': {
          ..._contract['context'] as Map,
          'id': identity,
          'kind': body['kind'],
        },
        ...metadata!.toJson(),
      ...
      _records.add(record);
      return (201, {'record': _clone(record)});
    }
    final revision = method == 'DELETE'
        ? _queryRevision(query['expectedRevision']?.single)
        : _revision(body!['expectedRevision']);
    final acl = method == 'DELETE'
        ? _queryRevision(query['expectedAclRevision']?.single)
    ...
    if (revision != old['revision'] || acl != old['aclRevision']) {
      return _error(409, 'revision_conflict');
    }
    if (method == 'DELETE') {
    ...
    final changed =
        old['label'] != metadata!.label || old['order'] != metadata.order;
    if (changed && revision == 9223372036854775807) {
      return _error(409, 'revision_conflict');
    }
    final record = {
      ...old,
    ...
    return (200, {'record': _clone(record)});
  }
  ...
  static Map<String, dynamic>? _body(String text) {
    ...
    while (index < text.length) {
      ...
      final end = index;
      while (index < text.length &&
          const [9, 10, 13, 32].contains(text.codeUnitAt(index))) {
        index++;
      }
