...
class SyntheticCoreResourceGrants {
  ...
  Map<String, Map<String, bool>> get grants =>
      Map<String, Map<String, bool>>.unmodifiable(
        _grants.map(
          (id, value) => MapEntry(id, Map<String, bool>.unmodifiable(value)),
        ),
      );
  List<Map<String, Object>> get users => [
    for (final id in ['2', '3', '9'])
      {
        'id': id * 32,
        'username': id == '9' ? 'fixture-core-user' : 'person_$id',
  ...
  Future<GrantsReply> handle(
    HttpRequest request,
    String coreId,
    String homeId,
    String userId,
    bool admin, {
    required int? Function() authStatus,
  }) async {
    final denied = authStatus();
    if (denied != null) {
      return error(
        denied,
        denied == 404
    ...
    if (coreId != scope['coreId'] || homeId != scope['homeId']) {
      return error(404, 'not_found');
    }
    final path = request.uri.path,
        readBase = '/api/v1/home-resources/$coreId/$homeId',
        aclBase =
            '/api/v1/admin/home-resources/$coreId/$homeId/${target['ref']['id']}/grants';
    final isUsers = path == '/api/v1/admin/users',
        isAcl = path == aclBase || path.startsWith('$aclBase/'),
        isRecord =
            path == readBase || path == '$readBase/${target['ref']['id']}';
    if (!isUsers && !isAcl && !isRecord) return error(404, 'not_found');
    if ((isUsers || isAcl) && !admin) return error(403, 'forbidden');
    if ((isUsers || isRecord) && request.method != 'GET' ||
        isAcl && !{'GET', 'PUT'}.contains(request.method)) {
      return error(403, 'forbidden');
    }
    if ((isUsers || isAcl) && request.uri.hasQuery) {
    ...
    if (body == null ||
        body.length != 2 ||
        !body.containsKey('expectedAclRevision') ||
        !body.containsKey('permissions')) {
      return error(400, 'invalid_request');
    }
    final revision = body['expectedAclRevision'],
    ...
    final desired = Map<String, bool>.from(permissions),
        old = _grants[id] ?? {'read': false, 'write': false};
    final changed =
        old['read'] != desired['read'] || old['write'] != desired['write'];
    if (changed && aclRevision == 9223372036854775807) {
  ...
  Map<String, dynamic> _grant(String id, Map<String, bool> permissions) => {
    'subjectId': id,
    'target': target['ref'],
    'aclRevision': aclRevision,
    'permissions': Map<String, bool>.from(permissions),
  };
  GrantsReply _readRecord(
    HttpRequest request,
    String base,
    String userId,
    bool admin,
    ...
    final permissions = admin
        ...
        : _grants[userId] ?? {'read': false, 'write': false};
    final record = {
      ...target,
      'aclRevision': aclRevision,
    ...
    final limit = query['limit']?.single ?? '25',
        after = query['after']?.single,
        expected = query['expectedSnapshot']?.single;
    if (!RegExp(r'^[1-9][0-9]{0,2}$').hasMatch(limit) ||
        int.parse(limit) > 100 ||
        after != null &&
            (after.length != 32 ||
                !RegExp(r'^[0-9a-f]+$').hasMatch(after) ||
                expected == null) ||
        expected != null &&
            (expected.length != 64 ||
                !RegExp(r'^[0-9a-f]+$').hasMatch(expected))) {
      return error(400, 'invalid_request');
    }
    final entries = permissions['read'] == true
        ? [record]
        : <Map<String, dynamic>>[];
    final snapshot = sha256
        .convert(
          utf8.encode(jsonEncode(['grants-fixture-v1', userId, entries])),
        )
        .toString();
    if (expected != null && expected != snapshot) {
      return error(409, 'revision_conflict');
    }
    if (after != null && (entries.isEmpty || after != record['ref']['id'])) {
      return error(404, 'not_found');
    }
    recordReads++;
    return GrantsReply(200, {
