...
void main() {
  ...
  final contract = jsonDecode(homePeopleContractFixture) as Map<String, dynamic>;
  ...
  Future<(int, Map<String, dynamic>?)> request(String path, {
    String method = 'GET', String? token = SyntheticCoreAccount.accessToken,
    Object? body,
  }) async {
    ...
    return (response.statusCode, text.isEmpty ? null : jsonDecode(text) as Map<String, dynamic>);
  }
  Future<void> login() async {
    final response = await request('/auth/login', method: 'POST', token: null, body: {
  ...
  test('embedded Android people contract is identical to real Server artifact', () {
    expect(contract, jsonDecode(File('contracts/home-people.v1.json').readAsStringSync()));
  });
  test('people fixture is opt-in and does not change ordinary account role or resource page', () async {
    ...
    final login = await request('/auth/login', method: 'POST', body: {
      'username': SyntheticCoreAccount.username,
      'password': SyntheticCoreAccount.password,
      'deviceName': 'Synthetic tablet',
    });
    ...
    expect((await request('/home-resources/${account.coreId}/${account.homeId}')).$2!['entries'], isEmpty);
  });
  test('member login and read use exact real typed Server response', () async {
    ...
    final page = HomePeoplePage.fromJson(response.$2, expectedContext: ServerContext.fromJson(contract['context']));
  ...
  test('read token is not valid before fixture login and wrong token stays denied', () async {
  ...
  test('bounded cursor pages match actual Server responses exactly', () async {
    ...
    final second = await request('${path()}?limit=1&after=${first['nextAfter']}&expectedSnapshot=${first['snapshot']}');
  ...
  test('explicit empty view retires old paging snapshot and returns real empty member response', () async {
    ...
    expect((await request('${path()}?limit=1&after=${first['nextAfter']}&expectedSnapshot=${first['snapshot']}')).$1, 409);
  ...
  for (final query in [
    '?limit=0', '?limit=101', '?limit=01', '?limit=-1', '?limit=1&limit=2',
    '?after=${'1' * 32}', '?expectedSnapshot=${'g' * 64}', '?token=synthetic',
    '?expectedSnapshot=${'a' * 64}&expectedSnapshot=${'b' * 64}',
    '?limit=1%0A', '?after=${'1' * 32}%0A&expectedSnapshot=${'a' * 64}',
  ]) {
  ...
  test('nonexistent cursor and stale snapshot do not count as reads', () async {
    ...
    expect((await request('${path()}?after=${'9' * 32}&expectedSnapshot=$token')).$1, 404);
    expect((await request('${path()}?expectedSnapshot=${'a' * 64}')).$1, 409);
    expect(account.peopleReads, 0);
  });
  test('no mutation, item, grant or auth bypass endpoint exists in read fixture', () async {
    await login();
    for (final method in ['POST', 'PUT', 'PATCH', 'DELETE']) {
      expect((await request(path(), method: method, body: {'label': 'do not create'})).$1, 403);
    }
    expect((await request('/admin${path()}', method: 'POST', body: {'label': 'do not create'})).$1, 403);
  ...
  test('member fixture retains empty existing resource endpoint without HA activity', () async {
    await login();
    final reply = await request('/home-resources/${account.coreId}/${account.homeId}');
    expect(reply.$1, 200);
    expect(reply.$2!['entries'], isEmpty);
    expect(account.peopleReads, 0);
  });
  test('duplicate authorization values cannot grant fixture read access', () async {
    await login();
    final call = await client.getUrl(Uri.parse('${host.baseUrl}/api/v1${path()}'));
    call.headers.add('authorization', 'Bearer ${SyntheticCoreAccount.accessToken}');
    call.headers.add('authorization', 'Bearer ${SyntheticCoreAccount.accessToken}');
