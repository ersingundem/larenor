...
void main() {
  for (final change in [
    ...
    test('streamed ACL body rechecks $change before any effect', () async {
      ...
      final core = _ChangingCore(grants);
      host.coreAccount = core;
      final client = FixtureNetwork(host.port).createHttpClient(null);
      try {
        final login = await client.postUrl(
          Uri.parse('${host.baseUrl}/api/v1/auth/login'),
        );
        ...
        grants.bodyStarted = started;
        final req = await client.openUrl(
          'PUT',
          Uri.parse(
            '${host.baseUrl}/api/v1/admin/home-resources/${core.coreId}/${core.homeId}/${'1' * 32}/grants/${'3' * 32}',
          ),
        );
        req.headers.contentType = ContentType.json;
        req.headers.set('authorization', 'Bearer ${core.currentAccessToken}');
        req.write('{"expectedAclRevision":1,');
        await req.flush();
        await started.future.timeout(const Duration(seconds: 2));
        if (change == 'core') core.coreId = 'c' * 32;
        if (change == 'home') core.homeId = 'd' * 32;
        if (change == 'actor') core.actor = '3' * 32;
        if (change == 'role') core.role = 'member';
        if (change == 'logout') core.revokeGrantSession();
        if (change == 'password') core.mustChange = true;
        req.write('"permissions":{"read":true,"write":false}}');
        final res = await req.close();
        await res.drain<void>();
        expect(res.statusCode, switch (change) {
          'none' => 200,
          'core' || 'home' => 404,
          'actor' || 'logout' => 401,
          _ => 403,
        });
        expect(grants.putRequests, change == 'none' ? 1 : 0);
        expect(grants.grants.isEmpty, change != 'none');
