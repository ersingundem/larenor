...
void main(){
 late SyntheticHaServer host;late SyntheticCoreAccount core;late HttpClient client;
 ...
 test('bundled grants fixture equals actual Server HTTP contract',(){expect(contract,jsonDecode(File('contracts/home-resource-grants.v1.json').readAsStringSync()));});
 test('authenticated ACL mode returns named existing Core users',()async{
  final(status,raw)=await request('GET','/admin/users');expect(status,200);
  final users=(raw as Map)['users'] as List;expect(users.map((u)=>u['id']),containsAll(['2'*32,'3'*32,'9'*32]));
 ...
 test('real HTTP follows exact ACL contract including no-op, revoke and stale revision',()async{
  for(final name in ['empty','readOnly','afterReadOnly','readOnlyNoop','secondReadWrite','sorted','upgrade','revoke','revokeNoop','afterRevoke','stale','writeRequiresRead']){
   final step=contract[name] as Map;final(status,body)=await request(step['method'] as String,step['path'] as String,body:step['body']);
   expect(status,step['status'],reason:name);expect(body,step['response'],reason:name);
  }
  expect(host.requests,0);expect(host.acceptedActions,isEmpty);
 });
}
