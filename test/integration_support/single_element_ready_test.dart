...
void main() {
  testWidgets('mounted readiness survives empty, pending, absent and ready frames', (tester) async {
    var calls = 0;
  ...
  testWidgets('duplicate mounted targets are not ready and never evaluate the condition', (tester) async {
    await tester.pumpWidget(const Column(children: [_Target(), _Target()]));
    var calls = 0;
    expect(singleElementReady(find.byType(_Target), (_) {calls++; return true;}), isFalse);
    expect(calls, 0);
  });
  testWidgets('single mounted target condition failures still propagate', (tester) async {
    await tester.pumpWidget(const _Target());
    final error = StateError('synthetic-condition-failure');
    expect(() => singleElementReady(find.byType(_Target), (_) => throw error), throwsA(same(error)));
  });
}
