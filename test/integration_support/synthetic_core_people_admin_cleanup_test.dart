...
void main() {
  for (final rejected in [false, true]) {
    testWidgets(
      'actual people-admin harness cleanup preserves global zero guard: $rejected',
      (tester) async {
        late AppHarness app;
        await tester.runAsync(() async {
          ...
          try {
            ...
            expect(
              await send('POST', base, {'label': 'Fixture person', 'order': 0}),
              201,
            );
            final record = '$base/${core.firstId}',
                grant = '$record/grants/${core.subjectId}';
            expect(
              await send('PUT', grant, {
            ...
            expect(
              await send(
                'DELETE',
                '$record?expectedRevision=1&expectedAclRevision=3',
              ),
              204,
            );
            expect(core.records, isEmpty);
            expect(core.mutations, ['POST', 'PUT', 'PUT', 'DELETE']);
            if (rejected) {
              core.retireSession();
              expect(await send('GET', '/admin/users'), 401);
            }
            expect(core.rejectedRequests, rejected ? 1 : 0);
