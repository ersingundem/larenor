...
void main() {
  for (final locale in const [Locale('en'), Locale('tr')]) {
    for (final size in const [Size(600, 900), Size(1280, 900)]) {
      testWidgets(
        'visible QR permission surface ${locale.languageCode} $size at 2x',
        (tester) async {
          ...
          await tester.pump();
          expect(
            find.byKey(const ValueKey('web-panel-qr-visible-permission')),
            findsOneWidget,
          );
          expect(
            find.byKey(const ValueKey('web-panel-qr-camera-preview')),
            findsOneWidget,
          );
          expect(tester.takeException(), isNull);
          platform.session.valuesController.add(
            'https://fixture.invalid/item/42',
          );
  ...
  testWidgets(
    'background and stale authority close camera without publishing',
    (tester) async {
      final platform = _Platform();
      var current = true;
      var accepted = false;
