...
class Trust implements RdpTrustStore {
  ...
  bool checked = false;
  @override
  Future<void> checkProfile(RemoteProfile profile, {required bool Function() isCurrent}) async {
    if (!isCurrent()) throw const RdpFailure('retired');
    checked = true;
  }
  @override
  Future<RdpCertificatePin?> readPin(RemoteProfile profile, {required bool Function() isCurrent}) async {
    if (!isCurrent()) throw const RdpFailure('retired');
    return pin;
  }
  @override
  Future<void> trust(RemoteProfile profile, RdpCertificatePin value, {required bool Function() isCurrent}) async {
    if (!isCurrent()) throw const RdpFailure('retired');
    pin = value;
  }
}
...
class Channel implements RdpChannel {
  ...
  @override
  void close() { closes++; if (!doneValue.isCompleted) doneValue.complete(); }
  @override
...
class Engine implements RdpEngine {
  ...
  @override
  Future<RdpCapabilities> capabilities({required bool Function() isCurrent}) async =>
      RdpCapabilities.fromJson(fixture()[available ? 'availableCapabilities' : 'unavailableCapabilities']);
  @override
  Future<RdpPeerSecurity> inspect(RemoteProfile profile, {required bool Function() isCurrent}) {
    inspections++;
    return delayed?.future ?? Future.value(RdpPeerSecurity(
      tls: true,
  ...
  @override
  Future<RdpChannel> open(RdpSessionRequest request, {required String? nlaPassword, required bool Function() isCurrent}) async {
    opens++;
...
RdpSessionController controller(Engine engine, Trust trust, bool Function() current) =>
    RdpSessionController(
      profile: profile,
      trust: trust,
      engineFactory: () => engine,
      isCurrent: current,
      display: const RdpDisplaySpec(width: 2560, height: 1600, dpi: 220, externalDisplay: true),
    );
...
void main() {
  test('unavailable engine is explicit and never opens a transport', () async {
    final engine = Engine(available: false), c = controller(engine, Trust(), () => true);
    await c.connect();
  ...
  test('first pin and NLA are separate explicit states', () async {
    final engine = Engine(), trust = Trust(), c = controller(engine, trust, () => true);
    final opening = c.connect(); await flush();
    expect(c.phase, RdpSessionPhase.certificate);
    expect(c.pendingCertificate, isNotNull);
    await c.trustCertificate(); await flush();
    expect(c.phase, RdpSessionPhase.nlaRequired);
    expect(engine.opens, 0);
    await c.authenticate('one-time-password'); await opening;
  ...
  test('changed certificate closes without retry or replay', () async {
    final trust = Trust()..pin = const RdpCertificatePin(
  ...
  test('focus or PIN retirement wipes input and closes exactly once', () async {
    ...
    final engine = Engine(requiresNla: false), c = controller(engine, trust, () => current);
    await c.connect();
    expect(c.phase, RdpSessionPhase.connected);
    c.pointer(const RdpPointerEvent(x: .5, y: .5, buttons: 0));
    c.key(const RdpKeyEvent(physicalKey: 42, down: true));
    current = false; c.synchronize();
  ...
  test('late inspection after sign-out cannot publish or open', () async {
    ...
    final opening = c.connect(); await flush();
    current = false; c.synchronize();
    engine.delayed!.complete(RdpPeerSecurity(
      tls: true,
      requiresNla: false,
      certificate: RdpCertificatePin.fromJson(fixture()['certificate']),
    ));
