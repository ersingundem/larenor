...
Matcher failure(String code) =>
    isA<RemoteProfilesFailure>().having((v) => v.code, 'code', code);

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  for (final pair in <String, String>{
    '192.168.1.4': '192.168.1.4',
    ' NAS.Example. ': 'nas.example',
    '[2001:db8::1]': '2001:db8::1',
    '::1': '::1',
    'xn--bcher-kva.example': 'xn--bcher-kva.example',
  }.entries) {
    test(
      'canonical address ${pair.key}',
      () => expect(normalizeRemoteHost(pair.key), pair.value),
    );
  }
  for (final bad in [
    'https://host',
    ...
    test(
      'reject malformed host $bad',
      () => expect(
        () => normalizeRemoteHost(bad),
        throwsA(failure('invalid_host')),
      ),
    );
  }
  ...
  test('closed record never accepts secret or launch command keys', () {
    ...
    for (final key in ['password', 'privateKey', 'command', 'url']) {
      expect(
        () => RemoteProfile.fromJson({...value, key: 'not-permitted'}),
  ...
  test('ports and label bounds are enforced without coercion', () {
    ...
    for (final port in [0, 65536, '22', true]) {
      expect(
        () => RemoteProfile.fromJson({...value, 'port': port}),
        throwsA(failure('invalid_record')),
      );
    }
    expect(() => profile(port: 0).toJson(), throwsA(failure('invalid_record')));
    expect(
      () => RemoteProfile.fromJson({...value, 'name': 'x' * 81}),
      throwsA(failure('invalid_record')),
    );
  });
  group('actual secure platform store', () {
    ...
    test(
      'Core-free create edit delete and reopen preserve unrelated records',
      () async {
        disk['unrelated'] = 'keep';
        final empty = await store.read(isCurrent: () => true);
        final saved = await store.replace(empty, [
          profile(),
        ], isCurrent: () => true);
        expect(saved.revision, 1);
        expect(saved.profiles.single.address, 'nas.example:22');
        final reopened = await RemoteProfilesStore().read(
          isCurrent: () => true,
        );
        final edited = await store.replace(reopened, [
          profile(host: '192.168.1.8'),
        ], isCurrent: () => true);
        ...
        expect(
          calls.every((c) => c.endsWith(RemoteProfilesStore.storageKey)),
          isTrue,
        );
      },
    );
    test(
      'unknown/corrupt/oversize data is never empty or overwritten',
      () async {
        for (final value in [
          'bad',
          'x' * 32769,
    ...
    test('stale snapshot cannot replace another editor save', () async {
      ...
      await expectLater(
        store.replace(a, [], isCurrent: () => true),
        throwsA(failure('conflict')),
      );
    ...
    test(
      'retirement during platform read prevents publication and next write',
      () async {
        var active = true;
        ...
        await expectLater(
          store.read(isCurrent: () => active),
          throwsA(failure('retired')),
        );
        expect(calls.length, 1);
      },
    );
    test('false and throwing owner deny before platform', () async {
      for (final owner in <bool Function()>[
        () => false,
        () => throw StateError('private'),
      ]) {
        await expectLater(
          store.read(isCurrent: owner),
          throwsA(failure('retired')),
        );
      }
      expect(calls, isEmpty);
    ...
    test('bounded profile count and duplicate IDs cannot write', () async {
      final empty = await store.read(isCurrent: () => true);
      await expectLater(
        store.replace(empty, List.filled(33, profile()), isCurrent: () => true),
        throwsA(failure('limit')),
      );
      await expectLater(
        store.replace(empty, [profile(), profile()], isCurrent: () => true),
        throwsA(failure('invalid_record')),
      );
      expect(calls.where((c) => c.startsWith('write')), isEmpty);
    });
    test(
      'queued replacement freezes caller list and decoded lists are immutable',
      () async {
        final before = await store.read(isCurrent: () => true);
        final release = Completer<void>();
        final block = ConfigurationWrites.run(() => release.future);
        final rows = [profile()];
        final pending = store.replace(before, rows, isCurrent: () => true);
        rows.clear();
        release.complete();
        await block;
        final saved = await pending;
        expect(saved.profiles.length, 1);
    ...
    test('readback third value never reports confirmed save', () async {
      ...
      await expectLater(
        store.replace(before, [profile()], isCurrent: () => true),
        throwsA(failure('write_unconfirmed')),
      );
      expect(calls.where((c) => c.startsWith('write')).length, 1);
    });
    for (final after in [false, true]) {
      test(
        'write failure ${after ? 'after' : 'before'} effect is honest and never retried',
        () async {
          final before = await store.read(isCurrent: () => true);
          intercept = (call) async {
            final a = Map<String, dynamic>.from(call.arguments as Map);
          ...
          intercept = null;
          expect(
            (await store.read(isCurrent: () => true)).profiles.length,
            after ? 1 : 0,
          );
        },
      );
