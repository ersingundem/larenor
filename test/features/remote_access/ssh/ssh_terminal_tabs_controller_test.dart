...
void main() {
  test('tabs are bounded and every connection requires an explicit action', () async {
    ...
    expect(tabs.tabs.where((tab) => tab.controller.phase.name == 'connected'), hasLength(1));
  });

  test('closing and lifecycle retirement clean every tab without replay', () async {
    final store = Security()..pin = hostPin;
    final engines = <Engine>[];
    final tabs = SshTerminalTabsController(
      profile: profile(),
