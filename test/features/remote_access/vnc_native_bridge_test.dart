...
void main() {
  ...
  test('method channel binds requests and zeroizes password payload', () async {
    ...
    final opened = await transport.open(
      binding,
      VncBridgeRequest.fixtureForTest(),
      password,
    );
    ...
    expect(payload.keys, unorderedEquals(['binding', 'request', 'password']));
  });

  test('unknown native diagnostics collapse without retry or secret text', () async {
    var calls = 0;
    ...
    await expectLater(
      ...
      throwsA(isA<VncBridgeException>().having(
        (error) => error.code,
        'code',
        'connectionFailed',
      )),
  ...
  test('late capability result after account or route change cancels before open', () async {
    ...
    final password = Uint8List.fromList('secret'.codeUnits);
    final start = session.connect(VncBridgeRequest.fixtureForTest(), password);
    await Future<void>.delayed(Duration.zero);
    current = false;
    transport.capabilityGate.complete(VncBridgeCapabilities.fromChannel(_capabilities()));
    await start;
  ...
  test('input backpressure and background retirement never queue or replay', () async {
    ...
    await expectLater(
      ...
      throwsA(isA<VncBridgeException>().having((e) => e.code, 'code', 'busy')),
    );
    ...
    await expectLater(
      ...
      throwsA(isA<VncBridgeException>().having((e) => e.code, 'code', 'retired')),
    );
    expect(transport.inputs, 1);
  });

  test('frame sequence requires one explicit ack and retires on gaps', () async {
    final transport = _FakeTransport()..completeCapabilities();
    final session = VncBridgeSession(
      transport: transport,
      binding: VncSessionBinding.fromChannel(_binding()),
...
class _FakeTransport implements VncNativeTransport {
  ...
  @override
  Future<VncOpenedSession> open(VncSessionBinding binding, VncBridgeRequest request, Uint8List password) async {
    opens++;
  ...
  @override
  Future<void> cancel(VncSessionBinding binding) async { cancels++; }
  @override
  Future<void> input(VncSessionBinding binding, int sequence, Map<String, Object?> event) async {
    inputs++;
    if (inputGate != null) await inputGate!.future;
  }
  @override
  Future<void> ackFrame(VncSessionBinding binding, int sequence) async { acks.add(sequence); }
}
