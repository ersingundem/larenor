...
void main() {
  ...
  test('bounded decoder rejects malformed frames and wipes owned bytes', () async {
    final raw = _frame(1);
  ...
  test('one frame stays backpressured until explicit presentation ack', () async {
    final sink = _Sink();
    ...
    await expectLater(controller.offer(_frame(2)), throwsA(surfaceFailure('busy')));
  ...
  test('input is single-flight, clipboard opt-in, and retirement is terminal', () async {
    final sink = _Sink();
    var foreground = true;
    final controller = VncFramebufferController(
      sink: sink,
    ...
    await expectLater(controller.sendClipboard('Merhaba'), throwsA(surfaceFailure('clipboardDisabled')));
    controller.setClipboardEnabled(true);
    sink.inputGate = Completer<void>();
    final first = controller.key(4, true);
    await expectLater(controller.key(4, false), throwsA(surfaceFailure('busy')));
    ...
    await expectLater(controller.key(5, true), throwsA(surfaceFailure('retired')));
    expect(sink.inputs, hasLength(2));
  });

  testWidgets('tablet and DeX surface is accessible and cleans on background', (tester) async {
    final sink = _Sink();
    final controller = VncFramebufferController(
      sink: sink,
      isCurrent: () => true,
    ...
    for (final width in [600.0, 1280.0]) {
      ...
      await controller.offer(_frame(1));
      await tester.pump();
      expect(find.bySemanticsLabel('Remote desktop'), findsOneWidget);
      expect(
        tester.getSize(find.byKey(const ValueKey('vnc-clipboard-toggle'))).height,
...
Matcher surfaceFailure(String code) => isA<VncSurfaceException>().having(
  (error) => error.code,
  'code',
  code,
);
...
class _Sink implements VncSurfaceSink {
  ...
  @override
  Future<void> acknowledge(int sequence) async { acks.add(sequence); }
  @override
