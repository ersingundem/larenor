...
final class FakeInventoryGateway implements InventoryGateway {
  ...
  int resolves = 0, histories = 0, grants = 0;
...
void main() {
  test('scan and manual entry resolve one current scoped read-only detail', () async {
    ...
    expect((gateway.resolves, gateway.histories, gateway.grants), (2, 2, 2));
  ...
  test('malformed foreign offline and late results fail closed without followups', () async {
    var current = true;
    ...
    current = false;
    controller.retire();
    delayed.complete(InventoryItem.fromResponse(itemResponse(), expected: context));
    await operation;
    expect(controller.entries, isEmpty);
    expect((gateway.histories, gateway.grants), (0, 0));
