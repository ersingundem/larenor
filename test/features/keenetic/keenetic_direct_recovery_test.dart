...
void main() {
  ...
  for (final action in ['refresh', 'navigate', 'signOut']) {
    testWidgets('connected root old $action callback expires but fresh action works', (tester) async {
      secure.values.addAll(keeneticRecord);
      final (c, _) = await routinesHome('direct');
      final interaction = AppInteractionController();
      addTearDown(interaction.dispose);
      ...
      await http.runWithClient(() async {
        ...
        VoidCallback callback() {
          if (action == 'navigate') {
            return tester.widgetList<CupertinoListTile>(find.byType(CupertinoListTile)).first.onTap!;
          }
          if (action == 'signOut') return tester.widget<CupertinoButton>(find.byKey(const ValueKey('service-account-action'))).onPressed!;
          return tester.widgetList<CupertinoButton>(find.byType(CupertinoButton)).firstWhere((b) => b.child is Icon && (b.child as Icon).icon == CupertinoIcons.refresh).onPressed!;
        }
        final old = callback();
        interaction.setActive(false);
        await settle(tester);
        ...
        if (action == 'navigate') expect(find.byType(KeeneticDevicesScreen), findsOneWidget);
        if (action == 'signOut') expect(secure.values.keys.where((k) => k.startsWith('keenetic_')), isEmpty);
        expect(tester.takeException(), isNull);
        await finish(tester, c);
  ...
  testWidgets('sign-out window loss after first deletion leaves quarantine for explicit recovery', (tester) async {
    secure.values.addAll(keeneticRecord);
    final (c, _) = await routinesHome('direct');
    final interaction = AppInteractionController();
    addTearDown(interaction.dispose);
    messenger.setMockMethodCallHandler(storageChannel, (call) async {
      final result = await secure.handle(call);
      if (call.method == 'delete' && (call.arguments as Map)['key'] == 'keenetic_base_url') interaction.setActive(false);
      return result;
    });
    await http.runWithClient(() async {
      await mount(tester, c, interaction, child: const KeeneticHomeScreen());
      ...
      expect(secure.values['keenetic_username'], keeneticRecord['keenetic_username']);
      expect(c.read(keeneticConnectionProvider).hasError, isTrue);
      interaction.setActive(true);
      await settle(tester);
  ...
  for (final interrupted in [false, true]) {
    testWidgets('late gateway prefill preserves ${interrupted ? 'retired' : 'edited'} draft', (tester) async {
      ...
      gateway.complete('192.0.2.1'); // Documentation-only address; native call mocked.
      await settle(tester);
      final url = tester.widget<CupertinoTextFormFieldRow>(find.byType(CupertinoTextFormFieldRow).first).controller!.text;
  ...
  testWidgets('actual IdleGate native focus clears setup and rejects its old callback', (tester) async {
    secure.values.addAll({...keeneticRecord, marker: '1'});
    final (c, _) = await routinesHome('direct');
    ...
    var requests = 0;
    await http.runWithClient(() async {
      await mount(tester, c, outer, child: const IdleGate(child: KeeneticHomeScreen()));
      await fill(tester);
      final old = tester.widget<CupertinoButton>(find.widgetWithText(CupertinoButton, 'Connect')).onPressed!;
      void focus(ViewFocusState state, {int? id}) => tester.binding.handleViewFocusChanged(ViewFocusEvent(viewId: id ?? tester.view.viewId, state: state, direction: ViewFocusDirection.undefined));
      focus(ViewFocusState.unfocused, id: tester.view.viewId + 1);
      await settle(tester);
      expect(tester.widget<CupertinoTextFormFieldRow>(find.byType(CupertinoTextFormFieldRow).first).controller!.text, 'https://new.invalid');
  ...
  for (final locale in ['en', 'tr']) {
    for (final width in [600.0, 1200.0]) {
      testWidgets('$locale ${width.toInt()} 2x real-font recovery is scrollable and keyboard-clearable', (tester) async {
        ...
        await mount(tester, c, interaction, size: Size(width, 650), scale: 2, locale: Locale(locale), child: const KeeneticHomeScreen());
        final l10n = AppLocalizations.of(tester.element(find.byType(KeeneticConnectScreen)));
        final clear = find.widgetWithText(CupertinoButton, l10n.keeneticRemoveConnection);
        await tester.scrollUntilVisible(clear, 150, scrollable: find.byType(Scrollable).first, maxScrolls: 12);
        await settle(tester);
        ...
        bool focused() {
          var found = false;
          FocusManager.instance.primaryFocus?.context?.visitAncestorElements((element) { if (identical(element, buttonElement)) found = true; return !found; });
          return found;
        }
        ...
        await tester.scrollUntilVisible(find.text(l10n.commonDone), -150, scrollable: find.byType(Scrollable).first, maxScrolls: 12);
  ...
  testWidgets('same GlobalKey form moved from retained Direct container to Core loses draft and callbacks', (tester) async {
  ...
  testWidgets('provider reload retires pending gateway response and keyboard submit uses current form', (tester) async {
    final gateway = Completer<String?>();
    messenger.setMockMethodCallHandler(networkChannel, (_) => gateway.future);
    final (c, _) = await routinesHome('direct');
    final interaction = AppInteractionController();
    ...
    await http.runWithClient(() async {
      ...
      await settle(tester);
      final fields = tester.widgetList<CupertinoTextFormFieldRow>(find.byType(CupertinoTextFormFieldRow)).toList();
      expect(fields.map((f) => f.controller!.text), everyElement(isEmpty));
      final inputs = tester.widgetList<CupertinoTextField>(find.byType(CupertinoTextField)).toList();
