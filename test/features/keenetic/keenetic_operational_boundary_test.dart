...
http.Response responseFor(http.Request request) {
  if (request.url.path.endsWith('/auth')) return http.Response('', 200, headers: {'set-cookie': 'session=synthetic; Path=/'});
  if (request.method == 'POST') return http.Response('', 200);
  if (request.url.path.endsWith('/show/interface')) {
    return http.Response(jsonEncode({'WifiMaster0/AccessPoint0': {'type': 'AccessPoint', 'description': 'Synthetic Wi-Fi', 'ssid': 'Synthetic SSID', 'state': 'up'}}), 200);
  }
  if (request.url.path.endsWith('/hotspot')) {
    return http.Response('{"host":[{"mac":"02:00:00:00:00:01","name":"Synthetic device","ip":"192.0.2.7","active":true}]}', 200);
  }
  if (request.url.path.endsWith('/ip/static')) return http.Response('[{"protocol":"tcp","port":8123,"to":"192.0.2.7","comment":"Synthetic rule"}]', 200);
  if (request.url.path.endsWith('/version')) return http.Response('{"model":"Synthetic router","release":"1.0"}', 200);
  return http.Response('{}', 200);
}

void focus(WidgetTester tester, ViewFocusState state) => tester.binding.handleViewFocusChanged(ViewFocusEvent(viewId: tester.view.viewId, state: state, direction: ViewFocusDirection.undefined));
...
void main() {
  ...
  setUp(() {
    ...
    secure = SecurePlatform()..values.clear()..values.addAll({...keeneticRecord, 'settings_pin': '2468'});
  ...
  for (final change in ['native_focus', 'idle', 'background', 'ticker', 'covered', 'account', 'source']) {
    testWidgets('captured Wi-Fi confirmation after $change cannot dispatch a command', (tester) async {
      ...
      await http.runWithClient(() async {
        await mount(tester, c, interaction, visible: ticker,
          child: change == 'native_focus' ? const IdleGate(child: KeeneticWifiScreen()) : const KeeneticWifiScreen());
        await tester.tap(find.byType(CupertinoSwitch));
        await settle(tester);
        final old = tester.widget<CupertinoDialogAction>(find.widgetWithText(CupertinoDialogAction, 'Turn Off')).onPressed!;
        if (change == 'native_focus') {
          ...
          final navigator = Navigator.of(tester.element(find.byType(KeeneticWifiScreen)));
          unawaited(navigator.push(CupertinoPageRoute<void>(builder: (_) => const CupertinoPageScaffold(child: Text('Covered route')))));
          await settle(tester);
          navigator.pop();
  ...
  testWidgets('PIN settings fresh Wi-Fi confirmation sends exactly the existing batch', (tester) async {
    final (c, _) = await routinesHome('direct');
    final interaction = AppInteractionController();
    addTearDown(interaction.dispose);
    final requests = <http.Request>[];
    await http.runWithClient(() async {
      await mount(tester, c, interaction, child: const IdleGate(child: SettingsGateScreen()));
      expect(requests, isEmpty);
      expect(find.byType(KeeneticWifiScreen), findsNothing);
      await tester.enterText(find.byType(CupertinoTextField), '2468');
      for (final label in ['Unlock', 'Integrations', 'Manage Integrations', 'Keenetic', 'Wi-Fi']) { await tap(tester, label); }
      ...
      expect(writes.single.url.toString(), 'https://router.invalid/prefix/rci/');
  ...
  for (final child in [const KeeneticDevicesScreen(), const KeeneticPortForwardingScreen()]) {
    testWidgets('${child.runtimeType} old refresh after idle sends no extra read', (tester) async {
      final (c, _) = await routinesHome('direct');
      final interaction = AppInteractionController();
      addTearDown(interaction.dispose);
      var requests = 0;
      await http.runWithClient(() async {
        ...
        final old = tester.widgetList<CupertinoButton>(find.byType(CupertinoButton)).firstWhere((b) => b.child is Icon && (b.child as Icon).icon == CupertinoIcons.refresh).onPressed!;
        interaction.setActive(false);
        await settle(tester);
        interaction.setActive(true);
        await settle(tester);
  ...
  testWidgets('device detail cannot keep former account metadata after replacement', (tester) async {
    final (c, _) = await routinesHome('direct');
    final interaction = AppInteractionController();
    addTearDown(interaction.dispose);
    await http.runWithClient(() async {
      await mount(tester, c, interaction, child: const KeeneticDevicesScreen());
