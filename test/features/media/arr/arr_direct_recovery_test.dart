...
void main() {
  ...
  for (final point in ['http', 'field']) {
    testWidgets(
      'pending recovery loses window permission during $point without later key effects',
      (tester) async {
            ...
            await tester.pumpWidget(
              UncontrolledProviderScope(
                ...
                child: CupertinoApp(
                  ...
                  home: const SonarrScreen(),
                ),
              ),
            );
            await settle(tester);
            final fields = find.byType(CupertinoTextFormFieldRow);
            await tester.enterText(fields.at(0), 'https://new.invalid');
            await tester.enterText(fields.at(1), 'synthetic-new-key');
            secure.calls.clear();
  ...
  for (final name in arrServices) {
    for (final action in ['view', 'clear', 'connect']) {
      testWidgets(
        '$name pending seed PIN route supports explicit $action without old prefill',
        (tester) async {
          var requests = 0;
          await http.runWithClient(
            () async {
            ...
            () => MockClient((request) async {
              ...
              final body = request.url.path.endsWith('/queue')
                  ? '{"records":[]}'
                  : request.url.path.endsWith('/calendar')
                  ? '[]'
                  : '{}';
