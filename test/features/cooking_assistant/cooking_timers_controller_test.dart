...
void main() {
  test('shared storage preserves exact revision across app restart', () async {
    ...
    await SharedPreferencesCookingTimerStore().write(
      value,
      expectedRevision: 0,
    );
  ...
  test(
    'multiple timers use monotonic time and restore exact deadlines',
    () async {
      final store = _Store();
      final clock = _Clock(
      ...
      clock.wall += const Duration(
        days: 2,
      ); // wall clock edits do not affect this run
      clock.monotonic += const Duration(minutes: 2);

      final oven = controller.timers.singleWhere(
        (timer) => timer.label == 'Oven',
      );
      final sauce = controller.timers.singleWhere(
        (timer) => timer.label == 'Sauce',
      );
      ...
      expect(
        restored.remaining(
          restored.timers.singleWhere((timer) => timer.label == 'Oven'),
        ),
        const Duration(minutes: 6),
      );
      expect(
        restored.remaining(
          restored.timers.singleWhere((timer) => timer.label == 'Sauce'),
        ),
        Duration.zero,
      );
    },
  ...
  test(
    'authority change rejects late restore, timer mutation and notification',
    () async {
      var current = true;
      final store = _Store();
      final notifications = _Notifications();
      ...
      await controller.resume();
      expect(
        await controller.acknowledge(controller.timers.single.id),
        isFalse,
      );
