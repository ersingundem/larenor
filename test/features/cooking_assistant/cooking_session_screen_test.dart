...
import 'package:larenor/features/cooking_assistant/presentation/cooking_session_screen.dart';
...
final class _Gateway implements CookingSessionGateway {
  ...
  Future<CookingSession> move({
    ...
    required int step,
      ...
      value = value.copyWith(revision: expectedRevision + 1, currentStep: step);
}

CookingSession _session() => CookingSession(
  id: 'session-1',
  accountId: 'account-a',
  recipeId: 'recipe-1',
  recipeRevision: 3,
...
void main() {
  ...
  testWidgets('keyboard and TalkBack expose previous and next', (tester) async {
    ...
    expect(
      find.bySemanticsLabel(CookingAssistantStrings.en.next),
      findsOneWidget,
    );
    await tester.sendKeyEvent(LogicalKeyboardKey.arrowRight);
    await tester.pump();
    expect(
      find.text(CookingAssistantStrings.en.stepProgress(2, 3)),
      findsOneWidget,
    );
    await tester.sendKeyEvent(LogicalKeyboardKey.arrowLeft);
    await tester.pump();
    expect(
      find.text(CookingAssistantStrings.en.stepProgress(1, 3)),
      findsOneWidget,
    );
    semantics.dispose();
  });
