...
final class _Notifications implements CookingTimerNotifications {
  @override
  Future<void> finished({required String idempotencyKey, required String label}) async {}
}

void main() {
  testWidgets('600/1200 rotation keeps one controller and accessible 48dp actions',
      (tester) async {
    final store = _Store();
    ...
    await controller.start(label: 'Oven', duration: const Duration(minutes: 5));
    for (final width in const [600.0, 1200.0, 600.0]) {
      ...
      await tester.pumpWidget(MaterialApp(
        home: MediaQuery(
          ...
          child: CookingTimersScreen(
            controller: controller, strings: CookingTimerStrings.en),
        ),
      ));
      await tester.pump();
      ...
      expect(find.bySemanticsLabel(CookingTimerStrings.en.acknowledge), findsOneWidget);
  ...
  testWidgets('Turkish copy remains visible at 2x', (tester) async {
    final controller = CookingTimersController(
      store: _Store(), notifications: _Notifications(), clock: _Clock(),
      authority: const CookingTimerAuthority(
        accountId: 'account-a', recipeSessionId: 'recipe-session', epoch: 1),
      isCurrent: () => true,
    );
    await controller.restore();
    await controller.start(label: 'Fırın', duration: const Duration(minutes: 5));
    ...
    await tester.pumpWidget(MaterialApp(
      home: MediaQuery(
        data: const MediaQueryData(
          size: Size(600, 900), textScaler: TextScaler.linear(2)),
        child: CookingTimersScreen(
          controller: controller, strings: CookingTimerStrings.tr),
      ),
    ));
