...
void main() {
  testWidgets('mounted Direct provider loads local switches and Core choices without reading credentials', (tester) async {
  ...
  testWidgets('explicit preview and confirm preserve local records and bind once', (tester) async {
    final h = TransferHarness(); await h.mount(tester); final c = h.controller!;
  ...
  testWidgets('lost confirm ACK recovers via explicit GET only', (tester) async {
    final h=TransferHarness(); await h.mount(tester); final c=h.controller!;
    await c.load(); await c.prepare(c.items.single,c.entities.single,isCurrent:()=>true);
    h.reply=(_) async=>response({'error':{'code':'server_error'}},503);
    await c.confirm(c.preview!,isCurrent:()=>true);
    expect(c.uncertain,isTrue); expect(c.receipt,isNull);
    final reads=h.platform.calls.length;
    h.reply=(r) async { expect(r.method,'GET'); return response({'receipt':transferReceiptJson()}); };
    await c.recover(isCurrent:()=>true);
    expect(c.uncertain,isFalse); expect(c.receipt,isNotNull);
    expect(h.platform.calls.length,reads);
    expect(h.requests.where((r)=>r.url.path.endsWith('/confirm')).length,1);
  });
  testWidgets('Core source cold transfer reads no local credentials or network', (tester) async {
    final h=TransferHarness(); await h.mount(tester,source:HomeSource.verifiedCore);
    await h.controller!.load(); expect(h.platform.calls,isEmpty); expect(h.requests,isEmpty);
  });
  testWidgets('PIN retired between platform URL and token stops token and outbound', (tester) async {
    final h=TransferHarness(); await h.mount(tester); final c=h.controller!;
    await c.load(); h.platform.afterRead=(key) async { if(key=='ha_base_url') { h.auth.pin=false; h.owner.synchronize(); } };
    await c.prepare(c.items.single,c.entities.single,isCurrent:()=>true);
    expect(h.platform.calls.map((e)=>e.$2), ['ha_connection_pending_v1','ha_base_url']);
    expect(h.requests.where((r)=>r.method=='POST'),isEmpty);
    expect(c.preview,isNull); expect(c.items,isEmpty);
  });
  testWidgets('retired same account late401 cannot logout Core account', (tester) async {
    final h=TransferHarness(); await h.mount(tester); final c=h.controller!;
