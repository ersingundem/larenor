...
VoidCallback held(WidgetTester tester, String id) => tester
...
void main() {
  for (final loss in ['focus', 'loading', 'error']) {
    testWidgets(
      'held Direct entry under window $loss opens no transfer route',
      (tester) async {
        final h = HaUiHarness();
  ...
  for (final pending in [false, true]) {
    for (final loss in [
      ...
      testWidgets(
        '$loss retires actual transfer ${pending ? 'late401' : 'held confirm'}',
        (tester) async {
          ...
          switch (loss) {
            ...
            case 'root':
              unawaited(
                Navigator.of(
                  tester.element(
                    find.byKey(const ValueKey('core-ha-transfer')),
                  ),
                  rootNavigator: true,
                ).push(
  ...
  testWidgets(
    'actual PIN loss between secure URL and token starts no preview POST',
    (tester) async {
      final h = TransferUiHarness();
      await h.open(tester);
