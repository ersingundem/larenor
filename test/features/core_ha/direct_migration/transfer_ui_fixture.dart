...
class TransferUiHarness extends HaUiHarness {
  ...
  Future<http.Response> handle(http.Request r) async {
    ...
    if (r.url.path.endsWith('/preview')) {
      ...
      final p = jsonDecode(
        jsonEncode(migration['preview']['response']['preview']),
      ) as Map<String, dynamic>;
      p['requestId'] = body['requestId'];
      (p['service'] as Map)
        ..['name'] = body['name']
        ..['baseUrl'] = body['baseUrl'];
      (p['binding'] as Map)['entityId'] = body['entityId'];
      publicPreview = p;
  ...
  Future<void> open(
    ...
    const layout = DashboardLayout(
      rooms: [
        DashboardRoom(
          id: 'local',
          name: 'Local room',
          entityIds: ['switch.synthetic', 'scene.evening', 'script.evening'],
        ),
      ],
      tiles: [
        TileConfig(
          ...
          width: 1,
          height: 1,
          url: 'https://panel.invalid/private',
        ),
      ],
    );
    await mount(
    ...
    router(tester).go('/settings/home-source');
    await flush(tester);
    await unlock(tester);
  ...
  Future<void> prepare(WidgetTester tester) async {
    ...
    await transferPress(
      tester,
      'core-ha-transfer-target-${f['resource']['ref']['id']}',
    );
...
Future<void> transferPress(WidgetTester tester, String key) async {
  ...
  await transferReveal(tester, target);
