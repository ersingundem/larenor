...
void main() {
  for (final pending in [false, true]) {
    testWidgets(
      'actual retained transfer container A→B→A retires ${pending ? 'late401' : 'held confirm'}',
      (tester) async {
        final h = TransferUiHarness();
        h.source.value = HomeSource.directLocal;
        SharedPreferences.setMockInitialValues({
          'dashboard_layout': jsonEncode(
            const DashboardLayout(
              rooms: [
                DashboardRoom(
                  id: 'room',
                  name: 'Room',
                  entityIds: ['switch.synthetic'],
                ),
              ],
            ).toJson(),
          ),
        ...
        FlutterSecureStorage.setMockInitialValues({
          'ha_base_url': 'http://fixture.invalid:8123',
          'ha_token': 'synthetic-transfer-token',
        });
        ...
        var callsB = 0;
        ProviderContainer make(bool b) => ProviderContainer(
          overrides: [
        ...
        final old = held(tester, 'core-ha-transfer-confirm'),
            element = screenKey.currentContext,
            state = tester.state(find.byType(CoreHaTransferRoute)),
        ...
        expect(tester.state(find.byType(CoreHaTransferRoute)), same(state));
        expect(
        ...
        expect(
          h.transferRequests
              .where((r) => r.url.path.endsWith('/confirm'))
              .length,
          pending ? 1 : 0,
        );
        expect(
          h.transferRequests
              .where((r) => r.url.path.endsWith('/preview'))
              .length,
          1,
        );
        expect(
          find.byKey(const ValueKey('core-ha-transfer-confirm')),
          findsNothing,
        );
