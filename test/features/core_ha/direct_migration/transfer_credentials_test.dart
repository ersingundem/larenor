...
Matcher failure(String code) => isA<DirectHomeAccessException>().having(
  (e) => e.code, 'safe code', code,
);
...
void main() {
  ...
  test('explicit transfer reads the actual HA pair without mutating it', () async {
    final value = await CredentialsStore().readForTransfer(isCurrent: () => true);
  ...
  for (final throws in [false, true]) {
    test('invalid initial action (throws=$throws) reads zero keys', () async {
      await expectLater(
        CredentialsStore().readForTransfer(isCurrent: () {
  ...
  for (var index = 0; index < keys.length; index++) {
    test('action loss after platform read ${keys[index]} stops continuation', () async {
      var active = true;
      ...
      expect(platform.calls, keys.take(index + 1).map((key) => ('read', key)));
    });
  }
  ...
  test('queued action retirement precedes first platform read', () async {
    ...
    var active = true;
    final operation = CredentialsStore().readForTransfer(isCurrent: () => active);
  ...
  test('Core borrowed store cannot read a transfer pair', () async {
    ...
    await expectLater(handle.read().readForTransfer(isCurrent: () => true),
      throwsA(failure('unavailable')));
    expect(platform.calls, isEmpty);
  });
  ...
  for (final missing in ['ha_base_url', 'ha_token']) {
    test('half credential tuple missing $missing is rejected', () async {
      ...
      await expectLater(CredentialsStore().readForTransfer(isCurrent: () => true),
        throwsA(failure('invalid_record')));
      expect(platform.calls.every((call) => call.$1 == 'read'), isTrue);
    });
  }
  for (final token in ['', 'with space', '\n', '\u00e9', 'x' * 2049]) {
    test('invalid transfer token case ${token.length}/${token.codeUnits.firstOrNull}', () async {
  ...
  test('platform failure is static and ordinary inactive reads are unchanged', () async {
