...
List<SemanticsNode> effectiveNodes(WidgetTester tester, Finder target) {
...
Future<void> openBinding(
  ...
  await reveal(
    tester,
    key('home-resource-edit-${h.f['resource']['ref']['id']}'),
  );
...
void main() {
  testWidgets(
    'actual Core home has no adapter request before selected member entry',
    (tester) async {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(key('core-ha-stale'), findsOneWidget);
      expect(key('core-ha-state-off'), findsNothing);
      h.snapshotStep = 'upstreamUnauthorized';
      await press(tester, 'core-ha-refresh');
      expect(key('core-ha-error'), findsOneWidget);
      expect(key('core-ha-state-off'), findsNothing);
  ...
  testWidgets(
    'missing switch state does not infer permission or display cached state',
    (tester) async {
      final h = HaUiHarness()..snapshotStep = 'hidden';
      await openSnapshot(tester, h);
      expect(key('core-ha-error'), findsOneWidget);
      expect(key('core-ha-state-off'), findsNothing);
      expect(
  ...
  testWidgets(
    'actual PIN binding preview cancel and confirm are explicit and one-use',
    (tester) async {
      final h = HaUiHarness();
      ...
      await press(
        tester,
        'core-ha-service-${h.f['preview']['body']['serviceId']}',
      );
      ...
      expect(
        h.adapterRequests.where((r) => r.url.path.endsWith('/binding-confirm')),
        isEmpty,
      );
  ...
  testWidgets(
    'uncertain binding acknowledgement exposes explicit GET without repeating POST',
    (tester) async {
      final h = HaUiHarness();
      await openBinding(tester, h);
      await press(
  ...
  for (final locale in ['en', 'tr']) {
    for (final width in [600.0, 1280.0]) {
      testWidgets(
        '$locale $width 2x binding native keyboard cancel and 48px semantics',
        (tester) async {
          ...
          try {
            ...
            final label = tester
                .widget<Text>(
                  find.descendant(of: native, matching: find.byType(Text)),
                )
                .data!;
            final nodes = effectiveNodes(tester, native)
                .where(
                  (n) =>
