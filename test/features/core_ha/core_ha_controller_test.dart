...
void main() {
  testWidgets('mounted provider selected resource GET then switch GET, no command', (tester) async {
  ...
  testWidgets('TTL subtracts network duration and stale clears projection without GET', (tester) async {
    final h = HaHarness();
    h.reply = (r) async {
      if (r.url.path.contains('/home-resources/')) return jsonResponse({'record': h.f['resource']});
    ...
    expect(h.list!.snapshot, isNull); expect(h.list!.stale, isTrue);
    expect(h.requests.length, 2);
  });
  testWidgets('response already expired during network is never displayed', (tester) async {
    final h = HaHarness();
    h.reply = (r) async {
      if (r.url.path.contains('/home-resources/')) return jsonResponse({'record': h.f['resource']});
  ...
  testWidgets('admin preview cancel has no confirmation, confirm consumes once', (tester) async {
    ...
    final c = h.list!;
    expect(c.binding, isNull); expect(c.canPreview, isTrue);
    ...
    expect(c.preview, isNull); await cancel;
    ...
    await c.confirm(second, isCurrent: () => true);
    expect(c.saved, isTrue); expect(c.binding, isNotNull);
    expect(h.requests.where((r) => r.url.path.endsWith('/binding-confirm')).length, 1);
  });
  testWidgets('expired preview cannot dispatch confirmation and requires fresh preview', (tester) async {
    final h = HaHarness(); await h.mount(tester);
    final c = h.list!;
    await c.prepare(c.services.single, 'switch.synthetic', isCurrent: () => true);
    final old = c.preview!; h.elapsed += const Duration(seconds: 61);
  ...
  testWidgets('uncertain confirm is consumed, explicit GET recovers without write retry', (tester) async {
    ...
    h.reply = (_) async => jsonResponse({'error': {'code': 'server_error'}}, 503);
    await c.confirm(preview, isCurrent: () => true);
    expect(c.uncertain, isTrue); expect(c.preview, isNull); expect(c.canPreview, isFalse);
    await c.confirm(preview, isCurrent: () => true);
    h.reply = null; h.bound = true; await c.refresh();
    expect(c.binding, isNotNull); expect(c.canPreview, isTrue);
    expect(h.requests.where((r) => r.url.path.endsWith('/binding-confirm')).length, 1);
  });
  for (final loss in ['PIN', 'route', 'window', 'source', 'logout']) {
    for (final status in [200, 401]) {
      testWidgets('late$status after $loss clears read and preserves unrelated current account', (tester) async {
        final h = HaHarness(), pending = Completer<http.Response>();
        h.reply = (r) => r.url.path.contains('/home-resources/') ? Future.value(jsonResponse({'record': h.f['resource']})) : pending.future;
        await h.mount(tester, admin: false);
        expect(h.requests.length, 2);
        ...
        h.owner.synchronize();
        pending.complete(jsonResponse(status == 200 ? h.f['snapshotOff']['response'] : {'error': {'code': 'unauthorized'}}, status));
        await settle(tester);
        expect(h.list!.snapshot, isNull); expect(h.list!.record, isNull);
        expect(h.account.session == null, loss == 'logout');
        expect(h.store.value == null, loss == 'logout');
      });
    }
  }
  testWidgets('current Core401 rejects account; upstream502 preserves it', (tester) async {
    final h = HaHarness()..snapshotStep = 'upstreamUnauthorized';
    await h.mount(tester, admin: false);
    expect(h.list!.failure, 'ha_upstream_unauthorized'); expect(h.account.session, isNotNull);
  ...
  for (final denied in ['direct', 'memberAdmin', 'password']) {
    testWidgets('$denied performs zero feature HTTP', (tester) async {
      ...
      await h.mount(tester); await h.list!.refresh();
      expect(h.requests, isEmpty); expect(h.transports, 0);
    });
  }
}
