...
import 'core_ha_ui_test.dart'
    show key, reveal, openBinding, openSnapshot, effectiveNodes;

Finder native(String id) =>
    find.descendant(of: key(id), matching: find.byType(CupertinoButton));
List<SemanticsNode> effective(WidgetTester tester) =>
    effectiveNodes(tester, key('core-ha-binding'));

bool focused(WidgetTester tester, String id) => Focus.of(
  tester.element(find.descendant(of: key(id), matching: find.byType(Text))),
).hasPrimaryFocus;
Rect outline(WidgetTester tester, String id) {
  final box = find
      ...
      .first;
  final border =
      (tester.widget<DecoratedBox>(box).decoration as ShapeDecoration).shape
          as OutlinedBorder;
  ...
  for (final locale in ['en', 'tr']) {
    for (final width in [320.0, 600.0, 1280.0]) {
      for (final dark in [false, true]) {
        testWidgets(
          '$locale $width ${dark ? 'dark' : 'light'} 2x real-font binding effective semantics and native Tab viewport',
          (tester) async {
            ...
            try {
              ...
              for (final pair in [
                ('core-ha-cancel', l.commonCancel),
                ('core-ha-confirm', l.coreHaConfirm),
              ]) {
                await reveal(tester, native(pair.$1));
                final nodes = effective(tester)
                    .where(
                      (n) =>
                          n.getSemanticsData().flagsCollection.isButton &&
                          n.getSemanticsData().label == pair.$2,
                    )
                    .toList();
                ...
                expect(
                  nodes.single.getSemanticsData().flagsCollection.isHeader,
                  isFalse,
                );
              }
              ...
              for (var i = 0; i < 6 && !seen.contains('core-ha-confirm'); i++) {
                ...
                for (final id in [
                  ...
                  seen.add(id);
                  final ring = outline(tester, id),
                      viewport = id == 'core-ha-back'
                          ? Rect.fromLTWH(0, 0, width, 1000)
                          : tester.getRect(key('core-ha-scroll'));
                  expect(ring.left, greaterThanOrEqualTo(viewport.left));
                  expect(ring.right, lessThanOrEqualTo(viewport.right));
                  expect(
                    ring.top,
              ...
              final ring = outline(tester, 'core-ha-cancel'),
                  viewport = tester.getRect(key('core-ha-scroll'));
              expect(ring.top, greaterThanOrEqualTo(viewport.top));
              expect(ring.bottom, lessThanOrEqualTo(viewport.bottom));
              await png(
                tester,
                h,
                '$locale-${width.toInt()}-${dark ? 'dark' : 'light'}-binding-2x',
              );
              await tester.sendKeyEvent(LogicalKeyboardKey.enter);
              await flush(tester);
              expect(
                h.adapterRequests.where(
                  (r) => r.url.path.endsWith('/binding-confirm'),
                ),
                isEmpty,
              );
              expect(h.haReads, 0);
