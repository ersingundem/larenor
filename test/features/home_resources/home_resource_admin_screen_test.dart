...
void main() {
  testWidgets('member Core metadata never exposes management entry', (tester) async {
    final h=ResourceAdminHarness()..role='member';
    await h.mount(tester);await h.signIn();await flush(tester);
    expect(adminKey('home-resources-manage'),findsNothing);expect(h.mutations,isEmpty);expect(h.haReads,0);
  });
  for(final kind in ['room','resource']) {
    testWidgets('actual Core PIN flow creates renames reorders and deletes $kind metadata only', (tester) async {
      ...
      expect(h.mutations.length,1);expect(h.mutations.single.method,'POST');
      expect(adminKey('home-resource-mutation-saved'),findsOneWidget);
      final saved=h.records.last;final id=saved['ref']['id'] as String;
      expect(saved['ref']['kind'],kind);expect(saved['label'],'Yeni kayıt');
      await adminPress(tester,'home-resource-edit-$id');
      await tester.enterText(adminKey('home-resource-label'),'Düzenlenen kayıt');
      await tester.enterText(adminKey('home-resource-order'),'2');
      await adminPress(tester,'home-resource-save');
      expect(h.mutations.length,2);expect(h.mutations.last.method,'PATCH');
      expect(h.records.last['label'],'Düzenlenen kayıt');expect(h.records.last['order'],2);
      ...
      expect(h.mutations.length,3);expect(h.mutations.last.method,'DELETE');
  ...
  testWidgets('cancel discards metadata form and delete target without a mutation', (tester) async {
    ...
    await adminPress(tester,'home-resource-delete-$id');
    await adminPress(tester,'home-resource-cancel-edit');
    expect(adminKey('home-resource-delete-confirmation'),findsNothing);expect(h.mutations,isEmpty);
  });
}
