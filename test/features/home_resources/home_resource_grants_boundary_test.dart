...
Future<void> grantLoss(
  ...
  if (loss == 'route') {
    unawaited(
      Navigator.of(tester.element(adminKey('home-resource-grants-screen')))
          .push(
...
VoidCallback callback(WidgetTester tester, String key) =>
...
void main() {
  ...
  testWidgets('held Save cannot bypass the separate revoke confirmation', (
    ...
    await select(tester);
    await adminPress(tester, 'resource-grants-none');
    final save = callback(tester, 'resource-grants-save');
    save();
    await flush(tester);
  ...
  testWidgets('held permission action cannot change a frozen revoke proposal', (
    tester,
  ) async {
    final h = GrantsHarness()..grants['3' * 32] = {'read': true, 'write': true};
    await openGrants(tester, h);
    await select(tester);
    final read = callback(tester, 'resource-grants-readOnly');
    await adminPress(tester, 'resource-grants-none');
    await adminPress(tester, 'resource-grants-save');
    read();
    await flush(tester);
  ...
  for (final loss in [
    ...
    for (final revoke in [false, true]) {
      testWidgets(
        '${revoke ? 'revoke' : 'save'} old grant callback on $loss never PUTs',
        (tester) async {
          final h = GrantsHarness()
            ..grants['3' * 32] = {'read': true, 'write': true};
          await openGrants(tester, h);
          await select(tester, revoke: revoke);
          final action = callback(
            tester,
            revoke ? 'resource-grants-confirm-revoke' : 'resource-grants-save',
          );
          await grantLoss(tester, h, loss);
          action();
          await flush(tester);
          expect(h.puts, isEmpty);
          expect(adminKey('resource-grants-revoke-confirmation'), findsNothing);
          expect(h.haReads, 0);
  ...
  for (final loss in ['window', 'background', 'source', 'role']) {
    testWidgets(
      'pending grant $loss closes client and late401 cannot reject current auth',
      (tester) async {
        final h = GrantsHarness();
        await openGrants(tester, h);
        await select(tester);
        ...
        if (loss != 'role') {
          ...
          expect(h.account.session!.user.role.name, 'member');
        }
        expect(adminKey('resource-grants-saved'), findsNothing);
        expect(h.puts.length, 1);
      },
  ...
  for (final loss in ['pin', 'root']) {
    testWidgets('$loss preframe retires grant callback', (tester) async {
      ...
      await openGrants(tester, h);
      await select(tester);
      final action = callback(tester, 'resource-grants-save');
      if (loss == 'pin') {
        final c = h.runtime(tester);
        c.invalidate(pinLockProvider);
        expect(c.read(pinLockProvider).isLoading, isTrue);
  ...
  testWidgets(
    'same Save callback cannot double dispatch or revive after successful PUT',
    (tester) async {
      final h = GrantsHarness();
      await openGrants(tester, h);
      await select(tester);
      ...
      late.complete(
        h.json({
          'grant': {
            'subjectId': '3' * 32,
            'target': h.records.first['ref'],
            'aclRevision': h.records.first['aclRevision'],
            'permissions': h.grants['3' * 32],
          },
        }),
      );
      h.pendingGrant = null;
      ...
      await flush(tester);
      expect(h.puts.length, 1);
      expect(adminKey('resource-grants-saved'), findsOneWidget);
    },
  );
}
