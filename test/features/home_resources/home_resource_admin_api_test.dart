...
Map<String, dynamic> initialRecord() {
  final fixture = jsonDecode(
    File('contracts/home-resources.v1.json').readAsStringSync(),
  ) as Map;
  return {
    ...fixture['adminList']['entries'][0] as Map<String, dynamic>,
    'aclRevision': 1,
  };
}

final scope = ServerContext.fromJson({
  'schemaVersion': 1,
  'coreId': 'a' * 32,
  'homeId': 'b' * 32,
});
final otherScope = ServerContext.fromJson({
  'schemaVersion': 1,
  'coreId': 'c' * 32,
  'homeId': 'd' * 32,
});
String get basePath => '/admin/home-resources/${scope.coreId}/${scope.homeId}';
HomeResourceRecord parsed(Map<String, dynamic> record) =>
    HomeResourceRecord.fromJson(record, expectedContext: scope);
...
void main() {
  ...
  for (final label in [
    ...
    test(
      'invalid metadata label ${label.codeUnits} is rejected without transport',
      () async {
        var calls = 0;
        final client = LarenorServerApi(
          endpoint: ServerEndpoint('https://synthetic.invalid'),
          client: MockClient((_) async {
            calls++;
            return response({'record': initialRecord()});
          }),
        );
        addTearDown(client.close);
        final api = HomeResourceAdminApi(client, 'synthetic-token', scope);
        await expectLater(
          api.create(kind: HomeResourceKind.room, label: label, order: 0),
          throwsA(failure('invalid_request')),
        );
        await expectLater(
          api.update(parsed(initialRecord()), label: label, order: 0),
          throwsA(failure('invalid_request')),
        );
        expect(calls, 0);
      },
  ...
  for (final order in [-1, 10001]) {
    test('invalid order $order is rejected before transport', () async {
      ...
      final api = HomeResourceAdminApi(client, 'synthetic-token', scope);
      await expectLater(
        api.create(kind: HomeResourceKind.room, label: 'Room', order: order),
        throwsA(failure('invalid_request')),
      );
      await expectLater(
        api.update(parsed(initialRecord()), label: 'Room', order: order),
        throwsA(failure('invalid_request')),
      );
      expect(calls, 0);
    });
  ...
  for (final pair in [
    ...
    test(
      'request label follows Server codepoint and strip contract ${pair.$2.runes.length}',
      () {
  ...
  for (final kind in HomeResourceKind.values) {
    test(
      'create ${kind.name} sends exact fields and binds generated identity',
      () async {
        final raw = initialRecord();
        raw['ref'] = {...raw['ref'] as Map, 'kind': kind.name};
        var calls = 0;
        final client = LarenorServerApi(
        ...
        expect(result.id, raw['ref']['id']);
        expect(result.kind, kind);
        expect(result.revision, 1);
        expect(result.aclRevision, 1);
  ...
  for (final changed in [false, true]) {
    test(
      'update ${changed ? 'changed' : 'no-op'} binds both revisions without altering ACL',
      () async {
        final raw = initialRecord();
        raw['aclRevision'] = 7;
        ...
        addTearDown(client.close);
        final result = await HomeResourceAdminApi(
          client,
          'synthetic-token',
          scope,
        ).update(before, label: ' $label ', order: 1);
        expect(result.label, label);
        expect(result.revision, changed ? 2 : 1);
        expect(result.aclRevision, 7);
        expect(calls, 1);
      },
    );
  }
  for (final mutation in [
    ...
    test(
      'create rejects response $mutation outside the submitted contract',
      () async {
        final raw = initialRecord();
        switch (mutation) {
          case 'id':
            raw['ref'] = {...raw['ref'] as Map, 'id': 'not-an-id'};
          case 'kind':
            raw['ref'] = {...raw['ref'] as Map, 'kind': 'resource'};
          case 'revision':
            raw['revision'] = 2;
          case 'aclRevision':
            raw['aclRevision'] = 2;
          ...
          case 'order':
            raw['order'] = 0;
          case 'permissions':
            raw['permissions'] = {'read': true, 'write': false};
        }
        var calls = 0;
        final client = LarenorServerApi(
          endpoint: ServerEndpoint('https://synthetic.invalid'),
          client: MockClient((_) async {
            ...
            return response({
              'record': raw,
              if (mutation == 'extra') 'extra': 'private',
            }, 201);
          }),
        );
        addTearDown(client.close);
        await expectLater(
          HomeResourceAdminApi(
            ...
            'synthetic-token',
            scope,
          ).create(kind: HomeResourceKind.room, label: 'Salon', order: 1),
  ...
  test('cross-context update/delete fail before any HTTP', () async {
    ...
    await expectLater(
      api.update(parsed(initialRecord()), label: 'X', order: 0),
      throwsA(failure('invalid_request')),
    );
    await expectLater(
      api.delete(parsed(initialRecord())),
      throwsA(failure('invalid_request')),
    );
    expect(calls, 0);
  });
  for (final mutation in [
    ...
    test('rejects update response $mutation without replay', () async {
      final raw = initialRecord();
      final target = parsed(raw);
      final changed = {...raw, 'revision': 2, 'label': 'Renamed'};
      switch (mutation) {
        case 'wrongScope':
          changed['ref'] = {...raw['ref'] as Map, 'homeId': 'd' * 32};
        case 'wrongId':
          changed['ref'] = {...raw['ref'] as Map, 'id': 'e' * 32};
        case 'wrongKind':
          changed['ref'] = {...raw['ref'] as Map, 'kind': 'resource'};
        case 'label':
          changed['label'] = 'Unexpected';
        case 'order':
          changed['order'] = 5;
        case 'revision':
          changed['revision'] = 3;
        case 'aclRevision':
          changed['aclRevision'] = 2;
        case 'readOnly':
      ...
      final client = LarenorServerApi(
        ...
        client: MockClient((_) async {
          ...
          return response({
            'record': changed,
            if (mutation == 'extraEnvelope') 'secret': 'private-upstream',
          });
        }),
      );
      addTearDown(client.close);
      await expectLater(
        HomeResourceAdminApi(
          ...
          'synthetic-token',
          scope,
        ).update(target, label: 'Renamed', order: 1),
        throwsA(failure('invalid_response')),
      );
      expect(calls, 1);
    });
  ...
  for (final status in [401, 403, 404, 409, 500, 503]) {
    test(
      'write error $status is static and never automatically retried',
      () async {
        var calls = 0;
        final client = LarenorServerApi(
          endpoint: ServerEndpoint('https://synthetic.invalid'),
          client: MockClient((_) async {
            ...
            return response({
              'error': {
                'code': status == 409 ? 'revision_conflict' : 'private-token',
        ...
        await expectLater(
          HomeResourceAdminApi(
            ...
            'synthetic-token',
            scope,
          ).create(kind: HomeResourceKind.room, label: 'Room', order: 0),
          throwsA(
            isA<LarenorServerException>().having(
              (e) => e.toString().contains('private-token'),
              'redacted',
              isFalse,
            ),
          ),
  ...
  for (final variant in [
    ...
    test('delete only accepts actual 204 empty: $variant', () async {
      ...
      addTearDown(client.close);
      final operation = HomeResourceAdminApi(
        client,
        'synthetic-token',
        scope,
      ).delete(target);
      if (variant == '204empty') {
        ...
        await expectLater(operation, throwsA(isA<LarenorServerException>()));
      }
      expect(calls, 1);
    });
  }
  test('closed transport rejects a late possibly applied write without replay', () async {
    var calls = 0;
    ...
    final pending = HomeResourceAdminApi(
      client,
      'synthetic-token',
      scope,
    ).create(kind: HomeResourceKind.room, label: 'Salon', order: 1);
    ...
    client.close();
    // MockClient does not implement AbortableRequest cancellation. Deliver the
    // late response explicitly and assert the real decoder rejects publication.
    gate.complete(response({'record': initialRecord()}, 201));
    await expectation;
    await Future<void>.delayed(Duration.zero);
    expect(calls, 1);
  });
  test(
    'timeout leaves the single potentially applied POST uncertain',
    () async {
      var calls = 0;
      final gate = Completer<http.Response>();
      final client = LarenorServerApi(
