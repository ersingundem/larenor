...
void main() {
  for (final locale in ['en', 'tr']) {
    for (final width in [320.0, 600.0, 1280.0]) {
      testWidgets('admin $locale $width 2x keyboard and named 48px controls', (
        tester,
      ) async {
        ...
        try {
          ...
          await capture(tester, h, 'admin-list-$locale-${width.toInt()}-2x');
          ...
          for (final field in ['home-resource-label', 'home-resource-order']) {
          ...
          await flush(tester);
          await capture(tester, h, 'admin-form-$locale-${width.toInt()}-2x');
          ...
          await tester.ensureVisible(adminKey('home-resource-confirm-delete'));
          await flush(tester);
          expect(
