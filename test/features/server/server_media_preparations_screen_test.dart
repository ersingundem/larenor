...
void main() {
  ...
  Future<void> mount(
    ...
    f = MediaPreparationsFixture(role: role);
  ...
  testWidgets('history loads without mutation and an existing preparation opens and cancels', (tester) async {
    await mount(tester);
    expect(f.mutations, isEmpty);
    final id = f.records.single['id'];
    await tap(tester, 'media-view-$id');
    expect(find.textContaining('Installation has not started'), findsOneWidget);
  ...
  testWidgets('one explicit action creates a six component preparation without secret forms', (tester) async {
    await mount(tester);
    f.records.clear();
    ...
    expect(find.textContaining('Installation has not started'), findsOneWidget);
    expect(f.calls.any((r) => r.url.path.contains('/jobs') || r.url.path.contains('/install')), isFalse);
    expect(tester.takeException(), isNull);
  });
  testWidgets('tablet Turkish at twice text scale supports keyboard and cancellation', (tester) async {
  ...
  for (final reason in ['background', 'hidden', 'account', 'route', 'idle']) {
    testWidgets('$reason retires a pending confirmation and cannot send cancellation', (tester) async {
      ...
      await tap(tester, 'media-cancel');
      final stale = tester.widget<CupertinoDialogAction>(find.byKey(const ValueKey('media-cancel-confirm'))).onPressed!;
      switch (reason) {
        case 'background':
          tester.binding.handleAppLifecycleStateChanged(AppLifecycleState.paused);
        case 'hidden': visible.value = false;
        case 'account': await f.account.signOut();
        case 'route': navigator.currentState!.push(CupertinoPageRoute<void>(builder: (_) => const CupertinoPageScaffold(child: Text('Covered'))));
      ...
      await tester.pumpAndSettle();
      expect(f.mutations, isEmpty);
      expect(find.byKey(const ValueKey('media-cancel-confirm')), findsNothing);
      tester.binding.handleAppLifecycleStateChanged(AppLifecycleState.resumed);
    });
  }
  testWidgets('member account never fetches preparations or offers creation', (tester) async {
  ...
  testWidgets('component screen links the unified preparation while existing jobs remain available', (tester) async {
