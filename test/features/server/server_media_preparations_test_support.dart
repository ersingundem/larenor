...
Map<String, dynamic> mediaFixtureJson() => jsonDecode(
  File('contracts/media-preparations.v1.json').readAsStringSync(),
) as Map<String, dynamic>;
Map<String, dynamic> mediaPreparationJson({bool cancelled = false}) =>
    mediaFixtureJson()[cancelled ? 'cancelled' : 'prepared'] as Map<String, dynamic>;
...
class MediaPreparationsFixture extends PluginsFixture {
  ...
  http.Response pluginResponse(http.Request request) {
    ...
    if (path.endsWith('/context')) return this.json(mediaFixtureJson()['context']);
    if (path.endsWith('/media/preparations')) {
      if (request.method == 'GET') {
        return this.json({'preparations': records, 'nextBefore': null});
      }
      final body = jsonDecode(request.body);
      final existing = records.where((r) => r['requestId'] == body['requestId']);
      if (existing.isNotEmpty) return this.json({'preparation': existing.first}, 201);
    ...
    if (path.contains('/media/preparations/')) {
      ...
      if (found < 0) return this.json({'error': {'code': 'not_found'}}, 404);
      if (path.endsWith('/cancel')) {
        final previous = records[found];
        if (jsonDecode(request.body)['expectedRevision'] != previous['revision']) {
