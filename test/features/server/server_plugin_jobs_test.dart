...
void main() {
  ...
  test('all durable states parse and completion retains failed and unknown checks', () {
    for (final state in ['queued', 'running', 'succeeded', 'failed', 'cancelled', 'needs_attention']) {
      ...
      if (state == 'succeeded') {
        expect(job.result!.checks.map((c) => c.status), ['passed', 'failed', 'unknown']);
  ...
  final bad = <String, void Function(Map<String, dynamic>)>{
    ...
    'non UTC timestamp': (v) => v['updatedAt'] = '2026-09-05T12:00:01.000+03:00',
    ...
    'succeeded missing result': (v) {v['state'] = 'succeeded'; v['phase'] = 'complete';},
    'cancelled without requested cancellation': (v) {v['state'] = 'cancelled'; v['phase'] = 'complete';},
  };
  for (final sample in bad.entries) {
    test('job rejects ${sample.key}', () {
      final json = pluginJobJson(); sample.value(json);
  ...
  for (final sample in <String, void Function(Map<String, dynamic>)>{
    ...
    test('result rejects ${sample.key}', () {
      final json = pluginJobJson(state: 'succeeded'); sample.value(json['result']);
      expect(() => ServerPluginJob.fromJson(json), invalid);
    });
  }
  test('capabilities never accept installation availability or secret fields', () {
    expect(ServerPluginJobCapabilities.fromJson({'preflightConfigured': true, 'installAvailable': false}).preflightConfigured, isTrue);
    expect(() => ServerPluginJobCapabilities.fromJson({'preflightConfigured': true, 'installAvailable': true}), invalid);
    expect(() => ServerPluginJobCapabilities.fromJson({'preflightConfigured': true, 'installAvailable': false, 'socket': '/secret'}), invalid);
  });

  group('guarded jobs API and controller', () {
    late PluginJobsFixture f;
    late ServerPluginJobsController controller;
    setUp(() async { f = PluginJobsFixture(); await f.account.initialize(); controller = ServerPluginJobsController(f.account, requestId: () => 'e' * 32); });
    tearDown(() {controller.dispose(); f.account.dispose();});
    ServerPluginPreview preview() => ServerPluginPreview.fromJson(pluginPreviewJson()['preview']);
    ...
    test('create is explicitly requested and binds returned request and plan', () async {
      await controller.load(current: () => true);
      expect(f.mutations, isEmpty);
      await controller.launch(preview(), current: () => true);
      expect(controller.selected!.requestId, 'e' * 32);
      expect(jsonDecode(f.mutations.single.body), {'operation': 'preflight', 'previewId': 'd' * 32, 'expectedRevision': 1, 'planHash': preview().plan.planHash, 'requestId': 'e' * 32});
      expect(f.calls.any((r) => r.url.path.contains('/install')), isFalse);
    });
    test('uncertain create retains the same request for manual recovery only', () async {
      await controller.load(current: () => true);
      var writes = 0;
    ...
    test('stale account response cannot expose history or restore pending requests', () async {
      ...
      f.respond = (request) => request.url.path.endsWith('/jobs') ? held.future : Future.value(f.pluginResponse(request));
      final loading = controller.load(current: () => true);
      await Future<void>.delayed(Duration.zero);
      controller.invalidate();
      held.complete(f.json({'jobs': [pluginJobJson()], 'nextBefore': null}));
      await loading;
    ...
    test('API rejects mismatched selected identity, secret errors and unsafe cursor pages', () async {
      await f.account.withSession((api, session) async {
        ...
        f.respond = (_) async => f.json({'job': {...pluginJobJson(), 'id': 'b' * 32}});
        await expectLater(jobs.get('a' * 32), invalid);
        f.respond = (_) async => f.json({'jobs': [pluginJobJson(), pluginJobJson()], 'nextBefore': null});
        await expectLater(jobs.list(), invalid);
        f.respond = (_) async => f.json({'events': [pluginJobEventJson(sequence: 2), pluginJobEventJson(sequence: 1)], 'nextAfter': null});
        await expectLater(jobs.events('a' * 32), invalid);
      });
    });
    test('cancel uses the current selected revision and never starts installation', () async {
      await controller.load(current: () => true);
