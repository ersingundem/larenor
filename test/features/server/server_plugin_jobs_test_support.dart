...
Map<String, dynamic> pluginJobJson({
  String state = 'queued',
  int revision = 1,
}) {
...
Map<String, dynamic> pluginJobEventJson({
  int sequence = 1,
  String code = 'job_queued',
  int revision = 1,
}) => {
  'sequence': sequence,
  'code': code,
...
class PluginJobsFixture extends PluginsFixture {
  ...
  http.Response pluginResponse(http.Request request) {
    ...
    if (path.endsWith('/jobs/capabilities')) {
      return this.json({
        'preflightConfigured': configured,
    ...
    if (path.endsWith('/events')) {
      return this.json({
        'events': request.url.queryParameters['after'] == '0'
            ? [pluginJobEventJson()]
            : [],
        'nextAfter': null,
      });
    ...
    if (path.endsWith('/cancel')) {
      job = {
        ...pluginJobJson(state: 'cancelled', revision: job['revision'] + 1),
