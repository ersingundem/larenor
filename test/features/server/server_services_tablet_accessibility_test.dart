...
void main() {
  ...
  Future<void> mount(
    ...
    for (var i = 2; i < recordCount; i++) {
      fixture.records.add({
        ...serviceJson(),
        'id': i.toRadixString(16) * 32,
        'name': 'Additional connection ${i + 1}',
      });
    }
  ...
  List<Color> backgrounds(WidgetTester tester, Finder button) {
    ...
    tester.element(caption(button)).visitAncestorElements((element) {
      if (element.widget case DecoratedBox(
        decoration: final BoxDecoration box,
      )) {
    ...
    expect(
      result,
      isNotNull,
      reason: 'measure the actual card or page surface',
    );
    return result!;
  }

  Future<double> paintedFocusContrast(
    WidgetTester tester,
    Finder button,
  ) async {
    final render =
  ...
  for (final language in ['en', 'tr']) {
    for (final width in [600.0, 1280.0]) {
        ...
        try {
          ...
          for (final record in fixture.records) {
            ...
            final effective = nodes(tester, title).where((node) {
              ...
              return data.flagsCollection.isHeader &&
                  data.label == record['name'];
            }).toList();
            expect(
              effective,
              hasLength(1),
      ...
      testWidgets(
        'service actions name their connection once $language $width 2x',
        (tester) async {
          final semantics = tester.ensureSemantics();
          try {
            await mount(tester, language: language, width: width);
            final l10n = AppLocalizations.of(
              tester.element(key('services-add')),
            );
            for (final record in fixture.records) {
              for (final entry in {
      ...
      testWidgets(
        'forget modal real Tab reverse Enter and Space $language $width 2x',
        (tester) async {
          await mount(tester, language: language, width: width);
          final open = key('service-forget-$serviceId');
          await tap(tester, open);
          final l10n = AppLocalizations.of(
            tester.element(key('service-confirm-forget')),
          );
          ...
          for (var i = 0; i < 8; i++) {
            await tester.sendKeyEvent(LogicalKeyboardKey.tab);
            await tester.pumpAndSettle();
            if (Focus.maybeOf(tester.element(caption(cancel)))
                    ?.hasPrimaryFocus ==
                true) {
              break;
            }
          ...
          expect(
            Focus.maybeOf(tester.element(caption(cancel)))?.hasPrimaryFocus,
            isTrue,
            reason: 'Tab must reach Cancel in the actual modal',
          );
          await tester.sendKeyEvent(LogicalKeyboardKey.tab);
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'form keyboard footer has painted focus $language $width 2x',
        (tester) async {
          await mount(
            tester,
            language: language,
          ...
          await tester.pumpAndSettle();
          final l10n = AppLocalizations.of(
            tester.element(key('service-submit')),
          );
          final cancel = find.widgetWithText(
            CupertinoButton,
            l10n.commonCancel,
          ...
          expect(focus(tester, cancel).hasPrimaryFocus, isTrue);
          expect(
            await paintedFocusContrast(tester, cancel),
            greaterThanOrEqualTo(3),
          );
          await preview(
            tester,
            'services-form-keyboard-$language-${width.toInt()}-2x',
          );
          await tester.sendKeyEvent(LogicalKeyboardKey.enter);
          await tester.pumpAndSettle();
      ...
      testWidgets('form input Next advances only once $language $width 2x', (
        tester,
      ) async {
        await mount(tester, language: language, width: width);
        await tap(tester, key('services-add'));
        ...
        expect(
          focus(tester, cancel).hasPrimaryFocus,
          isTrue,
          reason:
              'Next must move once; Save focused: '
      ...
      for (final scale in [1.0, 2.0]) {
        testWidgets(
          'forget effective button geometry $language $width ${scale}x',
          (tester) async {
            final semantics = tester.ensureSemantics();
            try {
              await mount(
              ...
              final failures = restoreDialogGeometryFailures(
                tester,
                labels: [l10n.commonCancel, l10n.serverServicesForget],
                cancel: cancel,
              );
              ...
              await preview(
                tester,
                'services-modal-$language-${width.toInt()}-${scale.toInt()}x',
              );
              await tester.sendKeyEvent(LogicalKeyboardKey.enter);
              await tester.pumpAndSettle();
              expect(find.byType(CupertinoAlertDialog), findsNothing);
        ...
        testWidgets(
          'form labeled fields fit native targets $language $width ${scale}x',
          (tester) async {
            final semantics = tester.ensureSemantics();
            try {
              await mount(
              ...
              for (final field in {
                ...
                if (matching.single.rect.width < 48 ||
                    matching.single.rect.height < 48) {
              ...
              await tester.pump();
              expect(
                tester.widget<CupertinoTextField>(token).obscureText,
                isTrue,
              );
              expect(
                tester
              ...
              await preview(
                tester,
                'services-form-$language-${width.toInt()}-${scale.toInt()}x',
              );
              await tester.sendKeyEvent(LogicalKeyboardKey.enter);
              await tester.pumpAndSettle();
              expect(key('service-submit'), findsNothing);
  ...
  for (final dark in [false, true]) {
    for (final width in [600.0, 1280.0]) {
      testWidgets('native Tab viewport and contrast $dark $width 2x', (
        tester,
      ) async {
        await mount(
          tester,
        ...
        for (final id in order) {
          ...
          if (id == 'service-check-$serviceId') {
            await preview(
              tester,
              'services-list-${dark ? 'tr-dark' : 'en-light'}-${width.toInt()}-2x',
            );
          }
        }
  ...
  testWidgets('busy list removes semantic actions and held Edit does nothing', (
    tester,
  ) async {
    final semantics = tester.ensureSemantics();
    ...
    try {
      await mount(tester);
      final edit = key('service-edit-$serviceId');
      await visible(tester, edit);
      final held = tester.widget<CupertinoButton>(edit).onPressed!;
      ...
      fixture.respond = (request) =>
          ...
          ? pending.future
          : normal(request);
      final refresh = key('services-refresh');
      ...
      await tester.pump();
      await tester.pump();
      expect(tester.widget<CupertinoButton>(edit).onPressed, isNull);
      final l10n = AppLocalizations.of(tester.element(edit));
      final node = nodes(tester, caption(edit)).singleWhere((node) {
        final data = node.getSemanticsData();
        return data.flagsCollection.isButton &&
            data.label.contains(fixture.records.first['name'] as String) &&
            data.label.contains(l10n.commonEdit);
      });
      expect(
  ...
  testWidgets('visible toolbar Tab reverse does not jump scroll offset', (
    tester,
  ) async {
    await mount(tester, width: 1280, recordCount: 4);
    final add = key('services-add'), refresh = key('services-refresh');
    final position = Scrollable.of(tester.element(add)).position;
    position.jumpTo(20);
    await tester.pumpAndSettle();
    expect(position.pixels, 20, reason: 'fixture has an actual scroll range');
    final top = tester.getRect(find.byType(CupertinoNavigationBar)).bottom;
    for (final button in [add, refresh]) {
    ...
    expect(position.pixels, 20);
    await reverseTab(tester);
    expect(focus(tester, add).hasPrimaryFocus, isTrue);
    expect(position.pixels, 20);
    expect(fixture.mutations, isEmpty);
  });
  ...
  for (final retirement in ['route', 'idle']) {
    testWidgets(
      '$retirement before focus frame prevents stale reveal and reads',
      (tester) async {
        final interaction = AppInteractionController();
        addTearDown(interaction.dispose);
        await mount(tester, interaction: interaction, recordCount: 4);
        ...
        position.jumpTo(
          position.pixels +
              tester.getRect(edit).top -
              tester.getRect(find.byType(CupertinoNavigationBar)).bottom,
        );
        await tester.pumpAndSettle();
        expect(
          tester.getRect(edit).top - 3.5,
          lessThan(tester.getRect(find.byType(CupertinoNavigationBar)).bottom),
        );
        focus(tester, edit).requestFocus();
        FocusManager.instance.applyFocusChangesIfNeeded();
        final offset = position.pixels, calls = fixture.calls.length;
        if (retirement == 'route') {
          unawaited(
            Navigator.of(tester.element(edit)).push(
    ...
    testWidgets('$retirement retires retained modal keyboard confirmation', (
      tester,
    ) async {
      final interaction = AppInteractionController();
      addTearDown(interaction.dispose);
      ...
      final held = detector.actions![ActivateIntent]!;
      if (retirement == 'route') {
        unawaited(
      ...
      await tester.pumpAndSettle();
      const ActionDispatcher().invokeAction(held, const ActivateIntent());
      await tester.pumpAndSettle();
      expect(fixture.mutations, isEmpty);
      expect(fixture.records, hasLength(2));
      expect(find.byType(CupertinoAlertDialog), findsNothing);
