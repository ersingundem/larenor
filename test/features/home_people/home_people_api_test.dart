...
final class _AuthApi extends LarenorServerApi {
  _AuthApi(this.value)
    : super(
        ...
        client: MockClient((_) async => response(null, 500)),
      );
...
void main() {
  ...
  HomePersonRecord target([String step = 'createPerson']) =>
      HomePersonRecord.fromJson(
        f[step]['response']['person'],
  ...
  test(
    'actual authenticated fixture matches read/admin CRUD and grant transport',
    () async {
      ...
      step = 'createPerson';
      var record = await api.create(
        label: f[step]['body']['label'] as String,
        order: 7,
      );
  ...
  test(
    'foreign context targets and malformed arguments dispatch zero requests',
    () async {
      var calls = 0;
      final api = apiWith(
        MockClient((_) async {
      ...
      final other = ServerContext.fromJson(
        f['otherContextList']['response']['scope'],
      );
      final foreign = HomePersonRecord.fromJson(
        f['otherContextList']['response']['entries'][0],
        expectedContext: other,
      );
      await expectLater(
        api.update(foreign, label: 'Foreign', order: 0),
      ...
      await expectLater(
        api.create(label: 'x' * 81, order: 0),
        throwsA(failure('invalid_request')),
      );
      await expectLater(
        api.create(label: 'Good', order: -1),
        throwsA(failure('invalid_request')),
      );
      final grants = HomePersonGrants.fromJson(
  ...
  test(
    'explicit retire before dispatch and throwing lifetime hook are safe',
    () async {
      var calls = 0;
      final api = apiWith(
        MockClient((_) async {
  ...
  for (final status in [200, 401]) {
    test(
      'actual account logout retires pending people $status response',
      () async {
        ...
        final api = apiWith(
          ...
          current: () =>
              ...
              identical(account.session, captured) &&
              !account.working &&
              !account.hasPendingContext,
        );
        final work = account.withSession((_, _) => api.list());
    ...
    for (final (status, code) in [
      ...
      final api = apiWith(
        MockClient((_) async {
          calls++;
          return response({
            'error': {'code': 'private_body', 'message': 'private_body'},
          }, status);
        }),
      );
  ...
  test(
    'person query allowlist is exact and rejects missing DELETE revisions',
    () async {
      var calls = 0;
      final transport = LarenorServerApi(
        endpoint: ServerEndpoint('https://synthetic.invalid'),
      ...
      addTearDown(transport.close);
      for (final (method, path, query)
          in <(String, String, Map<String, String>?)>[
            ('GET', public, {'after': '1' * 32}),
            ('GET', public, {'limit': '01'}),
            ('GET', public, {'limit': '101'}),
            ('GET', public, {'expectedSnapshot': 'bad'}),
            ('GET', public, {'userId': 'e' * 32}),
            ...
            (
              'DELETE',
              '$admin/${'1' * 32}',
              {'expectedRevision': '1', 'expectedAclRevision': '0'},
            ),
            (
              'DELETE',
              '$admin/${'1' * 32}',
              {
                'expectedRevision': '1',
                'expectedAclRevision': '9223372036854775808',
              },
            ),
            (
              'DELETE',
        ...
        await expectLater(
          transport.request(method, path, queryParameters: query),
          throwsA(failure('invalid_request')),
        );
      }
      expect(calls, 0);
  ...
  test('retired adapters block every admin method before dispatch', () async {
    var calls = 0;
    final api = apiWith(
      MockClient((_) async {
        calls++;
    ...
    for (final call in <Future<Object?> Function()>[
      () => api.create(label: 'Person', order: 0),
      () => api.update(record, label: 'Person', order: 0),
      () => api.delete(record),
      () => api.grants(record),
      () => api.get(record.id),
      () => api.setGrant(
      ...
      await expectLater(call(), throwsA(failure('cancelled')));
    }
    expect(calls, 0);
  });
  test('retirement after PUT dispatch rejects ACK and never retries or mutates the prior snapshot', () async {
    final pending = Completer<http.Response>();
    var calls = 0;
    final api = apiWith(
      MockClient((request) {
    ...
    final future = api.setGrant(
      before,
      subjectId: subject,
      permission: HomePersonPermission.readOnly,
    );
    final outcome = expectLater(future, throwsA(failure('cancelled')));
    await Future<void>.delayed(Duration.zero);
    api.retire();
    pending.complete(response(f['grantRead']['response']));
    await outcome;
    expect(calls, 1);
  ...
  test(
    'uncertain create response remains a typed failure with one POST',
    () async {
      var calls = 0;
      final api = apiWith(
        MockClient((request) async {
  ...
  test(
    'invalid page arguments and invalid metadata fail before transport',
    () async {
      var calls = 0;
      final api = apiWith(
        MockClient((_) async {
      ...
      for (final call in <Future<Object?> Function()>[
        () => api.list(limit: 0),
        () => api.list(limit: 101),
        () => api.list(after: 'bad', snapshot: '1' * 64),
        () => api.list(after: '1' * 32),
        () => api.list(snapshot: 'bad'),
        () => api.update(target(), label: 'Person', order: 10001),
      ]) {
        await expectLater(call(), throwsA(failure('invalid_request')));
      }
      expect(calls, 0);
    },
  );
  test('metadata revision ceiling allows only exact no-op and rejects changed write', () async {
    final raw = copy(f['createPerson']['response']['person']);
    raw['revision'] = HomePersonGrants.maximumRevision;
    final record = HomePersonRecord.fromJson(raw, expectedContext: context);
    var calls = 0;
    final api = apiWith(
      MockClient((request) async {
        calls++;
        expect(
          jsonDecode(request.body)['expectedRevision'],
          HomePersonGrants.maximumRevision,
        );
        return response({'person': raw});
      }),
    );
    await expectLater(
      api.update(record, label: 'Changed', order: 7),
      throwsA(failure('revision_conflict')),
    );
    expect(calls, 0);
    expect(
      (await api.update(
  ...
  test('mismatched create/update acknowledgement is never published', () async {
    for (final field in [
      'label',
      'order',
      'revision',
      ...
      final raw = copy(f['updatePerson']['response']['person']);
      if (field == 'write') {
        raw['permissions']['write'] = false;
      } else if (field == 'id') {
        raw['ref']['id'] = '9' * 32;
      } else {
        raw[field] = field == 'label' ? 'Different' : 99;
      }
      final api = apiWith(MockClient((_) async => response({'person': raw})));
      await expectLater(
        api.update(target('beforeUpdate'), label: 'Ece Öztürk', order: 0),
    ...
    final api = apiWith(MockClient((_) async => response({'person': raw})));
    await expectLater(
      api.create(label: 'Deniz Öztürk', order: 7),
      throwsA(failure('invalid_response')),
    );
  });
  test('active current people401 reaches actual account rejection', () async {
    final session = ServerSession(
      endpoint: ServerEndpoint('https://synthetic.invalid'),
      accessToken: 'synthetic_access_1234567890',
      refreshToken: 'synthetic_refresh_1234567890',
    ...
    final api = apiWith(
      ...
      current: () =>
          account.isCurrent(generation) && identical(account.session, captured),
    );
    await expectLater(
      account.withSession((_, _) => api.list()),
      throwsA(failure('unauthorized')),
    );
    expect(account.session, isNull);
    expect(store.value, isNull);
  ...
  test(
    'grant capacity and revision ceiling reject changed writes before dispatch',
    () async {
      var calls = 0;
      final api = apiWith(
        MockClient((_) async {
      ...
      final ref = f['createPerson']['response']['person']['ref'];
      final entries = [
        for (var i = 0; i < 128; i++)
          {
            'subjectId': i.toRadixString(16).padLeft(32, '0'),
            'target': ref,
            'aclRevision': 1,
            'permissions': {'read': true, 'write': false},
          },
      ];
      final full = HomePersonGrants.fromJson({
        'aclRevision': 1,
        'grants': entries,
      }, target: target());
      await expectLater(
      ...
      final ceiling = HomePersonGrants.fromJson({
        'aclRevision': HomePersonGrants.maximumRevision,
        'grants': [],
      }, target: target());
      await expectLater(
        api.setGrant(
