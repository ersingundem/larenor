...
void main() {
  for (final loss in ['false', 'throw', 'window']) {
    testWidgets('owner $loss retirement never revives after recovery', (
      tester,
    ) async {
      final h = PeopleHarness();
      await h.mount(tester);
      expect(h.list!.entries, isNotEmpty);
      if (loss == 'false') h.pin = false;
      if (loss == 'throw') h.throwsOwner = true;
  ...
  testWidgets(
    'same mounted Consumer reparent with old container alive retires both owners',
    (tester) async {
      final h = PeopleHarness(), key = GlobalKey<PeopleProbeState>();
      await h.mount(tester, key: key);
      final state = key.currentState, old = h.list!, a = h.container;
      final subscription = a.listen(
  ...
  testWidgets(
    'autoDisposed provider closes pending transport and suppresses late401',
    (tester) async {
  ...
  testWidgets(
    'same-context auth refresh pending hides data then binds new pair without cancelling GET',
    (tester) async {
      final h = PeopleHarness();
      await h.mount(tester);
      final c = h.list!;
      h.now = h.now.add(const Duration(minutes: 59, seconds: 40));
      final pending = Completer<ServerContext>();
      h.auth.pendingContext = pending;
      final result = c.refresh();
      await settle(tester);
      expect(h.account.hasPendingContext, isTrue);
      expect(c.entries, isEmpty);
      expect(h.auth.refreshes, 1);
      pending.complete(h.context);
      await result;
      await settle(tester);
  ...
  testWidgets('new user and home never publish old pending list', (
    tester,
  ) async {
    final h = PeopleHarness(), pending = Completer<http.Response>();
    h.reply = (_) => pending.future;
    ...
    await settle(tester);
    expect(h.list!.entries, isEmpty);
    expect(h.account.session!.user.id, 'd' * 32);
    expect(h.account.failure, isNull);
  });
  testWidgets(
    'late dispatched metadata write cannot mutate after owner confirmation retires',
    (tester) async {
      final h = PeopleHarness();
      await h.mount(tester);
      final c = h.list!, pending = Completer<http.Response>();
      h.reply = (_) => pending.future;
      bool current = true;
      final action = c.update(
        c.entries.first,
        label: 'Ece Öztürk',
        order: 0,
        isCurrent: () => current,
      ...
      await settle(tester);
      expect(h.requests.where((r) => r.method == 'PATCH').length, 1);
      current = false;
      pending.complete(jsonResponse(h.f['updatePerson']['response']));
      await action;
