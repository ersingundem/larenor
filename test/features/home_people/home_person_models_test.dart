...
Map<String, dynamic> fixture() => jsonDecode(File('contracts/home-people.v1.json').readAsStringSync()) as Map<String, dynamic>;
Map<String, dynamic> copy(Object value) => jsonDecode(jsonEncode(value)) as Map<String, dynamic>;
Matcher failure(String code) => isA<LarenorServerException>().having((e) => e.code, 'safe code', code);

void main() {
  final f = fixture();
  final context = ServerContext.fromJson(f['context']);
  final person = f['createPerson']['response']['person'];
  HomePersonRecord parse(Object? raw) => HomePersonRecord.fromJson(raw, expectedContext: context);

  test('actual person and Unicode contracts remain separate from rooms/resources', () {
    ...
    expect(parse(f['createUnicode']['response']['person']).label.runes.length, 80);
    expect(() => HomeResourceRecord.fromJson(person, expectedContext: context), throwsA(failure('invalid_response')));
  });
  ...
  for (final invalid in ['', ' ', 'x' * 81, '\n', '\udfff']) {
    test('request metadata rejects invalid ${invalid.codeUnits}', () {
      expect(() => HomePersonMetadata(label: invalid, order: 0), throwsA(failure('invalid_request')));
    });
  }
  test('actual pages bind exact snapshot, cursor and Core/home without aliasing', () {
    final first = HomePeoplePage.fromJson(f['firstPage']['response'], expectedContext: context, limit: 1);
    final second = HomePeoplePage.fromJson(f['secondPage']['response'], expectedContext: context, limit: 1, after: first.nextAfter, expectedSnapshot: first.snapshot);
    ...
    expect(() => HomePeoplePage.fromJson(f['otherContextList']['response'], expectedContext: context), throwsA(failure('invalid_response')));
  });
  final pages = <String, void Function(Map<String, dynamic>)>{
    ...
    'reverse order': (v) => v['entries'] = (v['entries'] as List).reversed.toList(),
    'unknown cursor': (v) => v['nextAfter'] = '9' * 32,
    'empty with cursor': (v) { v['entries'] = []; v['nextAfter'] = '1' * 32; },
  ...
  for (final change in pages.entries) {
    test('list rejects ${change.key}', () {
      ...
      change.value(value);
      expect(() => HomePeoplePage.fromJson(value, expectedContext: context), throwsA(failure('invalid_response')));
    });
  }
  test('page enforces requested limit, previous ID and expected snapshot', () {
    ...
    for (final limit in [0, 1, 101]) {
      expect(() => HomePeoplePage.fromJson(raw, expectedContext: context, limit: limit), throwsA(failure('invalid_response')));
    }
    expect(() => HomePeoplePage.fromJson(raw, expectedContext: context, after: '1' * 32), throwsA(failure('invalid_response')));
    expect(() => HomePeoplePage.fromJson(raw, expectedContext: context, after: '1' * 32, expectedSnapshot: raw['snapshot'] as String), throwsA(failure('invalid_response')));
    expect(() => HomePeoplePage.fromJson(raw, expectedContext: context, expectedSnapshot: '0' * 64), throwsA(failure('invalid_response')));
  });
  test('actual grants no-op, read-write and revoke have exact ACL revisions', () {
    var grants = HomePersonGrants.fromJson(f['emptyGrants']['response'], target: parse(person));
    final subject = f['subjectId'] as String;
    expect(grants.permissionFor(subject), HomePersonPermission.none);
    for (final (name, permission) in [('grantRead', HomePersonPermission.readOnly), ('grantWrite', HomePersonPermission.readWrite), ('grantNoop', HomePersonPermission.readWrite), ('revoke', HomePersonPermission.none)]) {
      grants = grants.withUpdatedGrant(f[name]['response'], subjectId: subject, permission: permission);
    }
    expect(grants.aclRevision, 4);
    expect(grants.grants, isEmpty);
    expect(() => grants.grants[subject] = HomePersonPermission.readWrite, throwsUnsupportedError);
  ...
  for (final change in grantChanges.entries) {
    test('grants reject ${change.key}', () {
      ...
      change.value(raw);
      expect(() => HomePersonGrants.fromJson(raw, target: parse(person)), throwsA(failure('invalid_response')));
    });
  }
  test('grant acknowledgements require exact subject, permission and successor revision', () {
    final before = HomePersonGrants.fromJson(f['emptyGrants']['response'], target: parse(person));
    final subject = f['subjectId'] as String;
    for (final changed in ['subject', 'revision', 'permission', 'unknown']) {
      ...
      expect(() => before.withUpdatedGrant(ack, subjectId: subject, permission: HomePersonPermission.readOnly), throwsA(failure('invalid_response')));
