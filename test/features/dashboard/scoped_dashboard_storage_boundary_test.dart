...
const layout = DashboardLayout(
  rooms: [DashboardRoom(id: 'room', name: 'Private room')],
);
...
class StoragePlatform extends InMemorySharedPreferencesStore {
  ...
  bool formatRead = false, formatWrite = false;
  int writes = 0;
  @override
...
void main() {
  ...
  for (final load in [false, true]) {
    test(
      'retirement after reload but before publication rejects ${load ? "load" : "snapshot"}',
      () async {
        var active = true;
        ...
        await expectLater(
          load ? repo.load() : repo.readSnapshot(),
  ...
  for (final commitFirst in [false, true]) {
    for (final throwing in [false, true]) {
      for (final failReload in [false, true]) {
        test(
          'false/throw write before/after commit $commitFirst/$throwing, reload failure $failReload is never success',
          () async {
            ...
            final restarted = DashboardRepository.core(
              scope: scope,
              isCurrent: () => true,
            );
            if (failReload) {
              ...
              expect(
                (await restarted.load()).rooms.length,
                commitFirst ? 1 : 0,
              );
            }
            expect(platform.writes, 1);
