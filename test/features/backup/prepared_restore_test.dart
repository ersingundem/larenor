...
ServerSession coreSession() => ServerSession(
...
BackupSnapshot restoreFixture([Map<String, dynamic>? settings]) =>
...
const restoreSettings = BackupSelection(
  settings: true,
  dashboard: false,
  connections: false,
);

Future<PreparedBackupRestore> prepare(
  BackupRepository repo,
  TestRestoreAccess access, [
  BackupSnapshot? snapshot,
]) => repo.prepareRestore(
  snapshot ?? restoreFixture(),
  restoreSettings,
...
void main() {
  test('prepared restore reads without effects then survives intended owner unmount exactly once', () async {
    ...
    expect(
      storage.secrets.keys.where(
        (key) => key.startsWith('backup_restore_journal'),
      ),
      isEmpty,
    );
  ...
  test(
    'target change after preparation rejects without journal or field mutation',
    () async {
      final storage = MemoryBackupStorage(preferences: {'appearance': 'dark'});
  ...
  for (final change in ['live', 'durable', 'retired', 'wrongBoundary']) {
    test('expired $change preparation cannot mutate storage', () async {
      ...
      await expectLater(
        change == 'wrongBoundary'
            ? prepared.applyAfterHandoff(
                Object(),
  ...
  test('Core source rejects Direct-bearing settings rather than silently retargeting', () async {
    final storage = MemoryBackupStorage(),
        access = TestRestoreAccess()..source = HomeSource.verifiedCore;
    await expectLater(
      prepare(
        BackupRepository(storage: storage),
        access,
  ...
  test('Core may explicitly restore device preferences without touching either layout or auth', () async {
    final storage = MemoryBackupStorage(
      preferences: {
        'dashboard_layout': 'private',
        'dashboard_layout_core_v1_fixture': 'core',
        'home_source_v1': 'verifiedCore',
      },
      secrets: {
        'larenor_server_session_v1': coreSession().encodeStorage(),
        'settings_pin': 'private-pin',
      },
    );
