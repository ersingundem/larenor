...
class CoreAccess extends f.TestRestoreAccess {
  CoreAccess() {
    source = HomeSource.verifiedCore;
  }
  @override
  Map<String, dynamic> get ownership => {
    'source': source.name,
    'scope': {'coreId': 'a' * 32, 'homeId': 'b' * 32, 'userId': 'one'},
  };
}

Future<List<MemoryBackupStorage>> images({
  bool core = false,
  bool home = true,
  bool ha = false,
}) async {
  final storage = MemoryBackupStorage(
    preferences: {
      ...
      if (core) SharedPreferencesHomeSourceStore.key: 'verifiedCore',
    },
    secrets: {
      'ha_base_url': 'https://a.test',
      'ha_token': 'old-origin',
      if (core) SecureServerSessionStore.key: session().encodeStorage(),
    },
  );
  final snapshot = ha
      ? BackupSnapshot.fromJson({
          ...
          'groups': {
            'connections': {
              'ha': {'baseUrl': 'https://new.test', 'token': 'approved-token'},
            },
          },
        })
      : f.restoreFixture(
          home
              ? {
                  'enabled_services': ['sonarr'],
                }
              : null,
        );
  final prepared = await BackupRepository(storage: storage).prepareRestore(
    snapshot,
    BackupSelection(settings: !ha, dashboard: false, connections: ha),
    conflictPolicy: BackupConflictPolicy.replaceSelected,
    access: core ? CoreAccess() : f.TestRestoreAccess(),
  );
  await f.apply(prepared);
  return storage.durableImages
      .where((v) => v.secrets[journal] != null)
      .toList();
}

MemoryBackupStorage phase(List<MemoryBackupStorage> values, String phase) =>
    values.lastWhere(
      (v) => (jsonDecode(v.secrets[journal]!) as Map)['phase'] == phase,
    );
Future<void> unchanged(MemoryBackupStorage image) async {
  ...
  expect(jsonEncode([image.preferences, image.secrets]), before);
...
class LostInitial extends MemoryBackupStorage {
  ...
  Future<void> writeSecret(String key, String? value) async {
    ...
    if (key == journal &&
        value != null &&
        (jsonDecode(value) as Map)['phase'] == 'applying') {
...
void main() {
  test(
    'Continue with removed own journal cannot reopen under another source',
    () async {
      final storage = LostInitial();
  ...
  test('keep-existing connections retain home witness even if only privacy policy changes', () async {
    final storage = MemoryBackupStorage(
      secrets: {'ha_base_url': 'https://a.test', 'ha_token': 'old-origin'},
    );
    ...
    final prepared = await BackupRepository(storage: storage).prepareRestore(
      ...
      conflictPolicy: BackupConflictPolicy.keepExisting,
      access: f.TestRestoreAccess(),
    );
    await f.apply(prepared);
    expect(storage.secrets['ha_token'], 'old-origin');
    expect(storage.secrets, isNot(contains(journal)));
  });

  for (final state in ['applying', 'committed']) {
    test(
      'boot $state Direct home intent cannot cross HA origin A to B',
      () async {
        final image = phase(await images(), state);
        image.secrets
          ..['ha_base_url'] = 'https://b.test'
          ..['ha_token'] = 'new-origin';
  ...
  test(
    'boot device intent cannot cross selected source Direct to Core',
    () async {
  ...
  for (final change in [
    ...
    test(
      'boot Core device recovery preserves intent after $change session',
      () async {
        ...
        switch (change) {
          case 'missing':
            image.secrets.remove(SecureServerSessionStore.key);
          case 'pending':
            value['authMutationPending'] = true;
          case 'expired':
            value['expiresAt'] = '2020-01-01T00:00:00Z';
          case 'user':
            (value['user'] as Map)['id'] = 'two';
          case 'context':
            (value['context'] as Map)['homeId'] = 'c' * 32;
          case 'token':
            value['accessToken'] = 'z' * 32;
        }
        if (change != 'missing') {
          image.secrets[SecureServerSessionStore.key] = jsonEncode(value);
        }
        await unchanged(image);
      },
    );
  ...
  test('unchanged Core context recovers device data and never reads Direct secrets', () async {
    ...
    expect(
      image.reads.where(
        (v) => v.contains('ha_token') || v.contains('ha_base_url'),
      ),
      isEmpty,
    );
    expect(
      image.secrets[SecureServerSessionStore.key],
      session().encodeStorage(),
    );
  });
  test('Direct device recovery ignores changed HA origin and never reads session or HA', () async {
    final image = phase(await images(home: false), 'applying');
    image.secrets['ha_token'] = 'elsewhere';
    expect(
      await BackupRepository(storage: image).recoverPendingRestore(),
      isTrue,
    );
    expect(
      image.reads.where(
        (v) =>
            v.contains('ha_token') ||
            v.contains('ha_base_url') ||
            v.contains(SecureServerSessionStore.key),
      ),
      isEmpty,
    );
  });
  test(
    'every approved HA tuple prefix crash can recover to complete before',
    () async {
      for (final image in await images(ha: true)) {
        final committed =
            (jsonDecode(image.secrets[journal]!) as Map)['phase'] ==
            'committed';
        expect(
          await BackupRepository(storage: image).recoverPendingRestore(),
          isTrue,
        ...
        expect(
          image.secrets['ha_base_url'],
          committed ? 'https://new.test' : 'https://a.test',
        );
        expect(
          image.secrets['ha_token'],
          committed ? 'approved-token' : 'old-origin',
        );
      }
    },
  );
  test(
    'HA before URL with after token is not an approved forward-write prefix',
    () async {
