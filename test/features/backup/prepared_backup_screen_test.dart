...
class _ScreenHarness {
  ...
  Future<void> mount(WidgetTester tester,{String locale="en",double width=800,double height=1400,double scale=1})async {
    SharedPreferences.setMockInitialValues({});FlutterSecureStorage.setMockInitialValues({});
    tester.view.physicalSize=Size(width,height);tester.view.devicePixelRatio=1;addTearDown(tester.view.reset);
    ...
    ],child:CupertinoApp(locale:Locale(locale),builder:(context,child)=>RepaintBoundary(key:boundary,child:MediaQuery(data:MediaQuery.of(context).copyWith(textScaler:TextScaler.linear(scale)),child:child!)),localizationsDelegates:AppLocalizations.localizationsDelegates,supportedLocales:AppLocalizations.supportedLocales,
    ...
    await reveal(tester,find.text(l10n.backupReplaceSelected));await tester.tap(find.text(l10n.backupReplaceSelected));await flush(tester);
  }
...
Future<void> reveal(WidgetTester tester,Finder finder)async {
  if(finder.evaluate().isEmpty) {
    await tester.scrollUntilVisible(finder,250,scrollable:find.descendant(of:find.byType(BackupScreen),matching:find.byType(Scrollable)).first,maxScrolls:30);
  }
  await tester.ensureVisible(finder);await flush(tester);
}
Future<void> tap(WidgetTester tester,String key)async {
  final finder=find.byKey(ValueKey(key));await reveal(tester,finder);await tester.tap(finder);await flush(tester);
}
void main() {
  ...
  testWidgets('confirmation keyboard Tab and Enter can cancel without restoring', (tester) async {
    final h=_ScreenHarness(); await h.mount(tester); await h.confirm(tester);
  ...
  for(final locale in ['en','tr']) {
    for(final width in [320.0,600.0,1280.0]) {
      testWidgets('prepared confirmation real fonts $locale $width at 2x', (tester)async {
        ...
        final accept=find.widgetWithText(CupertinoDialogAction,l10n.backupApply);
        for(final finder in [cancel,accept]) {
          await tester.ensureVisible(finder);await flush(tester);
          expect(tester.getRect(finder).height,greaterThanOrEqualTo(48));
          final node=tester.getSemantics(finder);
          expect(node.flagsCollection.isButton,isTrue);expect(node.label,isNotEmpty);
        }
        expect(find.descendant(of:dialog,matching:find.text(l10n.backupRestoreTargetDevice)),findsOneWidget);
        expect(h.storage.writes,isEmpty);expect(tester.takeException(),isNull);
        ...
        if(directory.isNotEmpty) {
          await tester.runAsync(()async {
            ...
            try {final data=await image.toByteData(format:ui.ImageByteFormat.png);await File('$directory/restore-$locale-${width.toInt()}-2x.png').writeAsBytes(data!.buffer.asUint8List());} finally {image.dispose();}
          });
        }
