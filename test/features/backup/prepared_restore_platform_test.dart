...
void main() {
  ...
  setUp(() { SharedPreferences.setMockInitialValues({}); FlutterSecureStorage.setMockInitialValues({}); });
  for(final throwing in [false,true]) {
    test('durable preview rejects false optimistic cached value throwing=$throwing', () async {
      final platform=_Platform(); SharedPreferencesStorePlatform.instance=platform;
      final prefs=await SharedPreferences.getInstance();
      platform.reject=true;platform.throwing=throwing;
      try { await prefs.setString('appearance','system'); } catch(_) {}
      expect(prefs.get('appearance'),'system'); // Actual legacy optimistic cache.
      platform.reject=false;platform.throwing=false;
      final repo=BackupRepository(storage:PlatformBackupStorage());
      final prepared=await fixtures.prepare(repo,fixtures.TestRestoreAccess());
  ...
  test('external durable target change is observed before journal write despite cached old value', () async {
    final platform=_Platform();SharedPreferencesStorePlatform.instance=platform;
    final prefs=await SharedPreferences.getInstance();
    final prepared=await fixtures.prepare(BackupRepository(storage:PlatformBackupStorage()),fixtures.TestRestoreAccess());
    ...
    expect(await const FlutterSecureStorage().read(key:'backup_restore_journal_v2'),isNull);
  });
  test('platform read failure is static and never an empty successful target', () async {
    final platform=_Platform();SharedPreferencesStorePlatform.instance=platform;
    await SharedPreferences.getInstance();platform.readFailure=true;
    await expectLater(fixtures.prepare(BackupRepository(storage:PlatformBackupStorage()),fixtures.TestRestoreAccess()),throwsA(isA<BackupException>()));
