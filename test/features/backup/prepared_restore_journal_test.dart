...
class _Faults extends MemoryBackupStorage {
  ...
  Future<void> writeSecret(String key, String? value) async {
    ...
    if (key != _key) return;
    if (value == null && loseDelete ||
        value != null &&
            ((jsonDecode(value) as Map)['phase'] == 'applying' &&
                    loseApplying ||
                (jsonDecode(value) as Map)['phase'] == 'committed' &&
                    loseCommitted)) {
...
Future<void> _run(MemoryBackupStorage storage) async => f.apply(
  await f.prepare(
    BackupRepository(storage: storage),
    f.TestRestoreAccess(),
    f.restoreFixture({'appearance': 'light', 'keep_screen_on': true}),
  ),
);
void main() {
  test(
    'every durable crash image recovers exactly before or committed after',
    () async {
      final storage = _Faults();
      await _run(storage);
      expect(storage.durableImages.length, 5);
      for (final image in storage.durableImages) {
        final raw = image.secrets[_key];
        final phase = raw == null
            ? 'complete'
            : (jsonDecode(raw) as Map)['phase'];
        final result = await BackupRepository(storage: image)
            .recoverPendingRestore();
        expect(result, raw != null);
        expect(
          image.preferences,
          phase == 'applying'
              ? {'appearance': 'dark', 'keep_screen_on': false}
              : {'appearance': 'light', 'keep_screen_on': true},
        );
  ...
  for (final phase in ['committed', 'delete']) {
    test(
      'lost $phase acknowledgement uses durable phase without double effects',
      () async {
        final storage = _Faults()
          ..loseCommitted = phase == 'committed'
          ..loseDelete = phase == 'delete';
        await _run(storage);
        expect(storage.preferences, {
          'appearance': 'light',
          'keep_screen_on': true,
        });
        expect(storage.writes.where((v) => v.startsWith('pref:')), [
          'pref:appearance',
          'pref:keep_screen_on',
        ]);
  ...
  test(
    'lost initial journal ACK leaves zero field effects and recoverable intent',
    () async {
  ...
  test('foreign third value preserves journal and every target', () async {
    final initial = _Faults();
    await _run(initial);
    final image = initial.durableImages.firstWhere(
      (i) => i.preferences['keep_screen_on'] == true && i.secrets[_key] != null,
    );
  ...
  for (final malformed in [
    ...
    test(
      'malformed journal is preserved and never exposes raw content $malformed',
      () async {
        ...
        await expectLater(
          ...
          throwsA(
            isA<BackupException>().having(
              (e) => e.toString(),
              'static',
              isNot(contains('private broken')),
            ),
          ),
        );
        expect(storage.secrets[_key], malformed);
  ...
  test('concurrent legacy and prepared journals require explicit unresolved recovery', () async {
    ...
    final before = Map.of(storage.secrets);
    storage.writes.clear();
    await expectLater(
      BackupRepository(storage: storage).recoverPendingRestore(),
      throwsA(isA<BackupException>()),
    );
    expect(storage.secrets, before);
    expect(storage.writes, isEmpty);
  });
}
