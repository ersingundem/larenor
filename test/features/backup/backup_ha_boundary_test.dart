...
BackupSnapshot _snapshot({bool connections = true}) =>
...
void main() {
  ...
  for (final value in ['1', '', 'false', 'unknown-private-marker']) {
    test('capture refuses pending marker ${value.length} without reading pair', () async {
  ...
  for (final operation in ['preview', 'keep', 'replace']) {
    test('$operation refuses pending even when imported HA pair is complete', () async {
  ...
  test('all selected connections stop even if incoming file has only another service', () async {
    ...
    final backup = BackupSnapshot.fromJson({
      ...
      'groups': {
        'connections': {
          'sonarr': {'baseUrl': 'https://arr.example.test', 'apiKey': 'synthetic'},
        },
      },
    });
    final repository = BackupRepository(storage: storage);
  ...
  for (final failure in [
    ...
    test('marker ${failure.runtimeType} fails closed and remains static', () async {
      ...
      await expectLater(repository.restore(_snapshot(), _connections), _pending);
      expect(storage.writes, isEmpty);
    });
  }
  ...
  test('settings and dashboard only do not inspect or reset pending connection', () async {
    ...
    expect((await repository.preview(_snapshot(connections: false))).hasConnections, isFalse);
  ...
  test('capture rejects marker appearing during pair read before publishing snapshot', () async {
    ...
    await expectLater(BackupRepository(storage: storage).capture(_connections), _pending);
    expect(storage.writes, isEmpty);
  });

  test('preview rejects marker appearing during final dashboard read', () async {
    final storage = _Storage(secrets: _oldPair);
    storage.afterRead = (key) async {
      if (key == 'dashboard_layout') storage.secrets[_marker] = '1';
    };
    await expectLater(BackupRepository(storage: storage).preview(_snapshot()), _pending);
    expect(storage.writes, isEmpty);
  });

  test('restore cannot journal pair when marker appears during capture', () async {
    final storage = _Storage(secrets: _oldPair);
    storage.afterRead = (key) async {
      if (key == 'ha_token') storage.secrets[_marker] = '1';
    };
  ...
  test('capture joins pending credential mutation before checking marker', () async {
    final storage = _Storage(secrets: _oldPair);
    ...
    final mutation = ConfigurationWrites.run(() async {
      ...
      await storage.writeSecret('ha_base_url', 'https://partial.example.test');
    });
  ...
  test('marker arriving with durable journal triggers bounded rollback, not commit', () async {
    ...
    storage.afterWrite = (key) async {
      if (key == BackupRepository.restoreJournalKey) storage.secrets[_marker] = '1';
    };
    await expectLater(
      BackupRepository(storage: storage).restore(
        _snapshot(), _connections, conflictPolicy: BackupConflictPolicy.replaceSelected,
      ),
      _pending,
    );
    expect(storage.secrets, {..._oldPair, _marker: '1'});
    expect(storage.writes, isNot(contains('secret:$_marker')));
    expect(storage.writes.first, 'secret:${BackupRepository.restoreJournalKey}');
    expect(storage.writes.last, 'secret:${BackupRepository.restoreJournalKey}');
  });

  test('existing restore journal recovers without reading or resetting HA marker', () async {
    final storage = _Storage(secrets: _oldPair);
    ...
    for (final image in interrupted) {
      ...
      expect(await BackupRepository(storage: pending).recoverPendingRestore(), isTrue);
      expect(pending.secrets, {..._oldPair, _marker: '1'});
      expect(pending.writes, isNot(contains('secret:$_marker')));
      expect(await BackupRepository(storage: pending).recoverPendingRestore(), isFalse);
      await expectLater(BackupRepository(storage: pending).capture(_connections), _pending);
    }
  });
  ...
  test('failed recovery retains both journals and later retry never clears HA marker', () async {
    ...
    final storage = _Storage(secrets: {
      _marker: '1', BackupRepository.restoreJournalKey: journal, 'ha_token': 'partial',
    })..failWrites.add(1);
    final repository = BackupRepository(storage: storage);
    await expectLater(repository.recoverPendingRestore(), throwsA(isA<BackupRestoreException>()));
  ...
  test('marker is neither backup data nor an allowed rollback target', () async {
    expect(backupPreferenceKeys, isNot(contains(_marker)));
    expect(backupConnectionFields.values.expand((fields) => fields.values), isNot(contains(_marker)));
    final storage = _Storage(secrets: {
      _marker: '1',
      BackupRepository.restoreJournalKey: jsonEncode({
        'version': 1,
        'changes': [{'secret': true, 'key': _marker, 'before': null}],
      }),
    });
    await expectLater(BackupRepository(storage: storage).recoverPendingRestore(), throwsA(isA<BackupException>()));
    expect(storage.writes, isEmpty);
    expect(storage.secrets[_marker], '1');
    expect(() => BackupSnapshot.fromJson({
      'version': 1, 'createdAt': '2026-09-06T00:00:00.000Z',
      'groups': {'connections': {'ha': {..._incoming['ha']!, _marker: '1'}}},
    }), throwsA(isA<BackupValidationException>()));
  });

  test('actual platform capture refuses marked pair and preserves marker', () async {
