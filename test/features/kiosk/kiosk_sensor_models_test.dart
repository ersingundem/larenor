...
void main() {
  ...
  test('controller rejects foreign/out-of-order reads and verifies stop', () async {
    final api = _Api();
    final controller = KioskSensorController(api);
    await controller.start();
    expect((await controller.refresh()).sequence, 1);
    api.readValue = _sample(sequence: 0);
    await expectLater(controller.refresh(), throwsA(isA<KioskSensorException>()));
    api.readValue = _sample(sessionId: '00000000-0000-0000-0000-000000000000');
    await expectLater(controller.refresh(), throwsA(isA<KioskSensorException>()));
    api.stopValue = {'version': 1, 'sessionId': _session, 'stopped': false};
    await expectLater(controller.stop(), throwsA(isA<KioskSensorException>()));
  ...
  test('Android channel uses exact bounded requests and redacts failures', () async {
    ...
    expect(calls.map((e) => e.method), ['sensorStart', 'sensorRead', 'sensorStop']);
