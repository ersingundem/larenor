...
void main() {
  for (final locale in const [Locale('en'), Locale('tr')]) {
    for (final width in const [600.0, 1280.0]) {
      testWidgets('${locale.languageCode} $width 2x has keyboard-reachable 48dp actions', (tester) async {
        var exits = 0;
        await _mount(tester, locale: locale, width: width, exit: () async => exits++);
        for (final key in const ['kiosk-quick-home', 'kiosk-quick-settings', 'kiosk-quick-exit']) {
  ...
  testWidgets('pending exit is single-flight and disables every stale action', (tester) async {
    final gate = Completer<void>();
    var exits = 0;
    await _mount(tester, locale: const Locale('en'), width: 600, exit: () {
      exits++;
      return gate.future;
    });
    final exit = find.byKey(const ValueKey('kiosk-quick-exit'));
