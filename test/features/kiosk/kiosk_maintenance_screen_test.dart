...
void main() {
  for (final locale in const [Locale('en'), Locale('tr')]) {
    for (final width in const [600.0, 1280.0]) {
      testWidgets(
        '${locale.languageCode} $width 2x exposes safe maintenance and CSV preview',
        (tester) async {
          ...
          expect(
            find.byKey(const ValueKey('kiosk-maintenance-csv')),
            findsOneWidget,
          );
          expect(find.textContaining('2026-09-21,1,0,1,0,0'), findsOneWidget);
          final refresh = find.byKey(
            const ValueKey('kiosk-maintenance-refresh'),
          );
  ...
  testWidgets('corrupt local journal shows error without stale counts', (
    tester,
  ) async {
