...
DashboardRepository repository(WidgetTester tester, ScopeHarness h) {
  final subscription = h
      .runtime(tester)
      .listen(dashboardRepositoryProvider, (_, _) {});
  addTearDown(subscription.close);
  return subscription.read();
}

class CopyPlatform extends InMemorySharedPreferencesStore {
  CopyPlatform(Map<String, Object> values)
    : super.withData({
  ...
  Future<bool> setValue(String valueType, String key, Object value) async {
    if (!key.startsWith('flutter.dashboard_layout_core_v1_')) {
...
void main() {
  ...
  for (final language in ['en', 'tr']) {
    for (final width in [600.0, 1200.0]) {
      testWidgets(
        'actual PIN preview and selected copy $language $width at 2x',
        (tester) async {
          final h = ScopeHarness(HomeSource.verifiedCore);
          await h.mount(
            tester,
  ...
  for (final change in [
    ...
    testWidgets('captured confirmation cannot copy after $change', (
      tester,
    ) async {
      final h = ScopeHarness(HomeSource.verifiedCore);
      await h.mount(
      ...
      switch (change) {
        ...
        case 'pending':
          ...
          h.now = h.account.session!.expiresAt.subtract(
            const Duration(seconds: 20),
          );
          pending = h.account.ensureSession().then<void>((_) {});
          await flush(tester);
      ...
      expect(
        prefs.getString('dashboard_layout'),
        jsonEncode(localLayout.toJson()),
      );
      if (pending != null) {
  ...
  testWidgets(
    'refresh intent without working=true closes old access, same tuple later keeps key',
    (tester) async {
      final h = ScopeHarness(HomeSource.verifiedCore);
      await h.mount(tester, pin: '1234');
      await h.signIn();
  ...
  testWidgets(
    'saved scope survives app remount while other Core home stays empty',
    (tester) async {
      final h = ScopeHarness(HomeSource.verifiedCore);
      await h.mount(
        tester,
      ...
      await flush(tester);
      final restored = ScopeHarness(HomeSource.verifiedCore)
        ..store.value = h.store.value;
      await restored.mount(tester, pin: '1234', preferences: persisted);
      expect(restored.api.meReads, 1);
      expect(restored.api.contextReads, 1);
      expect(
        (await repository(tester, restored).load()).rooms.map((r) => r.name),
        ['Kitchen'],
      );
      restored.api.homeId = 'c' * 32;
      restored.now = restored.account.session!.expiresAt.subtract(
        const Duration(seconds: 20),
      );
      final refreshing = restored.account.ensureSession();
      restored.now = DateTime.now();
      await refreshing;
      await flush(tester);
      ...
      expect(
        ...
        persisted[scopeA.storageKey],
      );
      expect(restored.connectionReads, 0);
    },
  );
  testWidgets('Direct repository stays legacy when a Server account signs in', (
    tester,
  ) async {
    ...
    expect(await before.load(), localLayout);
    expect(
      (await SharedPreferences.getInstance()).get(scopeA.storageKey),
      isNull,
    );
  });
  testWidgets(
    'keyboard selection, cancel and hidden route invalidate captured action',
    (tester) async {
      final h = ScopeHarness(HomeSource.verifiedCore);
      await h.mount(
        tester,
      ...
      navigator.push(
        CupertinoPageRoute<void>(
          builder: (_) =>
              const CupertinoPageScaffold(child: Text('Other local page')),
        ),
      );
      await flush(tester);
      old();
  ...
  for (final commit in [false, true]) {
    for (final throwing in [false, true]) {
      testWidgets(
        'uncertain copy $commit/$throwing never confirms or automatically retries',
        (tester) async {
          final h = ScopeHarness(HomeSource.verifiedCore);
          await h.mount(
            tester,
          ...
          final values = {
            for (final key in initial.getKeys()) key: initial.get(key)!,
          };
          SharedPreferences.resetStatic();
          final platform = CopyPlatform(values)
            ..commit = commit
            ..throwing = throwing;
          ...
          final current = find.byKey(
            const ValueKey('home-layout-current-room-0'),
          );
  ...
  for (final phase in ['signed-out', 'first-password', 'context404']) {
    testWidgets('Core $phase never makes legacy repository available', (
      tester,
    ) async {
      final h = ScopeHarness(HomeSource.verifiedCore);
      await h.mount(
      ...
      if (phase == 'first-password') expect(h.api.contextReads, 0);
      if (phase == 'context404') expect(h.account.hasPendingContext, isTrue);
    });
  }
}
