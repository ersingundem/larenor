...
void main() {
  ...
  setUpAll(() async {
    fixtureKey = await Pbkdf2.hmacSha256(
      iterations: 600000,
      bits: 256,
  ...
  test(
    'independent fixed-primitive fixture decrypts only to the exact model',
    () async {
  ...
  test(
    'Unicode portable roundtrip conceals room metadata without device keys',
    () async {
      final result = await codec.encrypt(archive(), passphrase);
      expect(
        result.length,
        lessThanOrEqualTo(CoreLayoutArchiveCodec.maxFileBytes),
      );
      ...
      expect(
        (await codec.decrypt(result, passphrase)).toJson(),
        archive().toJson(),
      );
    },
  );
  ...
  test(
    'wrong passphrase and visually similar Unicode are not normalized',
    () async {
      await expectLater(
        codec.decrypt(encrypted, 'Different long archive phrase'),
        fails('decrypt_failed'),
      );
      final composed = 'özel arşiv şifresi é';
      final result = await codec.encrypt(archive(), composed);
      await expectLater(
        codec.decrypt(result, 'özel arşiv şifresi e\u0301'),
        fails('decrypt_failed'),
      );
    },
  );
  ...
  for (final field in ['salt', 'nonce', 'ciphertext', 'tag']) {
    test('authenticated $field change is rejected', () async {
      ...
      final owner = field == 'salt'
          ? changed['kdf'] as Map
          : field == 'nonce'
          ? changed['cipher'] as Map
          : changed;
      final value = base64Decode(owner[field] as String);
      value[0] ^= 1;
      owner[field] = base64Encode(value);
      await expectLater(
        codec.decrypt(bytes(changed), passphrase),
        fails('decrypt_failed'),
      );
    });
  }
  ...
  test(
    'the format discriminator is AAD, not just an unauthenticated label',
    () async {
      final foreign = await seal(
        fixtureKey,
        utf8.encode(archive().encode()),
        aad: header(format: 'larenor-vault'),
      );
      final relabelled = envelope(foreign)
        ..['format'] = 'larenor-core-layout-archive';
      await expectLater(
        codec.decrypt(bytes(relabelled), passphrase),
        fails('decrypt_failed'),
      );
    },
  );
  ...
  test(
    'legacy vault and Core archive reject each other without format fallback',
    () async {
      ...
      await expectLater(
        codec.decrypt(oldFile, passphrase),
        fails('invalid_archive'),
      );
      await expectLater(
        const BackupCodec().decrypt(encrypted, passphrase),
  ...
  final invalidHeader = <String, void Function(Map<String, dynamic>)>{
    ...
    'long nonce': (v) =>
        v['cipher']['nonce'] = base64Encode(List.filled(13, 1)),
    ...
    'unpadded salt': (v) =>
        v['kdf']['salt'] = (v['kdf']['salt'] as String).replaceAll('=', ''),
    'base64url nonce': (v) =>
        v['cipher']['nonce'] = base64Encode(List.filled(12, 255))
            .replaceAll('/', '_'),
    'percent escaped salt': (v) =>
        v['kdf']['salt'] = (v['kdf']['salt'] as String).replaceAll('=', '%3D'),
    'noncanonical pad bits': (v) => v['tag'] = 'AAAAAAAAAAAAAAAAAAAAAB==',
    'empty ciphertext': (v) => v['ciphertext'] = '',
    'oversized ciphertext': (v) => v['ciphertext'] = base64Encode(
      Uint8List(maxCoreLayoutArchiveBytes + 1),
    ),
  };
  for (final change in invalidHeader.entries) {
    test('closed pre-KDF header rejects ${change.key}', () async {
      ...
      change.value(value);
      await expectLater(
        codec.decrypt(bytes(value), passphrase),
        fails('invalid_archive'),
      );
    });
  }
  ...
  test(
    'JSON key order and whitespace do not alter canonical authenticated header',
    () async {
      final value = envelope(encrypted);
      final reordered = {
      ...
      final input = Uint8List.fromList(
        utf8.encode(const JsonEncoder.withIndent(' ').convert(reordered)),
      );
      expect(
        (await codec.decrypt(input, passphrase)).toJson(),
        archive().toJson(),
      );
    },
  );
  ...
  test('exact 3MiB file is accepted and one extra byte is rejected', () async {
    final input = Uint8List(CoreLayoutArchiveCodec.maxFileBytes)
      ..fillRange(0, CoreLayoutArchiveCodec.maxFileBytes, 32);
    ...
    await expectLater(
      codec.decrypt(
        Uint8List(CoreLayoutArchiveCodec.maxFileBytes + 1),
        passphrase,
      ),
      fails('archive_too_large'),
    );
  });
  ...
  test('authenticated 2MiB plaintext boundary returns the model without hidden truncation', () async {
    ...
    final padded = Uint8List(maxCoreLayoutArchiveBytes)
      ..fillRange(0, maxCoreLayoutArchiveBytes, 32);
  ...
  for (final entry in badInner.entries) {
    test('authenticated plaintext still rejects ${entry.key}', () async {
      final input = await seal(
        fixtureKey,
        utf8.encode(jsonEncode(entry.value)),
      );
      await expectLater(
        codec.decrypt(input, passphrase),
        fails('invalid_archive'),
      );
    });
  }
  ...
  test(
    'invalid, non-UTF8, truncated and nonobject files fail statically',
    () async {
      for (final value in [
        Uint8List(0),
        ...
        await expectLater(
          codec.decrypt(value, passphrase),
          fails('invalid_archive'),
        );
      }
    },
  ...
  test(
    'passphrase policy preserves exact Unicode and enforces rune/UTF8 limits',
    () {
      for (final value in [
        '',
        'short',
        ...
        expect(
          () => CoreLayoutArchiveCodec.validatePassphrase(value),
      ...
      expect(
        () => CoreLayoutArchiveCodec.validatePassphrase(
          '123456789012',
  ...
  test(
    'public operations both validate passphrase without any fallback',
    () async {
      await expectLater(
        codec.encrypt(archive(), 'short'),
        fails('invalid_passphrase'),
      );
      await expectLater(
        codec.decrypt(encrypted, 'short'),
        fails('invalid_passphrase'),
      );
    },
  );
