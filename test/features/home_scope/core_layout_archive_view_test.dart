...
import 'core_layout_archive_screen_test.dart'
    show archive, scope, passphrase, importArchive;

class _View extends TestFlutterView {
  _View(TestFlutterView source)
    : super(
        view: source,
        platformDispatcher: source.platformDispatcher,
...
void main() {
  testWidgets(
    'same provider and navigator moved to a different native View cannot reuse confirmation',
    (tester) async {
      ...
      Widget app() => View(
        ...
        child: UncontrolledProviderScope(
          ...
          child: AppInteractionScope(
            controller: h.home.interaction,
            child: CupertinoApp(
              key: appKey,
              localizationsDelegates: AppLocalizations.localizationsDelegates,
              supportedLocales: AppLocalizations.supportedLocales,
              home: CoreLayoutArchiveScreen(
                key: screenKey,
                gateCurrent: () => true,
                runFileDialog: <T>(operation) => operation(),
              ),
            ),
          ),
        ),
      );
      await tester.pumpWidget(app(), wrapWithView: false);
      await flush(tester);
      await importArchive(tester, h, bytes!);
      ...
      await tester.pumpWidget(app(), wrapWithView: false);
      await flush(tester);
      expect(screenKey.currentState, same(state));
      expect(View.of(screenKey.currentContext!).viewId, 100);
      held();
      await flush(tester);
