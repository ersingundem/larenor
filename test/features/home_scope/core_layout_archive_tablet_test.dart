...
void main() {
  ...
  for (final language in ['en', 'tr']) {
    for (final width in [320.0, 600.0, 1280.0]) {
      testWidgets(
        'Core archive $language $width 2x has named 48px fields, keyboard and isolated confirmation',
        (tester) async {
          await loadFonts(tester);
          final semantics = tester.ensureSemantics();
          try {
            tester.platformDispatcher.platformBrightnessTestValue =
                width == 1280 ? Brightness.dark : Brightness.light;
            addTearDown(
            ...
            final text = find.descendant(
              of: pick,
              matching: find.text(l10n.coreLayoutArchiveImport),
            );
            final node = tester.getSemantics(text);
            expect(node.label, l10n.coreLayoutArchiveImport);
            ...
            await flush(tester);
            expect(
              Focus.of(tester.element(find.text(l10n.coreLayoutArchiveRefresh)))
                  .hasPrimaryFocus,
              isTrue,
            );
            await tester.sendKeyEvent(LogicalKeyboardKey.tab);
            ...
            final background = CupertinoDynamicColor.resolve(
              CupertinoTheme.of(tester.element(pick)).scaffoldBackgroundColor,
              tester.element(pick),
            );
            ...
            final confirm = find.byKey(
              const ValueKey('core-layout-archive-confirm'),
            );
            expect(tester.getSize(confirm).height, greaterThanOrEqualTo(48));
            final root = h.boundary.currentContext!
                .findRenderObject()!
                .owner!
                .semanticsOwner!
                .rootSemanticsNode!;
            ...
            expect(
              labels.any(
                (label) => label.contains(l10n.coreLayoutArchiveConfirm),
              ),
              isTrue,
            );
            expect(
              labels.any(
                (label) => label.contains(l10n.coreLayoutArchivePasswordHint),
              ),
              isFalse,
            );
            await capture(
              tester,
              h,
              'archive-confirm-$language-${width.toInt()}-2x',
            );
            ...
            final password = find.byKey(
              const ValueKey('core-layout-archive-password'),
            );
            ...
            expect(
              tester.getSemantics(password).label,
              contains(l10n.coreLayoutArchivePassword),
            );
            await tester.enterText(password, 'synthetic new password');
            await tester.sendKeyEvent(LogicalKeyboardKey.tab);
            await flush(tester);
            final repeated = tester.widget<EditableText>(
              find.descendant(
                of: find.byKey(const ValueKey('core-layout-archive-repeat')),
            ...
            await capture(
              tester,
              h,
              'archive-form-$language-${width.toInt()}-2x',
            );
