...
class ArchiveFiles extends CoreLayoutArchiveFileAccess {
  ...
  Future<Uint8List?> pick() async {
    picks++;
    return onPick == null ? input : await onPick!();
  }
  ...
  Future<Uri?> save(Uint8List bytes) async {
    ...
    return onSave == null
        ? Uri.parse('content://synthetic/export')
        : await onSave!();
  }
}
...
class ArchiveHarness {
  ...
  Future<void> mount(
    ...
    SharedPreferences.setMockInitialValues({
      'dashboard_layout': 'legacy-private',
      'unrelated': 'unchanged',
    });
    ...
    await session.account.initialize();
  ...
  DashboardRepository get repository =>
...
Future<void> archivePress(WidgetTester tester, String key) async {
  await archiveVisible(tester, key);
