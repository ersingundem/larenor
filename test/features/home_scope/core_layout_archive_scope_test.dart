...
void main() {
  testWidgets(
    'same home alive in another ProviderContainer cannot reuse archive State or callbacks',
    (tester) async {
      ...
      final held = tester
          .widget<CupertinoButton>(
            find.byKey(const ValueKey('core-layout-archive-pick')),
          )
          .onPressed!;
      rebuild(() => current = second);
      await flush(tester);
      expect(key.currentState, same(oldState));
      held();
      await flush(tester);
      expect(h.files.picks, 0);
      expect(otherFiles.picks, 0);
      expect(find.text('sensitive draft'), findsNothing);
      expect(
        find.byKey(const ValueKey('core-layout-archive-pick')),
        findsNothing,
      );
      expect(h.home.account.session, isNotNull);
      expect(h.home.interaction.active, isTrue);
