...
void main() {
  ...
  test(
    'legacy absence selects direct local without writing or clearing',
    () async {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('unrelated', 'preserved');
      expect(
        await SharedPreferencesHomeSourceStore().read(),
  ...
  for (final raw in <Object>[
    ...
    test('malformed source $raw fails closed without overwriting it', () async {
      SharedPreferences.setMockInitialValues({
        SharedPreferencesHomeSourceStore.key: raw,
      });
      await expectLater(
        SharedPreferencesHomeSourceStore().read(),
        throwsA(failure('source_read_failed')),
      );
      expect(
  ...
  test(
    'preference acquisition and get errors have static read failures',
    () async {
  ...
  test(
    'a rejected setString is a failure and never clears stored selection',
    () async {
      final prefs = _Preferences()..accept = false;
      prefs.values[SharedPreferencesHomeSourceStore.key] =
          HomeSource.directLocal.name;
      final store = SharedPreferencesHomeSourceStore(
        loadPreferences: () async => prefs,
      );
      await expectLater(
        store.write(HomeSource.verifiedCore),
        throwsA(failure('source_write_failed')),
      );
      expect(prefs.values, {
  ...
  test('write and acquisition exceptions are static and do not poison future writes', () async {
    ...
    await expectLater(
      inaccessible.write(HomeSource.verifiedCore),
      throwsA(failure('source_write_failed')),
    );
    final prefs = _Preferences()
      ..writeError = StateError('private write details');
    final store = SharedPreferencesHomeSourceStore(
      loadPreferences: () async => prefs,
    );
    await expectLater(
  ...
  for (final throwing in [false, true]) {
    test(
      'real preferences failed write ($throwing) cannot make its cache authoritative',
      () async {
        ...
        expect(
          (await platform.getAll())['flutter.home_source_v1'],
          'directLocal',
        );
        expect(prefs.getString('unrelated'), 'preserved');
  ...
  test(
    'a reload failure rejects the old cached source instead of trusting it',
    () async {
      SharedPreferences.resetStatic();
  ...
  test(
    'source write waits for the existing whole configuration transaction',
    () async {
  ...
  test(
    'concurrent stores preserve enqueue order through preference loading',
    () async {
      final entered = Completer<void>();
      final release = Completer<void>();
      addTearDown(() {
      ...
      final prefs = _Preferences();
      final first = SharedPreferencesHomeSourceStore(
        loadPreferences: () async {
          entered.complete();
          await release.future;
          return prefs;
      ...
      final second = SharedPreferencesHomeSourceStore(
        loadPreferences: () async => prefs,
      );
  ...
  test('actual backup capture and replacement restore preserve local source choice', () async {
    ...
    expect(
      ...
      isNot(contains(SharedPreferencesHomeSourceStore.key)),
    );
  });
}
