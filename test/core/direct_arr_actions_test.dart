...
typedef SignIn = Future<void> Function({
...
Future<void> Function() arrSignOut(ProviderContainer c, String name) =>
    switch (name) {
...
void main() {
  ...
  for (final name in arrServices) {
    test(
      '$name cold Core signIn must reject before creating network client',
      () async {
    ...
    test(
      '$name late login after Direct Core Direct cannot reacquire a fresh store',
      () async {
    ...
    test(
      '$name old signOut after Direct Core Direct cannot clear saved credentials',
      () async {
        final (c, home) = await routinesHome('direct');
        final sub = holdArr(c, name);
        addTearDown(sub.close);
    ...
    test(
      '$name concurrent login is rejected and explicit logout wins over the old HTTP response',
      () async {
        ...
        await http.runWithClient(
          () async {
            ...
            addTearDown(sub.close);
            await arrConnection(c, name);
            final pending = arrSignIn(c, name)(
              baseUrl: 'https://new.invalid',
              apiKey: 'new',
            );
            final rejected = expectLater(
            ...
            await arrSignOut(c, name)();
            expect(await arrConnection(c, name), isNull);
            response.complete(http.Response('{}', 200));
            await rejected;
    ...
    test(
      '$name current rejected credentials close check transport and never mutate storage',
      () async {
        var closed = 0;
        await http.runWithClient(
          () async {
            ...
            secure.calls.clear();
            await expectLater(
              arrSignIn(c, name)(baseUrl: 'https://new.invalid', apiKey: 'new'),
              throwsA(isA<Exception>()),
            );
            expect(secure.calls, isEmpty);
            expect(closed, 1);
            expect((await arrConnection(c, name))!.apiKey, 'synthetic-old-key');
          },
          () => TrackedArrHttp(
            (_) async => http.Response('private-error-body', 401),
            () => closed++,
          ),
        );
      },
    );
    test('$name disposed notifier cannot start a check or clear', () async {
      final (c, _) = await routinesHome('direct');
      final sub = holdArr(c, name);
      await arrConnection(c, name);
      final login = arrSignIn(c, name), logout = arrSignOut(c, name);
      sub.close();
      c.dispose();
      secure.calls.clear();
      var clients = 0;
      await http.runWithClient(
        () async {
          await expectLater(
            login(baseUrl: 'https://new.invalid', apiKey: 'new'),
            throwsA(isA<DirectHomeAccessException>()),
          );
          await expectLater(
            logout(),
    ...
    test(
      '$name successful current login closes its one-use check transport',
      () async {
        var closed = 0;
        await http.runWithClient(
          () async {
            ...
            addTearDown(sub.close);
            await arrConnection(c, name);
            await arrSignIn(c, name)(
              baseUrl: 'https://new.invalid',
              apiKey: 'synthetic-new',
            );
            expect(
              (await arrConnection(c, name))!.baseUrl,
              'https://new.invalid',
            );
            expect(closed, 1);
          },
