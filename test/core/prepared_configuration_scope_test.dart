...
class _Harness {
  ...
  Future<void> mount(WidgetTester tester) async {
    ...
    final config=Provider<String>((ref) {
      created++;
      ref.onDispose(() {disposed++;access.live=false;});
      return storage.preferences['appearance'] as String;
    });
    await tester.pumpWidget(ConfigurationScope(child:CupertinoApp(home:Consumer(builder:(context,ref,_)=>Column(children:[
      Text(ref.watch(config)),CupertinoButton(child:const Text('Restore'),onPressed:()async {
        try { await ConfigurationScope.restorePrepared(context,prepared:prepared,progressLabel:'Restoring',failureLabel:'Recovery required',continueLabel:'Retry recovery'); }catch(error){errors.add(error);}
      }),
    ])))));
  }
}
void main() {
  testWidgets('prepared handoff owns legitimate UI disposal before persistence and fresh provider reload', (tester) async {
    final h=_Harness(_Storage());await h.mount(tester);
    await tester.tap(find.text('Restore'));await tester.pumpAndSettle();
    expect(h.errors,isEmpty);expect(h.disposed,1);expect(h.created,2);
    expect(h.storage.preferences['appearance'],'light');
    expect(find.text('light'),findsOneWidget);
    expect(h.storage.secrets,isEmpty);
  });
  testWidgets('queued stale approval never disposes providers or writes an intent', (tester) async {
    final h=_Harness(_Storage());await h.mount(tester);
    final finish=Completer<void>();
    final queued=ConfigurationWrites.run(()=>finish.future);
    await tester.tap(find.text('Restore'));await tester.pump();
    expect(h.disposed,0);h.access.live=false;finish.complete();await queued;await tester.pumpAndSettle();
    expect(h.disposed,0);expect(h.storage.writes,isEmpty);
    expect(h.errors,hasLength(1));expect(h.errors.single,isA<BackupException>());
  });
  testWidgets('uncertain initial ACK cannot reopen providers until exact recovery succeeds', (tester) async {
    final h=_Harness(_Storage());await h.mount(tester);h.storage.failInitialAck=true;
    await tester.tap(find.text('Restore'));await tester.pumpAndSettle();
    expect(find.text('Recovery required'),findsOneWidget);expect(h.created,1);expect(h.disposed,1);
