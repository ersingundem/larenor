...
ArrCredentialsStore arrStore(ProviderContainer c, String name) => switch (name) {
...
Future<ArrConfig?> arrConnection(ProviderContainer c, String name) => switch (name) {
...
ProviderSubscription<dynamic> holdArr(ProviderContainer c, String name) => switch (name) {
...
void main() {
  ...
  for (final name in arrServices) {
    for (final mode in ['core', 'pending', 'error']) {
      test('$name $mode actual provider never reads Direct credentials', () async {
    ...
    test('$name held Core store cannot read write or clear', () async {
      ...
      for (final operation in <Future<void> Function()>[
        ...
        () => store.save(baseUrl:'https://new.invalid', apiKey:'synthetic-new-key'),
        store.clear,
      ]) {
    ...
    test('$name standalone complete tuple retains public constructor behavior', () async {
      final store = ArrCredentialsStore(servicePrefix:name);
      expect((await store.read())!.baseUrl, 'https://old.invalid');
      await store.save(baseUrl:'https://new.invalid', apiKey:'synthetic-new-key');
      final saved = await store.read();
      expect(saved!.baseUrl, 'https://new.invalid');
      expect(saved.apiKey, 'synthetic-new-key');
      await store.clear(); expect(await store.read(), isNull);
    });
    test('$name pending marker blocks mixed tuple before any credential read', () async {
      secure.values['${name}_base_url'] = 'https://new.invalid';
      secure.values['${name}_connection_pending_v1'] = '1';
      await expectLater(ArrCredentialsStore(servicePrefix:name).read(), throwsA(
        isA<DirectHomeAccessException>().having((e)=>e.code,'code','pending_mutation')));
      expect(secure.calls, [('read','${name}_connection_pending_v1')]);
    });
  }
  test('unknown Arr prefix cannot select arbitrary secure-storage keys', () async {
    for (final prefix in ['other','ha','sonarr/../radarr','Sonarr','']) {
      await expectLater(Future.sync(() async {
