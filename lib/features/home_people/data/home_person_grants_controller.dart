...
class HomePersonGrantsController extends ChangeNotifier {
  ...
  Future<void> setPermission(
    AdminUser selected,
    HomePersonPermission permission, {
  ...
  Future<void> _run(
    ...
    try {
      await home!.account.withSession((_, session) async {
        ...
        try {
          if (writing) {
            ...
            check();
            if (nextUsers!.any(
              (user) => !HomePersonGrants.isSubjectId(user.id),
            )) {
              throw const LarenorServerException('invalid_response');
            }
            result = await api.grants(target);
            check();
      ...
      if (writing) {
        outcome = permission == HomePersonPermission.none
            ? HomePersonGrantOutcome.revoked
            : HomePersonGrantOutcome.saved;
      }
    } catch (error) {
      if (current()) {
        _clear();
        ...
        if (writing) {
          outcome = switch (failure) {
            'revision_conflict' ||
            'conflict' => HomePersonGrantOutcome.conflict,
            'invalid_request' ||
            'forbidden' ||
            'not_found' => HomePersonGrantOutcome.failed,
            _ => HomePersonGrantOutcome.uncertain,
          };
        }
      }
    } finally {
