...
import '../data/power_budget_api.dart';
import '../data/power_budget_controller.dart';
import 'power_budget_screen.dart';

const _routeId = '48484848484848484848484848484848';

class PowerBudgetRoute extends ConsumerStatefulWidget {
  const PowerBudgetRoute({super.key});
  @override
  ConsumerState<PowerBudgetRoute> createState() => _PowerBudgetRouteState();
}

class _PowerBudgetRouteState extends ConsumerState<PowerBudgetRoute> {
  ServerAccountController? _account;
  AppInteractionController? _interaction;
  CorePowerBudgetApi? _api;
  PowerBudgetController? _controller;
  int _generation = 0;
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller case final controller?) {
      return PowerBudgetScreen(controller: controller);
    }
          ...
          child: SettingsSection(
            ...
            children: [
              if (_loading)
                ...
                SettingsActionTile(
                  buttonKey: const ValueKey('power-budget-reconnect'),
                  leading: const Icon(CupertinoIcons.refresh),
                  title: Text(l10n.powerBudgetRefresh),
                  additionalInfo: Text(l10n.powerBudgetFailed),
