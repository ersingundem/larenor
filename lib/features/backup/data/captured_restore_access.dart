...
Never _unavailable() => throw const BackupException('restore_expired','Read the restore preview again.');
String _fingerprint(String value)=>sha256.convert(utf8.encode(value)).toString();
...
abstract final class CapturedRestoreAccess {
  static Future<BackupRestoreAccess> capture({
    ...
    try {
      ...
      if(!isCurrent() || source==null || home!=null && (home.busy || home.failure!=null || !home.interaction.active)) _unavailable();
      final layout=source==HomeSource.verifiedCore ? homeLayoutAccess(home,clock:now) : null;
      ...
      final access=_CapturedRestoreAccess(
        source,sourceStore,sessionStore,pinStore,expectedPin,isCurrent,
        () => home==null || home.source==source && !home.busy && home.failure==null && home.interaction.active && home.interaction.epoch==epoch && home.runtimeIdentity==identity && (layout?.isCurrent??true),
        layout==null ? null : {'coreId':layout.scope.coreId,'homeId':layout.scope.homeId,'userId':layout.scope.userId},
        layout==null ? null : _fingerprint(home!.account.session!.encodeStorage()),
        layout?.validUntil ?? now().toUtc().add(const Duration(minutes:5)),now,
      );
      access.checkLive();
      await access.checkDurable();
      access.checkLive();
      return access;
    } on BackupException {rethrow;} catch(_) {_unavailable();}
  }
}

final class _CapturedRestoreAccess implements BackupRestoreAccess {
  _CapturedRestoreAccess(this.source,this._sourceStore,this._sessionStore,this._pinStore,this._pin,this._current,this._runtime,Map<String,dynamic>? scope,this._sessionDigest,this.validUntil,this._clock)
    : _scope=scope==null ? null : Map.unmodifiable(scope);
  @override final HomeSource source;
  ...
  bool _retired=false;
  @override Map<String,dynamic> get ownership=>Map.unmodifiable({'source':source.name,if(_scope!=null) 'scope':_scope});
  @override void checkLive() {
    if(_retired) _unavailable();
    if(!_current() || !_runtime() || !_clock().toUtc().isBefore(validUntil)) {
      _retired=true;_unavailable();
    }
  }
  @override Future<void> checkDurable() async {
    try {
      ...
      if(_sessionDigest!=null) {
        ...
        if(session==null || session.authMutationPending || session.user.mustChangePassword || session.expiresSoon(_clock()) || _fingerprint(session.encodeStorage())!=_sessionDigest) _unavailable();
      }
      if(!_clock().toUtc().isBefore(validUntil)) _unavailable();
    } on BackupException {rethrow;} catch(_) {_unavailable();}
  }
  @override String toString()=>'CapturedRestoreAccess';
}
