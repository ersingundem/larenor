...
Never _expiredRestore() => throw const BackupException(
  'restore_expired',
  'Read the restore preview again.',
);
Never _changedRestore() => throw const BackupException(
  'restore_changed',
  'The selected destination changed. Read it again.',
);
Never _recoveryRequired() => throw const BackupException(
  'recovery_required',
...
class _ReadRecorder implements BackupStorage {
    ...
    if (_mutable(secret, key)) {
      ...
      if (values.length > 100 ||
          utf8.encode(_canonical(values)).length >
              BackupRepository._maxJournalBytes) {
        _recoveryRequired();
      }
    }
  ...
  @override
  Future<Object?> readPreference(String key) => read(false, key);
  @override
  Future<String?> readSecret(String key) async =>
      (await read(true, key)) as String?;
  @override
  Future<void> writePreference(String key, Object? value) async =>
      throw StateError('Read-only restore preparation');
  @override
  Future<void> writeSecret(String key, String? value) async =>
      throw StateError('Read-only restore preparation');
}

Future<PreparedBackupRestore> _prepareRestore(
  ...
  try {
    ...
    return await ConfigurationWrites.run(() async {
      ...
      final selected = <String, dynamic>{
        ...
        if (chosen.settings && frozen.hasSettings)
      ...
      if (access.source == HomeSource.verifiedCore &&
        ...
        throw const BackupException(
          ...
          'This backup targets Direct home data.',
        );
      }
      final homeBearing =
          narrowed.hasDashboard ||
      ...
      final origin = _sourceOriginNeeded(access.source, homeBearing)
          ? await _captureHaOrigin(repository, current)
          : null;
      ...
      await bound();
      final prepared = PreparedBackupRestore._(
        repository,
        access,
        owner,
        changes,
        ...
        _restoreDigest({
          'snapshot': json,
          'selection': [chosen.settings, chosen.dashboard, chosen.connections],
          'conflict': conflict.name,
        }),
        repository._now().toUtc().add(const Duration(minutes: 5)),
      );
...
final class PreparedBackupRestore {
  PreparedBackupRestore._(
    ...
    this._recoveryIdentity,
    this.summary,
    this._snapshotDigest,
    DateTime expires,
  ) : _haOrigin = origin == null ? null : Map.unmodifiable(origin),
      _readSet = Map.unmodifiable({
        ...reads.map((k, v) => MapEntry(k, _cloneValue(v))),
        if (origin != null)
          for (final e in origin.entries) 's:${e.key}': e.value,
      }),
      expiresAt = expires.isBefore(_access.validUntil)
          ? expires
          : _access.validUntil;
  ...
  final String _intentId = List.generate(
    16,
  ...
  Future<void> checkBeforeHandoff() async {
    ...
    if (_haOrigin != null) {
      await _verifyHaOrigin(_repository, _haOrigin, current);
    }
    await current();
  }
  ...
  Future<void> recoverAfterHandoff() => ConfigurationWrites.run(() async {
    ...
    try {
      Future<void> identity() async {
        if (await _readRecoveryIdentity(_repository, _ownership) !=
            _recoveryIdentity) {
          _recoveryRequired();
        }
      }
      ...
      await identity();
      if (await _repository._storage.readSecret(
      ...
      for (final change in _plan.changes) {
        if (!_same(
          await _readChange(_repository, change),
          before ? change.before : change.after,
        )) {
          _recoveryRequired();
        }
      }
      if (_haOrigin != null) {
        ...
        if (!before) {
          for (final c in _plan.changes) {
            if (c.secret && origin.containsKey(c.key)) {
              origin[c.key] = c.after as String?;
            }
          }
        }
        for (final entry in origin.entries) {
          if (await _repository._storage.readSecret(entry.key) != entry.value) {
            _recoveryRequired();
          }
        }
      }
      await identity();
      if (await _repository._storage.readSecret(_journalV2Key) != null) {
        _recoveryRequired();
      }
      await identity();
    } on BackupException {
  ...
  bool get targetsDirect =>
      _haOrigin != null ||
      _plan.changes.any((c) => _directTarget(c.secret, c.key));
  void retire() {
  ...
  void claimForHandoff(Object owner) {
    ...
    _state = 4;
    _access.checkLive();
    if (!_repository._now().toUtc().isBefore(expiresAt) ||
        !_same(_ownership, _access.ownership)) {
      _expiredRestore();
    }
    _owner = owner;
    _state = 1;
  ...
  Future<void> _verifyReadSet(Future<void> Function() current) async {
    for (final entry in _readSet.entries) {
      await current();
      final value = entry.key.startsWith('s:')
          ? await _repository._storage.readSecret(entry.key.substring(2))
          : await _repository._storage.readPreference(entry.key.substring(2));
      await current();
      if (!_same(value, entry.value)) _changedRestore();
    }
  }

  Future<void> applyAfterHandoff(
    Object owner, {
    required bool Function() isCurrentBoundary,
  }) => ConfigurationWrites.run(() async {
    if (_state != 1 || !identical(owner, _owner)) _expiredRestore();
    _state = 2;
    final expectedOrigin = _haOrigin == null
        ? null
        : Map<String, String?>.of(_haOrigin);
    void boundary() {
      if (!isCurrentBoundary() ||
          !_repository._now().toUtc().isBefore(expiresAt) ||
          !_same(_ownership, _access.ownership)) {
        _expiredRestore();
      }
    }
    ...
    Future<void> current() async {
      ...
      if (expectedOrigin != null) {
        await _verifyHaOrigin(_repository, expectedOrigin, () async {
          boundary();
        });
        await _access.checkDurable();
        boundary();
    ...
    try {
      ...
      final journal = _encodeV2(
        _repository,
        _plan.changes,
        _ownership,
        _snapshotDigest,
        _restoreDigest(_readSet),
        'applying',
        _intentId,
        _recoveryIdentity,
        _haOrigin,
      );
      final committed = _encodeV2(
        _repository,
        _plan.changes,
        _ownership,
        _snapshotDigest,
      ...
      if (await _repository._storage.readSecret(_journalV2Key) != journal) {
        _recoveryRequired();
      }
      try {
        await current();
        for (final change in _plan.changes) {
          ...
          if (change.secret &&
              expectedOrigin?.containsKey(change.key) == true) {
            expectedOrigin![change.key] = change.after as String?;
          }
          await _repository._write(change, previous: false);
          await current();
          if (!_same(await _readChange(_repository, change), change.after)) {
            _recoveryRequired();
          }
        }
        await _repository._requireStableConnections(_plan.services);
        ...
        await current();
        try {
          await _repository._storage.writeSecret(_journalV2Key, committed);
        } catch (_) {
          if (await _repository._storage.readSecret(_journalV2Key) !=
              committed) {
            rethrow;
          }
        }
        if (await _repository._storage.readSecret(_journalV2Key) != committed) {
          _recoveryRequired();
        }
      } catch (_) {
  ...
  @override
  String toString() => 'PreparedBackupRestore';
}

bool _sourceOriginNeeded(HomeSource source, bool homeBearing) =>
    source == HomeSource.directLocal && homeBearing;
Future<Map<String, String?>> _captureHaOrigin(
  BackupRepository repository,
  Future<void> Function() current,
) async {
  await repository._requireStableHaConnection();
  await current();
  final values = <String, String?>{};
  for (final key in backupConnectionFields['ha']!.values) {
    values[key] = await repository._storage.readSecret(key);
    await current();
  }
...
Future<void> _verifyHaOrigin(
  ...
  for (final entry in expected.entries) {
    final value = await repository._storage.readSecret(entry.key);
    await current();
...
void _checkRestoreOwner(Object? owner) {
  if (owner is! Map<String, dynamic> ||
      owner.isEmpty ||
      owner.keys.any((k) => !{'source', 'scope'}.contains(k)) ||
      !HomeSource.values.any((v) => v.name == owner['source'])) {
    _recoveryRequired();
  }
  if (owner['source'] == 'verifiedCore' && !owner.containsKey('scope')) {
    _recoveryRequired();
  }
...
Future<Object?> _readChange(BackupRepository repo, _Change c) => c.secret
    ? repo._storage.readSecret(c.key)
    : repo._storage.readPreference(c.key);
String _encodeV2(
  ...
  if (origin != null) {
    ...
    for (final change in changes) {
      if (change.secret && current.containsKey(change.key)) {
        current[change.key] = change.after as String?;
      }
      originDigests.add(_originDigest(intentId, current));
    }
  }
  final value = {
    ...
    'owner': owner,
    'recovery': {
      'sessionDigest': identity,
      'originDigests': origin == null ? null : originDigests,
    },
...
(String, List<_Change>, Map<String, dynamic>) _decodeV2(
  BackupRepository repo,
  String raw,
) {
  try {
    if (utf8.encode(raw).length > BackupRepository._maxJournalBytes) {
      _recoveryRequired();
    }
    ...
    if (data is! Map<String, dynamic> ||
        ...
        data['version'] is! int ||
        data['version'] != 2 ||
        !{'applying', 'committed'}.contains(data['phase'])) {
      _recoveryRequired();
    }
    final digest = data.remove('digest');
    if (digest != _restoreDigest(data)) _recoveryRequired();
    for (final k in ['sourceDigest', 'targetDigest']) {
      if (data[k] is! String ||
          !RegExp(r'^[a-f0-9]{64}$').hasMatch(data[k] as String)) {
        _recoveryRequired();
      }
    }
    if (data['intentId'] is! String ||
        !RegExp(r'^[a-f0-9]{32}$').hasMatch(data['intentId'] as String)) {
      _recoveryRequired();
    }
    ...
    for (final row in list) {
      if (row is! Map ||
          row.length != 4 ||
          !row.keys.toSet().containsAll({'secret', 'key', 'before', 'after'})) {
        _recoveryRequired();
      }
    }
    ...
    if (recovery is! Map<String, dynamic> ||
        recovery.length != 2 ||
        !recovery.keys.toSet().containsAll({
          'sessionDigest',
          'originDigests',
        })) {
      _recoveryRequired();
    ...
    if (core
        ? !hash(recovery['sessionDigest'])
        : recovery['sessionDigest'] != null) {
      _recoveryRequired();
    }
    ...
    if (origins != null &&
        (core ||
            origins is! List ||
            origins.length != changes.length + 1 ||
            !origins.every(hash))) {
      _recoveryRequired();
    }
...
Future<void> _requireJournal(BackupRepository repo, String expected) async {
  if (await repo._storage.readSecret(_journalV2Key) != expected) {
    _recoveryRequired();
  }
}

String _originDigest(String intent, Map<String, String?> origin) =>
    _restoreDigest({
      'domain': 'larenor.restore.origin.v2',
      'intentId': intent,
      'origin': origin,
    });
Future<String?> _readRecoveryIdentity(
  BackupRepository repo,
  Map<String, dynamic> owner,
) async {
  ...
  if (repo._recoverySourceStore != null) {
    ...
    source = switch (await repo._storage.readPreference(
  ...
  if (repo._recoverySessionStore != null) {
    ...
    session = stored == null
        ? null
        : ServerSession.decodeStorage(stored.encodeStorage());
  } else {
  ...
  if (session == null ||
      ...
      session.expiresSoon(repo._now())) {
    _recoveryRequired();
  }
  if (!_same(owner['scope'], {
  ...
  return _restoreDigest({
    'domain': 'larenor.restore.session.v2',
...
Future<List<Object?>> _recoveryView(
  BackupRepository repo,
  String raw,
  String phase,
  List<_Change> changes,
  ...
  Future<void> context() async {
    if (await _readRecoveryIdentity(repo, owner) != binding['sessionDigest']) {
      _recoveryRequired();
    }
  }
  ...
  Future<String?> origin() async {
    ...
    final values = <String, String?>{};
    for (final key in backupConnectionFields['ha']!.values) {
      values[key] = await repo._storage.readSecret(key);
    }
  ...
  await context();
  final values = <Object?>[
    for (final c in changes) _cloneValue(await _readChange(repo, c)),
  ];
  ...
  var matched = false;
  for (
    var prefix = phase == 'committed' ? changes.length : 0;
    prefix <= changes.length;
    prefix++
  ) {
    if (List.generate(changes.length, (i) => i).every(
          (i) => _same(
            values[i],
            i < prefix ? changes[i].after : changes[i].before,
          ),
        ) &&
        (observedOrigin == null ||
            (binding['originDigests'] as List)[prefix] == observedOrigin)) {
      matched = true;
      break;
    }
  }
  ...
  for (var i = 0; i < changes.length; i++) {
    if (!_same(await _readChange(repo, changes[i]), values[i])) {
      _recoveryRequired();
    }
  }
...
Future<bool> _recoverV2(BackupRepository repo, {Set<String>? expected}) async {
  try {
    ...
    final finalValues = await _recoveryView(
      repo,
      raw,
      phase,
      changes,
    ...
    for (var i = 0; i < changes.length; i++) {
      if (!_same(
        finalValues[i],
        phase == 'applying' ? changes[i].before : changes[i].after,
      )) {
        _recoveryRequired();
      }
    }
    try {
      await repo._storage.writeSecret(_journalV2Key, null);
    } catch (_) {
      if (await repo._storage.readSecret(_journalV2Key) != null) rethrow;
    }
    if (await repo._storage.readSecret(_journalV2Key) != null) {
      _recoveryRequired();
    }
    return true;
  } on BackupException {
