...
class SshCredential {
  ...
  @override
  String toString() => 'SshCredential(redacted)';
}
...
class SshSecurityStore {
  ...
  Future<void> _profile(RemoteProfile p, void Function() check) async {
    ...
    check();
    if (p.protocol != RemoteProtocol.ssh ||
        p.username.isEmpty ||
        !snapshot.profiles.any((v) => reference(v) == reference(p))) {
      throw const SshFailure('profile_changed');
    }
  }
  ...
  Future<T> _run<T>(
    ...
    return ConfigurationWrites.run(() async {
      try {
        ...
        rethrow;
      } on RemoteProfilesFailure catch (e) {
        throw SshFailure(e.code == 'retired' ? 'retired' : 'profile_changed');
      } catch (_) {
        check();
  ...
  static void _credential(SshCredential v) {
    if (v.secret.isEmpty ||
        utf8.encode(v.secret).length >
            (v.kind == SshCredentialKind.password ? 4096 : 32768) ||
        v.secret.contains('\u0000') ||
        (v.passphrase.length > 1024 ||
            utf8.encode(v.passphrase).length > 1024) ||
        (v.kind == SshCredentialKind.password && v.passphrase.isNotEmpty)) {
      throw const SshFailure('invalid_credential');
    }
  }

  static void _pin(SshHostPin p) {
    if (!RegExp(r'^[a-zA-Z0-9@._+-]{1,80}$').hasMatch(p.type) ||
        !RegExp(r'^SHA256:[A-Za-z0-9+/]{43}$').hasMatch(p.fingerprint)) {
      throw const SshFailure('invalid_record');
    }
  }
  ...
  Future<void> saveCredential(
    ...
    final old = await _read(p, 'credential', check);
    if (old != null) _decodeCredential(old);
    await _write(p, 'credential', {
      'kind': v.kind.name,
      'secret': v.secret,
  ...
  SshCredential _decodeCredential(Map<String, dynamic> v) {
    ...
    final c = SshCredential(
      SshCredentialKind.values.byName(v['kind']),
      v['secret'],
      passphrase: v['passphrase'],
    );
    _credential(c);
  ...
  Future<SshCredential?> readCredential(
    ...
    final v = await _read(p, 'credential', check);
    return v == null ? null : _decodeCredential(v);
  });
  ...
  Future<SshHostPin?> readPin(
    ...
    final v = await _read(p, 'pin', check);
    return v == null ? null : _decodePin(v);
  });
  Future<void> trust(
    RemoteProfile p,
    SshHostPin pin, {
