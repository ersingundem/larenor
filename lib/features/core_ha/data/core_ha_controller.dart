...
class CoreHaController extends ChangeNotifier {
  ...
  bool _action(bool Function() check) {
    try {
      return check();
    } catch (_) {
      return false;
  ...
  Future<void> _run(
    Future<void> Function(CoreHaApi Function(HomeResourceRecord)) action, {
    bool write = false,
    bool Function()? guard,
    ...
    await _run((api) async {
      ...
      if (admin) {
        ...
        if ((value?.revision ?? 0) < _bindingRevision) {
          throw const LarenorServerException('ha_binding_changed');
        }
        ...
        _resourceRevision = value.resourceRevision;
        _aclRevision = value.aclRevision;
        _snapshot = value;
        _arm(started + Duration(milliseconds: value.remainingTtlMs));
      }
      loaded = true;
    });
  }
  ...
  Future<void> prepare(
    ...
    if (!canPreview ||
        !services.any((s) => identical(s, service)) ||
        !_action(isCurrent)) {
      return;
    }
    ...
    await _run((api) async {
      ...
      final value = await api(target)
          .preview(service: service, entityId: entityId, existing: existing);
      _preview = value;
      _stale = false;
  ...
  Future<void> confirm(
    ...
    if (!canConfirm || !identical(value, preview) || !_action(isCurrent)) {
      if (preview != null && _deadline != null && monotonic() >= _deadline!) {
        _expire();
        _emit();
      }
      return;
