...
final class CoreHaTransferApi {
  ...
  String get _path =>
      '/admin/home-assistant/${target.context.coreId}/${target.context.homeId}/resources/${target.id}/direct-migration';
  ...
  Future<CoreHaTransferPreview> preview({
    required String requestId,
    required String name,
    required String baseUrl,
    required String credential,
    ...
    _check();
    final value = CoreHaTransferPreview.fromJson(
      _envelope(raw, 'preview'),
      target: target,
      requestId: requestId,
    );
    if (value.commit.service.name != input['name'] ||
        value.commit.service.baseUrl != input['baseUrl'] ||
        value.commit.binding.entityId != entityId) {
      throw const LarenorServerException('invalid_response');
    }
    return value;
  });
  Future<CoreHaTransferReceipt> confirm(
    ...
    if (preview.commit.binding.target.context != target.context ||
        preview.commit.binding.target.id != target.id ||
        preview.commit.service.name != input['name'] ||
        preview.commit.service.baseUrl != input['baseUrl'] ||
        preview.commit.binding.entityId != entityId) {
      throw const LarenorServerException('invalid_request');
    }
    final raw = await transport.request(
      'POST',
      '$_path/confirm',
      token: token,
      body: {...input, 'previewId': preview.id},
    );
    _check();
    final receipt = CoreHaTransferReceipt.fromJson(
      _envelope(raw, 'receipt'),
      target: target,
      requestId: preview.requestId,
    );
    if (!receipt.sameCommit(preview.commit)) {
      throw const LarenorServerException('invalid_response');
    }
    return receipt;
  });
  Future<CoreHaTransferReceipt?> result(CoreHaTransferPreview preview) =>
      _run(() async {
        try {
          final raw = await transport.request(
            'GET',
            '$_path/results/${preview.requestId}',
            token: token,
          );
          _check();
          final receipt = CoreHaTransferReceipt.fromJson(
  ...
  @override
  String toString() => 'CoreHaTransferApi';
}
