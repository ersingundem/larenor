...
final coreHaTransferControllerProvider = Provider.autoDispose
    .family<CoreHaTransferController, CoreHaTransferOwner>((ref, owner) {
      owner._bind(ref);
      ...
      bool current() =>
          ...
          identical(ref.read(dashboardRepositoryProvider), repository) &&
          identical(ref.read(credentialsStoreProvider), credentials) &&
          home?.runtimeIdentity == identity &&
          home?.interaction.epoch == epoch &&
          home?.account.generation == generation &&
          owner.isCurrent;
      final controller = CoreHaTransferController(
        home: home,
        repository: repository,
        credentials: credentials,
        factory: ref.watch(coreHaApiFactoryProvider),
        clock: ref.watch(coreHaClockProvider),
        monotonic: ref.watch(coreHaMonotonicProvider),
