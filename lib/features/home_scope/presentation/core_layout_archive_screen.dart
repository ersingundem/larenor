...
class CoreLayoutArchiveScreen extends ConsumerStatefulWidget {
  const CoreLayoutArchiveScreen({
    super.key,
    required this.gateCurrent,
    required this.runFileDialog,
  });
  final bool Function() gateCurrent;
  final SettingsFileDialogRunner runFileDialog;
  @override
  ConsumerState<CoreLayoutArchiveScreen> createState() =>
      _CoreLayoutArchiveScreenState();
}

class _CoreLayoutArchiveScreenState
    extends ConsumerState<CoreLayoutArchiveScreen>
    with WidgetsBindingObserver {
  final _scroll = ScrollController();
  final _password = TextEditingController(),
      _repeat = TextEditingController(),
      _openPassword = TextEditingController();
  ...
  void initState() {
    ...
    _session = _home?.account.session;
    _accountGeneration = _home?.account.generation;
    _clock = ref.read(homeLayoutClockProvider);
    _openedAt = _clock();
  ...
  bool _sameOwner() {
    ...
    return mounted &&
        ...
        _home.account.generation == _accountGeneration &&
        elapsed >= Duration.zero &&
        elapsed < CoreLayoutArchiveController.lifetime;
  }
  ...
  void didChangeDependencies() {
    ...
    } else if (!identical(container, _container)) {
      _retire();
    }
    final next = AppInteractionScope.maybeOf(context);
    if (!identical(next, _interaction)) {
      final old = _interaction;
      old?.removeListener(_interactionChanged);
      _interaction = next;
  ...
  bool _bind() {
    ...
    final access = homeLayoutAccess(_home, clock: _clock),
    ...
    _controller = CoreLayoutArchiveController(
      destination: repository,
      clock: _clock,
      isCurrent: () => binding == _binding && _current(),
    );
    return true;
  }
  ...
  String _failure(
    ...
    if (error is CoreLayoutArchiveException &&
        error.code == 'unsupported_layout') {
      return l10n.coreLayoutArchiveUnsupported;
    }
    if (error is CoreLayoutArchiveCodecException &&
        error.code == 'invalid_passphrase') {
      return l10n.coreLayoutArchiveInvalidPassword;
    }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_allowed(generation)) {
        _report(_failure(error, l10n), error: true);
      }
    } finally {
  ...
  Future<void> _pick(int generation) async {
    ...
    try {
      ...
      if (!_current()) return;
      if (bytes == null) {
        _report(l10n.coreLayoutArchiveCancelled);
        return;
      }
      if (bytes.isEmpty || bytes.length > CoreLayoutArchiveCodec.maxFileBytes) {
  ...
  Future<void> _export(int generation) async {
    ...
    try {
      CoreLayoutArchiveCodec.validatePassphrase(password, settingsPin: _pin);
      if (password != _repeat.text) {
        throw const CoreLayoutArchiveCodecException('invalid_passphrase');
      }
    } catch (_) {
      _report(l10n.coreLayoutArchiveInvalidPassword, error: true);
      return;
    }
    final operation = ++_generation;
    var dispatchedFile = false;
    _busy = true;
    _message = null;
    _changed();
    try {
      if (!await _durablePin(operation)) return;
      CoreLayoutArchiveV1? archive = await _controller!.capture();
      if (!_allowed(operation)) return;
      final bytes = await ref
          .read(coreLayoutArchiveCodecProvider)
      ...
      if (_current()) {
        _report(
          saved == null
              ? l10n.coreLayoutArchiveCancelled
              : l10n.coreLayoutArchiveSaved,
        );
      }
    } catch (error) {
      if (dispatchedFile ? _current() : _allowed(operation)) {
        _report(_failure(error, l10n), error: true);
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _decrypt(int generation) async {
    ...
    final bytes = Uint8List.fromList(_file!),
        password = _openPassword.text,
    ...
    try {
      ...
      if (!_allowed(operation)) return;
      final preview = await _controller!.preview(archive);
      if (!_allowed(operation)) return;
      _preview = preview;
      _file = null;
      _reviewTop();
    } catch (error) {
      if (_allowed(operation)) {
        _report(_failure(error, l10n, decrypting: true), error: true);
      }
    } finally {
      if (mounted) {
        _openPassword.clear();
        if (operation == _generation) _busy = false;
        _changed();
      }
    }
  }
  ...
  Future<void> _confirm(int generation) async {
    ...
    final phase = ++_generation;
    var answered = false;
    late final CupertinoDialogRoute<bool> route;
    void answer(bool value) {
      if (answered || !identical(_dialog, route) || !_allowed(phase)) return;
      answered = true;
    ...
    route = CupertinoDialogRoute<bool>(
      ...
      builder: (_) => CupertinoAlertDialog(
        title: Text(l10n.coreLayoutArchiveReplace),
        content: Text(l10n.coreLayoutArchiveConfirm),
        actions: [
          _ArchiveDialogAction(
            actionKey: const ValueKey('core-layout-archive-confirm-cancel'),
            onPressed: () => answer(false),
            label: l10n.commonCancel,
          ),
          _ArchiveDialogAction(
            actionKey: const ValueKey('core-layout-archive-confirm'),
            isDestructiveAction: true,
            onPressed: () => answer(true),
            label: l10n.coreLayoutArchiveReplace,
          ),
        ],
      ),
    );
    ...
    try {
      if (!await _durablePin(operation)) return;
      await _controller!.apply(preview);
      if (!_allowed(operation)) return;
      _rooms = preview.archivedRoomNames;
      ref.invalidate(dashboardLayoutProvider);
      _report(l10n.coreLayoutArchiveApplied);
    } catch (error) {
      if (_allowed(operation)) _report(_failure(error, l10n), error: true);
    } finally {
      if (mounted && operation == _generation) {
        _busy = false;
        _changed();
      }
    }
  ...
  Widget _button(
    ...
    final focusColor = CupertinoTheme.brightnessOf(context) == Brightness.dark
  ...
  Widget _roomSection(
    ...
    header: Semantics(container: true, header: true, child: Text(label)),
    children: [
      if (names.isEmpty)
        Padding(
          padding: const EdgeInsets.all(16),
          child: Text(l10n.coreLayoutArchiveEmpty),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    if (!_pinResolved && !pin.isLoading && !pin.hasError && pin.hasValue) {
      _pinResolved = true;
      _pin = pin.value;
    }
    ...
    ref.listen(windowPolicySnapshotProvider, (_, next) {
      ...
      if (next.isLoading ||
          next.hasError ||
          !next.hasValue ||
          next.requireValue.supported &&
              (!next.requireValue.isResumed ||
                  !next.requireValue.hasWindowFocus ||
                  next.requireValue.isPictureInPicture)) {
        _retire(permanent: !_fileDialog);
      }
    });
    ...
    if (_started && !_fileDialog && !_baseCurrent() && !_closed) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && !_fileDialog && !_baseCurrent()) _retire();
      });
    }
    final enabled = !_busy && _current(), generation = _generation;
    return AppPageScaffold(
      key: const ValueKey('core-layout-archive-screen'),
      navigationBar: CupertinoNavigationBar(
        automaticallyImplyLeading: false,
        middle: Text(l10n.coreLayoutArchiveTitle),
      ),
      child: SafeArea(
        child: Center(
          child: ConstrainedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                Padding(
                  padding: const EdgeInsets.all(12),
                  child: Text(l10n.coreLayoutArchiveHint),
                ),
                if (_closed)
                  Padding(
                    padding: const EdgeInsets.all(16),
                    child: Text(l10n.coreLayoutArchiveExpired),
                  )
                else ...[
                  if (_message != null)
                    Padding(
                      ...
                      child: Semantics(
                        ...
                        child: Text(
                          _message!,
                          key: const ValueKey('core-layout-archive-message'),
                          style: TextStyle(
                            color: _error
                  ...
                  if (_preview case final preview?) ...[
                    Column(
                      key: const ValueKey('core-layout-archive-preview'),
                      children: [
                        _roomSection(
                          l10n.coreLayoutArchiveCurrent,
                          preview.currentRoomNames,
                          l10n,
                        ),
                        _roomSection(
                          l10n.coreLayoutArchiveSavedRooms,
                          preview.archivedRoomNames,
                          l10n,
                        ),
                      ],
                    ),
                    _button(
                      'core-layout-archive-replace',
                      l10n.coreLayoutArchiveReplace,
                      enabled ? () => _confirm(generation) : null,
                      primary: true,
                    ),
                  ] else ...[
                    if (_rooms case final rooms?)
                      _roomSection(l10n.coreLayoutArchiveCurrent, rooms, l10n),
                    _button(
                      'core-layout-archive-refresh',
                      l10n.coreLayoutArchiveRefresh,
                      enabled
                          ? () {
                              if (_allowed(generation)) _load();
                            }
                          : null,
                    ),
                  ],
                  SettingsSection(
                    header: Semantics(
                      container: true,
                      header: true,
                      child: Text(l10n.coreLayoutArchiveImport),
                    ),
                    children: [
                      _button(
                        'core-layout-archive-pick',
                        l10n.coreLayoutArchiveImport,
                        enabled ? () => _pick(generation) : null,
                      ),
                      if (_file != null) ...[
                        _field(
                          'core-layout-archive-open-password',
                          l10n.coreLayoutArchivePassword,
                          _openPassword,
                        ),
                        _button(
                          'core-layout-archive-decrypt',
                          l10n.coreLayoutArchiveOpen,
                          enabled ? () => _decrypt(generation) : null,
                          primary: true,
                        ),
                      ],
                    ],
                  ...
                  SettingsSection(
                    header: Semantics(
                      container: true,
                      header: true,
                      child: Text(l10n.coreLayoutArchiveExport),
                    ),
                    footer: Text(l10n.coreLayoutArchivePasswordHint),
                    children: [
                      _field(
                      ...
                      _field(
                        ...
                        l10n.coreLayoutArchiveRepeat,
                        _repeat,
                      ),
                      _button(
                        'core-layout-archive-export',
                        l10n.coreLayoutArchiveExport,
                        enabled ? () => _export(generation) : null,
                        primary: true,
                      ),
                    ],
                  ),
...
class _ArchiveDialogAction extends StatefulWidget {
  ...
  @override
  State<_ArchiveDialogAction> createState() => _ArchiveDialogActionState();
}
