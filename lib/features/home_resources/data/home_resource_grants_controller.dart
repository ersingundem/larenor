...
class HomeResourceGrantsController extends ChangeNotifier {
  ...
  bool get _sourceCurrent => !_disposed && _visible && windowCurrent() &&
  ...
  ServerSession? get _ready {
    ...
    if (!_sourceCurrent || !account.initialized || account.working ||
        account.hasPendingContext || session == null ||
        session.context != target.context || session.authMutationPending ||
        session.user.mustChangePassword || !session.user.canAdminister) return null;
    return session;
  }
  ...
  void _clear() { users = const []; snapshot = null; }
  void _retire() {
    epoch++;
    busy = false;
    _preparing = false;
  ...
  void setVisible(bool value) {
    ...
    _visible = value; _retire(); _attempted = false; _emit(); _startIfReady();
  }
  void _startIfReady() {
  ...
  Future<void> _run(AdminUser? selected, HomeResourcePermission? permission,
    ...
    final generation = home.account.generation, homeEpoch = home.interaction.epoch,
        operation = ++epoch;
    bool current() => !_disposed && epoch == operation && _sourceCurrent &&
        home.interaction.epoch == homeEpoch && home.account.isCurrent(generation) &&
        ownerCurrent();
    ...
    if (!writing) _clear();
    busy = true; failure = null; outcome = null; _attempted = true;
    _preparing = true; _preparationCurrent = current; _emit();
    HomeResourceGrants? result;
    List<AdminUser>? nextUsers;
    try {
      await home.account.withSession((_, session) async {
        void check() {
          if (!current() || !sameScope(session) || !identical(_ready, session) || !fresh) {
            throw const LarenorServerException('cancelled');
          }
        }
        check();
        _preparing = false; _boundSession = session;
        _expiry?.cancel();
        ...
        final transport = factory(session.endpoint); _transport = transport;
        final api = HomeResourceGrantsApi(transport, session.accessToken, session.context!);
        try {
          if (writing) {
            result = await api.set(before!, subjectId: selected.id, permission: permission!);
            check();
          } else {
            nextUsers = await ServerAdminApi(transport, session.accessToken).users();
            check();
            if (nextUsers!.any((user) => !HomeResourceGrants.isSubjectId(user.id))) {
      ...
      if (writing) outcome = permission == HomeResourcePermission.none
          ? HomeResourceGrantOutcome.revoked : HomeResourceGrantOutcome.saved;
    } catch (error) {
      if (current()) {
        _clear();
        failure = error is LarenorServerException ? error.code : 'connection_failed';
        if (writing) outcome = switch (failure) {
          'revision_conflict' || 'conflict' => HomeResourceGrantOutcome.conflict,
          'invalid_request' || 'forbidden' || 'not_found' => HomeResourceGrantOutcome.failed,
      ...
      if (!_disposed && epoch == operation) {
        _transport?.close(); _transport = null;
        _preparing = false; _preparationCurrent = null; busy = false; _emit();
      }
    }
  }
  @override void dispose() {
    home.removeListener(_changed); home.account.removeListener(_changed);
