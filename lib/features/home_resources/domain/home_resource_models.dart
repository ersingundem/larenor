...
String _hex(Object? value, int length) {
  if (value is! String || value.length != length || !RegExp(r'^[0-9a-f]+$').hasMatch(value)) _invalid();
  return value;
}
...
final _edgeWhitespace = RegExp(r'^[\x09-\x0d\x1c-\x20\u0085\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000]+|[\x09-\x0d\x1c-\x20\u0085\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000]+$');
...
final class HomeResourceRecord {
  const HomeResourceRecord._({required this.context, required this.id, required this.kind,
    required this.label, required this.order, required this.revision,
    required this.aclRevision, required this.canWrite});
  factory HomeResourceRecord.fromJson(Object? raw, {required ServerContext expectedContext}) {
    final value = _object(raw, {'ref', 'label', 'order', 'revision', 'aclRevision', 'permissions'});
    final ref = _object(value['ref'], {'schemaVersion', 'coreId', 'homeId', 'kind', 'id'});
    final context = ServerContext.fromJson({for (final key in ['schemaVersion', 'coreId', 'homeId']) key: ref[key]});
    ...
    if (label is! String || label.runes.isEmpty || label.runes.length > 80 ||
        label.runes.any((rune) => rune < 32 || rune == 127 || rune >= 0xd800 && rune <= 0xdfff)) _invalid();
    ...
    if (permissions['read'] != true || permissions['write'] is! bool) _invalid();
    return HomeResourceRecord._(
      context: context, id: _hex(ref['id'], 32), kind: kind, label: trimmed,
...
final class HomeResourcePage {
  const HomeResourcePage._(this.context, this.entries, this.snapshot, this.nextAfter);
  static const pageSize = 25;
  static const maximumRecords = 512;
  factory HomeResourcePage.fromJson(Object? raw, {
    required ServerContext expectedContext, String? after, String? expectedSnapshot,
  }) {
    ...
    if (expectedSnapshot != null && snapshot != _hex(expectedSnapshot, 64)) _invalid();
    if (after != null) {
      _hex(after, 32);
      if (expectedSnapshot == null) _invalid();
    }
    final rawEntries = value['entries'];
    if (rawEntries is! List || rawEntries.length > pageSize) _invalid();
    ...
    final next = value['nextAfter'] == null ? null : _hex(value['nextAfter'], 32);
    if (next != null && (entries.isEmpty || next != entries.last.id)) _invalid();
    return HomeResourcePage._(context, List.unmodifiable(entries), snapshot, next);
  }
  final ServerContext context;
