  ...
  Future<Map<String, dynamic>?> request(
    ...
    if (queryParameters != null && queryParameters.isNotEmpty) {
      ...
      final homeResourcesQuery =
          method == 'GET' &&
          RegExp(r'^/home-resources/[0-9a-f]{32}/[0-9a-f]{32}$').hasMatch(path) &&
          (!queryParameters.containsKey('after') ||
              queryParameters.containsKey('expectedSnapshot')) &&
          queryParameters.entries.every((entry) => switch (entry.key) {
            'limit' => RegExp(r'^[1-9][0-9]{0,2}$').hasMatch(entry.value) &&
                (int.tryParse(entry.value) ?? 0) <= 100,
            'after' => RegExp(r'^[0-9a-f]{32}$').hasMatch(entry.value),
            'expectedSnapshot' => RegExp(r'^[0-9a-f]{64}$').hasMatch(entry.value),
            _ => false,
          });
      ...
      if (!readQuery && !forgetQuery && !jobsQuery && !mediaQuery && !homeResourcesQuery) {
        throw const LarenorServerException('invalid_request');
      }
