...
final class DirectCredentialRecord {
  DirectCredentialRecord({
    ...
    DirectHomeAccess? access,
       ...
       // Preserve the public named parameter while keeping the field private.
  ...
  void _check([bool Function()? isCurrent]) {
    ...
    try {
      ...
      /* Private action failure is never a storage diagnostic. */
    }
  ...
  Future<Map<String, String?>> readFields() => ConfigurationWrites.run(
    () async {
      _check();
      final pending = await _call(
        () => _storage.read(key: service.pendingMutationKey),
      );
      _check();
      if (pending != null) {
  ...
  Future<void> _mutate(
    ...
    _check(isCurrent);
    await _call(
      () => _storage.write(key: service.pendingMutationKey, value: '1'),
      mutation: true,
      isCurrent: isCurrent,
    );
    for (final field in service._fields.entries) {
      ...
      if (values == null) {
        await _call(
          () => _storage.delete(key: field.value),
          mutation: true,
          isCurrent: isCurrent,
        );
      } else {
        await _call(
          () => _storage.write(key: field.value, value: values[field.key]!),
          mutation: true,
          isCurrent: isCurrent,
        );
      }
    ...
    _check(isCurrent);
    await _call(
      () => _storage.delete(key: service.pendingMutationKey),
      mutation: true,
      isCurrent: isCurrent,
    );
  });
